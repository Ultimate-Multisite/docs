...
## Cap Captcha: Samostalna zaštita koja jednostavno radi {#cap-captcha-self-hosted-protection-that-just-works}
...
## Zaštita od testiranja kartica u WooCommerceu {#woocommerce-card-testing-protection}
...
## Još uvijek vam treba Google reCAPTCHA ili hCaptcha? {#still-need-google-recaptcha-or-hcaptcha}
...
## Izgrađeno za skalabilnost {#built-for-scale}
...
## Da li moram nešto da konfigurišem? {#do-i-need-to-configure-anything}
...
## Da li mi trebaju API ključevi za Cap Captcha? {#do-i-need-api-keys-for-cap-captcha}
...
## Hoće li ovo usporiti moju naplatu? {#will-this-slow-down-my-checkout}

Cap Captcha je samo 20KB i pokreće izazove dokaza rada u pozadini. Vaši kupci neće primijetiti nikakav kašnjenje.

## Da li ovo štiti od napada testiranjem kartica? {#does-this-protect-against-card-testing-attacks}
...
## Mogu li koristiti različite captcha provajdere na različitim sajtovima? {#can-i-use-different-captcha-providers-on-different-sites}

Captcha provajder je postavljen na nivou cijele mreže, ali možete podesiti nivo težine u zavisnosti od potreba.

## Je li ovo usklađeno sa GDPR-om? {#is-this-gdpr-compliant}
...
## Rate Limiting (Ograničavanje brzine) {#rate-limiting}
...
#### IP Bijela Lista Filter (IP Whitelist Filter) {#ip-whitelist-filter}
...
#### Akcija prije blokiranja (Pre-Block Action) {#pre-block-action}
...
## Detekcija IP adrese klijenta i pouzdani proxyji {#client-ip-detection--trusted-proxies}
...
### Model povjerenja (Trust Model) {#trust-model}
...
#### Konfiguracija pouzdanih proxyja {#trusted-proxy-configuration}
...
#### Automatska detekcija pri prvom uključivanju {#first-enable-auto-detection}
