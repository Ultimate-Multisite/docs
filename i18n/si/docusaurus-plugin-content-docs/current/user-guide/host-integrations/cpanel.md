...
### 1. ඔබේ cPanel තොරතුරු ලබා ගැනීම (Get Your cPanel Credentials) {#1-get-your-cpanel-credentials}
...
### 2. wp-config.php හි Constant එකතු කිරීම (Add Constants to wp-config.php) {#2-add-constants-to-wp-configphp}
...
### 3. සම්බන්ධතාවය සක්‍රිය කිරීම (Enable the Integration) {#3-enable-the-integration}
...
### Addon Domains (අමතර වෙබ් අඩවි) {#addon-domains}
...
### API Connection Issues (API සම්බන්ධතා ගැටළු) {#api-connection-issues}
...
### Domain Not Added (Domain එක එක් කර නොමැත) {#domain-not-added}
