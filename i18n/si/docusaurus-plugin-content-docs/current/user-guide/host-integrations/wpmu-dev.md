...
### 1. WPMU DEV Hosting තහවුරු කිරීම (Verify WPMU DEV Hosting) {#1-verify-wpmu-dev-hosting}
...
### 2. Integration එක සක්‍රීය කිරීම (Enable the Integration) {#2-enable-the-integration}
...
### ප්‍රදේශ ඉවත් කිරීම (Domain Removal) {#domain-removal}
...
### API認証 (API Authentication) {#api-authentication}
