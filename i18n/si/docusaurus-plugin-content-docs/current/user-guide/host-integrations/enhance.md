...
### පද්ධති අවශ්‍යතා (System Requirements) {#system-requirements}
...
### API ප්‍රවේශය (API Access) {#api-access}
API tokens නිර්මාණය කිරීමට Enhance Control Panel හි administrator access ඔබ සතු විය යුතුය.

## ඔබේ API Credentials ලබා ගැනීම (Getting Your API Credentials) {#getting-your-api-credentials}

### 1. API Token එකක් නිර්මාණය කිරීම (Create an API Token) {#1-create-an-api-token}
...
### 2. உங்கள் Organization ID ஐ பெறுதல் {#2-get-your-organization-id}
...
### 3. உங்கள் Server ID ஐ பெறுதல் {#3-get-your-server-id}
...
### 4. உங்கள் API URL ஐப் பெறுதல் {#4-get-your-api-url}
...
### தேவையான மாறிலிகள் (Required Constants) {#required-constants}
...
### Integration Wizard හරහා සැකසීම {#setup-via-integration-wizard}
...
## අමතර WordPress සැකසුම් {#additional-wordpress-configuration}
...
### .htaccess වින්‍යාසය {#htaccess-configuration}
...
### ડોමේනයක් මැප කරන විට {#when-a-domain-is-mapped}
...
### ડોමේනයක් ඉවත් කිරීමේදී (When a Domain is Removed) {#when-a-domain-is-removed}
...
### DNS සහ SSL පරීක්ෂා කිරීම (DNS and SSL Checking) {#dns-and-ssl-checking}
...
## සැකසුම තහවුරු කිරීම (Verifying Setup) {#verifying-setup}

### සම්බන්ධතාවය පරීක්ෂා කිරීම (Test the Connection) {#test-the-connection}
...
### ડોමේනයක් මැප් කිරීමෙන් පසු (After Mapping a Domain) {#after-mapping-a-domain}
...
### ડોමේන් එක නොඑන විට (Domain Not Added) {#domain-not-added}
...
### SSL සහතික ගැටළු (SSL Certificate Issues) {#ssl-certificate-issues}
...
### DNS පරීක්ෂණ කාලපරිච්ඡේදය (DNS Check Interval) {#dns-check-interval}
...
### প্রমাণীকরণ ত্রুটি (Authentication Errors) {#authentication-errors}
...
## API রেফারেন্স (API Reference) {#api-reference}
...
### ব্যবহৃত সাধারণ এন্ডপয়েন্টসমূহ (Common Endpoints Used) {#common-endpoints-used}
...
### সম্পূর্ণ API ডকুমেন্টেশন (Full API Documentation) {#full-api-documentation}
সম্পূর্ণ API ডকুমেন্টেশন: [https://apidocs.enhance.com](https://apidocs.enhance.com)

## সেরা অনুশীলনসমূহ (Best Practices) {#best-practices}

### ආරක්ෂාව (Security) {#security}
...
### කාර්ය සාධනය (Performance) {#performance}
...
### අධීක්ෂණය (Monitoring) {#monitoring}
...
## අමතර සම්පත් (Additional Resources) {#additional-resources}
