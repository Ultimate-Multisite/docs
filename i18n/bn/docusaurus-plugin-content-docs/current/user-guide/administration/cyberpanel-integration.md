...
---
# CyberPanel ইন্টিগ্রেশন {#cyberpanel-integration}

এই গাইডটি ব্যাখ্যা করে যে কীভাবে Ultimate Multisite CyberPanel ইন্টিগ্রেশন কনফিগার করবেন, যাতে আপনার নেটওয়ার্কের ম্যাপ করা ডোমেইনগুলি স্বয়ংক্রিয়ভাবে CyberPanel-এ ভার্চুয়াল হোস্ট হিসেবে যুক্ত (এবং সরানো) হয়। এর সাথে Let's Encrypt এর মাধ্যমে ঐচ্ছিক অটো-SSL প্রোভিশনিংও করা যায়।

## এটি কী করে {#what-it-does}

- যখন Ultimate Multisite-এ কোনো ডোমেইন ম্যাপ করা হয়, তখন ইন্টিগ্রেশনটি সেই ডোমেইনের জন্য একটি ভার্চুয়াল হোস্ট তৈরি করতে CyberPanel API কে কল করে।
- যখন কোনো ডোমেইন ম্যাপিং সরানো হয়, তখন ইন্টিগ্রেশনটি সংশ্লিষ্ট ভার্চুয়াল হোস্টটি মুছে ফেলার জন্য API কে কল করে।
- যখন অটো-SSL চালু থাকে, তখন ভার্চুয়াল হোস্ট তৈরি হওয়ার পরপরই ইন্টিগ্রেশনটি Let's Encrypt সার্টিফিকেট ইস্যু করার প্রক্রিয়া শুরু করে।
- এটি ঐচ্ছিকভাবে আপনার ডোমেইন ম্যাপিং সেটিংসে থাকা "Auto-create www subdomain" সেটিং অনুযায়ী `www.` অ্যালিয়াস যোগ বা সরিয়ে দেয়।
...
- CyberPanel-এ একটি বিদ্যমান ওয়েবসাইট থাকতে হবে যা আপনার WordPress নেটওয়ার্ক রুটকে হোস্ট করে। ইন্টিগ্রেশনটি এই সার্ভারে নতুন ভার্চুয়াল হোস্ট যুক্ত করে।
...
৩. মেয়াদ শেষ হওয়ার আগে CyberPanel স্বয়ংক্রিয়ভাবে সার্টিফিকেটটি রিনিউ করে দেয়।

> **গুরুত্বপূর্ণ:** Let's Encrypt-কে ডোমেইনটি যাচাই করার জন্য DNS অবশ্যই আপনার সার্ভারের IP অ্যাড্রেসে সম্পূর্ণরূপে প্রোপাগেট (propagated) হতে হবে। ম্যাপিং করার পরপরই SSL ইস্যু করা ব্যর্থ হলে, DNS প্রোপাগেশন হওয়ার জন্য অপেক্ষা করুন এবং CyberPanel ড্যাশবোর্ডের **SSL** > **Manage SSL** এর অধীনে ম্যানুয়ালি SSL পুনরায় ট্রিগার করুন।

### www সাবডোমেইন {#www-subdomain}

যদি আপনার ডোমেইন ম্যাপিং সেটিংসে **Auto-create www subdomain** সক্ষম থাকে, তবে ইন্টিগ্রেশনটি `www.<domain>` এর জন্য একটি ভার্চুয়াল হোস্ট অ্যালিয়াসও তৈরি করে এবং অটো-SSL চালু থাকলে, অ্যাপেক্স এবং www উভয় ভেরিয়েন্ট কভার করে একটি সার্টিফিকেট ইস্যু করে।

### ইমেল ফরোয়ার্ডার {#email-forwarders}

যখন [Ultimate Multisite: Emails](../../addons/ultimate-multisite-emails/) অ্যাডনটি সক্রিয় থাকে, তখন CyberPanel গ্রাহকদের জন্য ইমেল ফরোয়ার্ডারও সরবরাহ করতে পারে। ফরোয়ার্ডারগুলি একটি সম্পূর্ণ মেইলবক্স তৈরি না করেই একটি ডোমেইন অ্যাড্রেস থেকে অন্য ইনবক্সে বার্তাগুলি রুট করে, যা `info@customer-domain.test` বা `support@customer-domain.test`-এর মতো অ্যালিয়াসের জন্য উপযোগী।
...
২. ইমেল অ্যাডন সেটিংসে CyberPanel ইমেল প্রোভাইডার সক্ষম করুন।
৩. ফরোয়ার্ডার তৈরি করার আগে নিশ্চিত করুন যে গ্রাহকের ডোমেইনটি CyberPanel-এ বিদ্যমান।
৪. প্রোডাকশন প্ল্যানে ফিচারটি দেওয়ার আগে একটি টেস্ট ফরোয়ার্ডার তৈরি করুন এবং এর মাধ্যমে একটি বার্তা পাঠিয়ে পরীক্ষা করুন।
...
|---|---|---|---|
| `WU_CYBERPANEL_HOST` | Yes | — | আপনার CyberPanel ইনস্ট্যান্সের সম্পূর্ণ URL, পোর্ট সহ, যেমন: `https://cp.example.com:8090` |
| `WU_CYBERPANEL_USERNAME` | Yes | — | CyberPanel অ্যাডমিন ইউজারনেম |
| `WU_CYBERPANEL_PASSWORD` | Yes | — | CyberPanel অ্যাডমিন পাসওয়ার্ড |
| `WU_CYBERPANEL_PACKAGE` | Yes | `Default` | নতুন ভার্চুয়াল হোস্টগুলিতে বরাদ্দ করার জন্য CyberPanel হোস্টিং প্যাকেজ |
| `WU_CYBERPANEL_AUTO_SSL` | No | `true` | ডোমেইন তৈরি করার পরে একটি Let's Encrypt SSL সার্টিফিকেট ইস্যু করে |
| `WU_CYBERPANEL_PHP_VERSION` | No | `PHP 8.2` | নতুন ভার্চুয়াল হোস্টগুলির জন্য PHP সংস্করণ (CyberPanel-এ ইনস্টল করা সংস্করণের সাথে মিলতে হবে) |
| `WU_CYBERPANEL_EMAIL` | No | — | SSL সার্টিফিকেট রেজিস্ট্রেশনের জন্য যোগাযোগের ইমেল |
...
- CyberPanel ডিফল্টভাবে `8090` পোর্টে চলে। যদি আপনার সার্ভারে ফায়ারওয়াল থাকে, তবে নিশ্চিত করুন যে এই পোর্টটি ওয়ার্ডপ্রেস অ্যাপ্লিকেশন সার্ভার থেকে অ্যাক্সেসযোগ্য।
...
- আপনি যদি OpenLiteSpeed (OLS) ব্যবহার করেন, তবে ভার্চুয়াল হোস্ট পরিবর্তনের পরে স্বয়ংক্রিয়ভাবে একটি গ্রেসফুল রিস্টার্ট ট্রিগার হয়। কোনো ম্যানুয়াল হস্তক্ষেপের প্রয়োজন নেই।
...
- পরীক্ষা করুন যে আপনার CyberPanel SSL সার্টিফিকেটটি বৈধ; সেলফ-সাইনড সার্টিফিকেটগুলি TLS ভেরিফিকেশন ব্যর্থ করতে পারে। শুধুমাত্র বিশ্বস্ত ব্যক্তিগত নেটওয়ার্ক পরিবেশে `WU_CYBERPANEL_VERIFY_SSL` কে `false` সেট করুন।

### Authentication Errors {#authentication-errors}

- সরাসরি CyberPanel-এ লগ ইন করে নিশ্চিত করুন যে আপনার `WU_CYBERPANEL_USERNAME` এবং `WU_CYBERPANEL_PASSWORD` সঠিক।
- বারবার ব্যর্থ লগইন প্রচেষ্টার পরে CyberPanel অ্যাকাউন্ট লক করে দেয়। যদি লকআউট হয় তবে CyberPanel-এ **Security** > **Brute Force Monitor** চেক করুন।

### Domain Not Created {#domain-not-created}

- API ত্রুটির বার্তার জন্য Ultimate Multisite অ্যাক্টিভিটি লগ (**Ultimate Multisite** > **Activity Logs**) চেক করুন।
...
- নিশ্চিত করুন যে DNS সম্পূর্ণরূপে প্রোপাগেট হয়েছে: `dig +short your-domain.com` এ আপনার সার্ভারের IP আসা উচিত।
...
- একটি বিকল্প হিসেবে, আপনি CyberPanel থেকে ম্যানুয়ালি SSL ইস্যু করতে পারেন: **SSL** > **Manage SSL** > ডোমেইনটি নির্বাচন করুন > **Issue SSL**।
