...
// একটি ব্যবহার কাউন্টারে ৫ যোগ করুন।
...
class Membership_API_Limiter {
    ...
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        if ( (int) $membership->api_calls >= self::LIMIT ) {
            return false;  // Quota অতিক্রম করেছে — প্রত্যাখ্যান করুন।
        }

        // অ্যাটমিক ইনক্রিমেন্ট: কনকারেন্ট রিকোয়েস্টের অধীনে নিরাপদ।
...
```php
// UNSAFE — রিড এবং রাইটের মধ্যে রেস কন্ডিশন।
$membership = $memberships->get_item( $membership_id );
$new_count  = (int) $membership->api_calls + 1;
$memberships->update_item( $membership_id, [ 'api_calls' => $new_count ] );
```

দুটি একই সময়ে রিকোয়েস্ট একই মান পড়তে পারে এবং উভয়ই একই বৃদ্ধিপ্রাপ্ত মান লিখে দিতে পারে, ফলে একটি কাউন্ট হারিয়ে যায়। `increment_item()` গাণিতিক কাজটি ডেটাবেস ইঞ্জিনকে একটি একক `UPDATE ... SET column = column + ?` স্টেটমেন্টের মাধ্যমে অর্পণ করে, যা অপারেশনটিকে সহজাতভাবে অ্যাটমিক করে তোলে।
