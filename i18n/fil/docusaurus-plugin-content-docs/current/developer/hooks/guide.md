...
Nagbibigay ang Ultimate Multisite ng 200+ action hooks at 280+ filter hooks. Tinatalakay ng pahinang ito ang pinakamadalas gamiting hooks kasama ang mga praktikal na halimbawa. Para sa kumpletong awtomatikong nabuong reference, tingnan ang ibang mga pahina sa seksyong ito.

## Action Hooks {#action-hooks}

### Customer Lifecycle {#customer-lifecycle}

#### After Customer Creation {#after-customer-creation}
...
add_action('wu_customer_status_change', function($customer, $old_status, $new_status) {
    // Tumugon sa pagbabago ng status
}, 10, 3);
```

### Site Hooks {#site-hooks}

#### After Site Published {#after-site-published}
...
#### Before Template Application {#before-template-application}
...
add_action('wu_checkout_before_processing', function($cart) {
    // I-validate o baguhin ang cart bago iproseso
});
