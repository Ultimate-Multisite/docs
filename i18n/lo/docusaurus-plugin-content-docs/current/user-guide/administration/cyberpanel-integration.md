...
## มันทำงานอย่างไร {#setup-instructions}

### Domain Mapping (การจับคู่โดเมน) {#1-enable-the-cyberpanel-api}
...
### Auto-SSL (SSL อัตโนมัติ) {#2-add-constants-to-wp-configphp}
...
### www Subdomain (ซับโดเมน www) {#3-enable-the-integration}
...
### Email Forwarders (ตัวส่งอีเมล) {#4-verify-connectivity}
...
## ອ້າງອີງການຕັ້ງຄ່າ {#how-it-works}
...
## การแก้ไขปัญหา (Troubleshooting) {#auto-ssl}

### API Connection Refused (ปฏิเสธการเชื่อมต่อ API) {#www-subdomain}
...
### Authentication Errors (ข้อผิดพลาดในการยืนยันตัวตน) {#email-forwarders}
...
### Domain Not Created (โดเมนไม่ถูกสร้าง) {#configuration-reference}
...
### SSL Certificate Not Issued (ยังไม่มีการออกใบรับรอง SSL) {#important-notes}
