---
title: 'Dysgu 5: Cyfathrebu eich cynlluniau'
sidebar_position: 6
_i18n_hash: 8c58dfd0a186bd585f43342bc4ea4f43
---
# Lesiwn 5: Dyddiadau eich Planau {#lesson-5-designing-your-plans}

Mae'r cyflenau eich planau nid yn rhywbethau o ddefnyddio neu gael, ond mae'n cyfleiwyd o beth sy'n gweithredu'n wirol gyda'u cyddeeion. Yn y lesen hwnnw, byddwch yn ddefnyddio dyddiadau o produsg sy'n gyfathogh â'r angenwirol o gweithgareddau fitness mewn cyflenau gwahanol.

## Oedd Ni wedi Cynhaltio {#where-we-left-off}

Mae FitSite gyda thri modelau paratoi (Studio Essential, Gym Pro, Fitness Chain). Mae'n ddim ond y planau sy'n darnau'r cyfathrebu beth sydd yn cael ei gael a beth sydd yn cael eu pwyll.

## Mynddirolio mewn Cyfathrebu Cyddeeion {#thinking-in-customer-segments}

Ymddangos y chyfeiriad mwyaf o bobl yw ddefnyddio planau ar gyfer ffeithiau teknig (stor, bandwidth, amserau). Nid yn meddwl eich cyddeeion yn y termau hyn. Mae'r rheolwr gym yn meddwl am beth sydd ei angen i gweithredu ei gweithredol.

Ar gyfer studio fitness, mae tri cyfathrebu natural yn dod:

| Cyfathrebu | Pwy Hynnynt | Beth Mae angen nhw |
|---|---|---|
| **Gweithwyr unig / Studio bach** | 1-3 gweithwyr, lle unig, yn dechrau | Site proffesiynol, wybodaeth clasau, ffurf cyfathrebu |
| **Gymau wedi'u sefydlu** | 5-20 gweithwyr, lle unig, yn ddigwydd | Clydaidd allan ysbryd, blog, galler, domain custom |
| **Cyddeeion fitness** | Llawer o lleoliadau, brand wedi'i sefydlu | Clydaidd allan ysbryd, lleoliadau cyfathrebu, ddyddiadau gweithwyr |

Dylai eich planau i gyd-fynd â'r cyfathrebu hyn, nid gyda chymrydion ffeithig.

## Creu'r Planau FitSite {#creating-the-fitsite-plans}

Myndwch i **Ultimate Multisite > Products > Add Product** ar gyfer pob plan.

### Plan 1: FitSite Starter -- $49/mthydawd {#plan-1-fitsite-starter----49month}

**Cymryd:** Solo trainers a chwaraeau bach a styddiadau bach

**Tab Disgrif:**
- Enw: FitSite Starter
- Disgrif: "Gelltu allweddol yr hyn sy mae angen i chwaraeau personol neu styddiad bach i fod yn profyn ar-lein."

**Tab Cyffredinol:**
- Math y produs: Plan
- Rol cyddeewr: Administrator

**Tab Modelau Gwefan:**
- Mae modelau gwefan yn cael eu chwilio: Mae'n cael eu chwilio
- Modelau ar gael: Studio Essential yn unig

**Brosesau:**
- Sites: 1
- Cyflen gysgod: 1 GB
- Ddwyllt cyfathrebu: Nid cael ei chwarae (cynllun gwella)

**Tab Pluginau:**
- Plugin ffurf cyddeewr: Mae'n cael ei chwarae yn llwyr
- Plugin SEO: Mae'n cael ei chwarae yn llwyr
- Plugin cyflenion: Nid ar gael (cynllun gwella)

**Tab Theimau:**
- Y theim sydd wedi'i dewis: Mae'n cael ei chwarae yn llwyr
- Penodol theimau eraill: Yn gwblhau

### Plan 2: FitSite Growth -- $99/mth {#plan-2-fitsite-growth----99month}
**Cymryd:** Gymeddau unig wedi'u sefydlu

**Tab Disgrif:**
- Enw: FitSite Growth
- Disgrif: "Ar gyfer gymeddau wedi'u sefydlu sydd yn barod i drosu eu cyfathrebu ar-lein a chwarae newydd."

**Tab Modelau Gwefan:**
- Modelau ar gael: Studio Essential a Gym Pro

**Brosesau:**
- Sites: 1
- Cyflen gysgod: 5 GB
- Ddwyllt cyfathrebu: Mae'n cael ei chwarae

**Tab Pluginau:**
- Clyda'r Gymuned yn y Starter, plus:
- Plugin cyflenion: Mae'n cael ei chwarae yn llwyr
- Plugin gallu: Mae'n cael ei chwarae yn llwyr
- Ffurf blog: Ar gael

**Tab Uwch a Ddrefnu:**
- Grŵp planau: FitSite Plans
- Bryd o barchnad: 2

### Plan 3: FitSite Pro -- $199/mth {#plan-3-fitsite-pro----199month}
**Cymryd:** Chwaraeau fitness gyda chyfnodau aml

**Tab Disgrif:**
- Enw: FitSite Pro
- Disgrif: "Y ffurf cyfan ar gyfer brandau chwarae fitness gyda chyfnodau aml."

**Tab Model Si Llyfrgellau (Site Templates)**:
- Templa ar gael: Mae'r tri templau i gyd

**Brosesaueddau (Limitations)**:
- Sites: 5 (unigol ar gyfer lleoliad unig)
- Cyflenau diskw: 20 GB
- Domain custom: Mae'n cael ei chwarae

**Tab Pluginau**:
- Clywedog pob hyn yn y Gwaith (Growth), plus:
- Pob cyflenau priemu: Force Activate

**Tab Uwch a Isel (Up & Downgrades)**:
- Grŵp cynlluniau: FitSite Plans
- Cyfeiriad o dreddau: 3

## Rheoli'r Grŵp Cynlluniau (Setting Up the Plan Group) {#setting-up-the-plan-group}

Mae'r grŵp cynlluniau yn sicr y bydd ei fod y golybusau yn gallu gwneud cyfathri neu isel yn y tebyg FitSite. Ar y **Tab Uwch a Isel** o bob cynllun:

1. Rheoliwch y **Grŵp Cynlluniau** i "FitSite Plans" ar gyfer pob un o'r tri cynllun
2. Rheoliwch y **Cyfeiriad o Dreddau** i 1 (Starter), 2 (Growth), 3 (Pro)

Mae hyn yn creu llwybr cyfathri'n llwyr: Starter → Growth → Pro.

## Ymyddu Cyfeiriadau O Dreddau (Adding Order Bumps) {#adding-order-bumps}

Cyfeiriadau o dreddau yw produsau ychwanegol a cael eu cynnig yn ystod y cyflenau. Ar FitSite, meddwch am:

- **Extra Storage Pack** ($19/mêr) -- diskw gyfaddol 5 GB
- **Priority Support** ($29/mêr) -- amser ymateb gwellter
- **Additional Site** ($39/mêr) -- ar gyfer golybusau sydd angen nhw i fod yn fwy o sites na'i gallu eu cynlluniau

Creu'r hyn fel produsau mewn tyg **Package** yn Ultimate Multisite a chysylltu nhw â'r cynlluniau cyflwynig.

## Pam Mae'r Estrwyll Hwn yn Gweithredu? (Why This Structure Works) {#why-this-structure-works}

- **Starter** yn rhwystau i gael y bythau -- pris isel, cynnig simple, mae'n rhoi'r traineron ar-lein yn gyflym
- **Gronau** yn ychwanegu'r ffeithiau sydd yn eu gwneud gymnalion -- cyflwyniadau, gallerïau, domennau cyfathrebu
- **Pro** yn gwasanaethu'r segment o gael gwerth uchel sydd angen cefnogaeth am ymlaen lleoliadau aml
- **Order bumps** yn sylw i'r cydweithwyr gwneud ei chwarae heb ddatblygu'r planau cyflym
- **Clir y llwybr o ddatblygu** mae'n golygu bod y cydweithwyr yn ddigwydd ar gyfer chi yn hytrudol heb gadael

## Y FitSite Network Erioedd {#the-fitsite-network-so-far}

```
FitSite Network
├── WordPress Multisite (mode subdomain)
├── Ultimate Multisite (configwyd)
├── Hosting gyda wildcard SSL + mapio domennau
├── Site Templates (Studio Essential, Gym Pro, Fitness Chain)
├── Products
│   ├── FitSite Starter ($49/mo) → Studio Essential template
│   ├── FitSite Growth ($99/mo) → Studio Essential + Gym Pro templates
│   ├── FitSite Pro ($199/mo) → Penod all templata
│   └── Order Bumps (Storage extra, Cefnogaeth priwart, Site ychwanegol)
└── Barod ar gyfer cyflwyniadau (dysg nesaf)
```

## Beth Rydyn Ni wedi'i Gwnaeth Mae'r Dysgu Hwnnw {#what-we-built-this-lesson}

- **Tri o ddeallau plan** a ddifynwyd i segmentau busnes ffitr yn reol
- **Cyflwyniad ffeithiau** gan ddefnyddio'r plugin a rheoliadau templata Ultimate Multisite
- **Grup plan** gyda llwybr o ddatblygu clir
- **Producaid order bump** ar gyfer cyflym ystodol
- **Struktur prisau** yn seiliedig ar gwerth y cydweithwyr, nid ar adran technolegol

**Manor nesaf:** [Lles 6: Y ffurf cyfrifolion](lesson-6-checkout) -- byddwn yn llwybrffwr y ffurf cyfrifolion i gael y ddefnyddwyr studio fithrin yn cydweithwyr talu.
