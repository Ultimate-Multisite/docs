...
## Beth yw'r Cyfathrebu? {#what-it-does}
...
## Prifysgolion Cynllunio (Prerequisites) {#prerequisites}
...
## Mae'r Prifysgolion yn Cyffredinol (Requirements) {#requirements}
...
## Cynlluniau Rheoliadau (Setup Instructions) {#setup-instructions}

### 1. Gwellydd y API CyberPanel {#1-enable-the-cyberpanel-api}
...
### 2. Refynion Cynlluniau (Constants) i wp-config.php {#2-add-constants-to-wp-configphp}
...
### 3. Cynllunio'r Cyfathrebu (Enable the Integration) {#3-enable-the-integration}
...
### 4. Gyflwyno'r Cyfyngiad (Verify Connectivity) {#4-verify-connectivity}
...
### Domain Mapping (Mapiad Ddwyllt) {#domain-mapping}
...
### Email Forwarders (Cyflawni E-bost) {#email-forwarders}
...
## Cyfeiriad Cysylltiadau {#configuration-reference}
...
## Cynllunio Problemau (Troubleshooting) {#troubleshooting}

### Mae Cyfathrebu API wedi'i Gofio (API Connection Refused) {#api-connection-refused}
...
### Errau Cyfathrebu (Authentication Errors) {#authentication-errors}
...
### Nid cael Domain wedi'i Creu (Domain Not Created) {#domain-not-created}
...
### Nid cael Cert SSL wedi'i Cyflwyno (SSL Certificate Not Issued) {#ssl-certificate-not-issued}
