---
id: wu_password_require_special
title: Isihlungi - wu_password_require_special
sidebar_label: wu_password_require_special
_i18n_hash: 87f845abf068a92161fb7db01f88ee99
---
# Isihlungi: wu_password_require_special {#filter-wupasswordrequirespecial}

Hlunga ukuthi kudingeke yini izinhlamvu ezikhethekile kumaphasiwedi.

## Amapharamitha {#parameters}

| Igama | Uhlobo | Incazelo |
|------|------|-------------|
| $require | `bool` | Ukuthi kudingeke yini izinhlamvu ezikhethekile. Okuzenzakalelayo kungu-true lapho imithetho iphoqelelwa. |
| $defender_active | `bool` | Ukuthi i-Defender Pro Strong Password iyasebenza yini. |

### Kusukela {#since}

- 2.4.0
### Umthombo {#source}

Kuchazwe ku-[`inc/class-scripts.php`](https://github.com/Ultimate-Multisite/ultimate-multisite/blob/main/inc/class-scripts.php#L583) emugqeni 583
