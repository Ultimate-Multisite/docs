---
id: wu_site_exporter_plugin_exclusion_list
title: Isihlungi - wu_site_exporter_plugin_exclusion_list
sidebar_label: wu_site_exporter_plugin_exclusion_list
_i18n_hash: 547f26a2bdb1117cc1616485ef4525e6
---
# Isihlungi: wu_site_exporter_plugin_exclusion_list

Ivumela abathuthukisi ukuphatha uhlu lwe-plugin olungase lukhishwe ku-zip ekhiqiziwe.

## Amapharamitha {#parameters}

| Igama | Uhlobo | Incazelo |
|------|------|-------------|
| $plugin_list | `array` | Ama-plugin azokhishwa. |

### Kusukela {#since}

- 2.0.0
### Umthombo {#source}

Kuchazwe ku-[`inc/class-wp-ultimo-site-exporter.php`](https://github.com/Ultimate-Multisite/ultimate-multisite-site-exporter/blob/main/inc/class-wp-ultimo-site-exporter.php#L210) emugqeni 210


## Okubuyiswayo {#returns}
Uhlu lwe-plugin.
