...
**Tasarım Yönü** adımı, Tema Oluşturucu'nun (Theme Builder) tüm temayı oluşturmasından önce temanızın görsel tasarımını gözden geçirmenize, iyileştirmenize ve son haline getirmenize olanak tanır.

## Genel Bakış {#overview}

Keşif Görüşmesi'ni (Discovery Interview) tamamladıktan sonra, Tema Oluşturucu tasarım yönünüzü **masaüstü ve mobil önizleme gösterimi** ile sunar. Bu, temaya bağlı kalmadan, tasarımınızın farklı cihazlarda tam olarak nasıl görüneceğini görmenizi sağlar.
...
- **Ana renk (Primary color)** — düğmeler, bağlantılar ve vurgular için kullanılan ana marka rengi
- **İkincil renk (Secondary color)** — çeşitlilik için tamamlayıcı renk
- **Vurgu rengi (Accent color)** — önemli öğeler için dikkat çekici renk
- **Nötr renkler (Neutral colors)** — arka planlar ve metinler için gri ve beyazlar
- **Metin rengi (Text color)** — okunabilirlik için ana metin rengi

### 2. Tipografi {#2-typography}

Şunlar için yazı tipi seçimleri:

- **Başlık yazı tipi (Heading font)** — sayfa başlıkları ve bölüm başlıkları için kullanılır
- **Gövde yazı tipi (Body font)** — paragraf metinleri ve içerik için kullanılır
- **Yazı boyutları (Font sizes)** — farklı ekran boyutları için duyarlı boyutlandırma
- **Satır aralığı (Line height)** — okunabilirlik için satırlar arasındaki boşluk
- **Yazı ağırlıkları (Font weights)** — kalın, normal ve açık varyasyonlar

### 3. Düzen ve Boşluk (Layout and Spacing) {#3-layout-and-spacing}

- **Kapsayıcı genişliği (Container width)** — içerik için maksimum genişlik
- **İç dolgu ve dış boşluklar (Padding and margins)** — öğeler etrafındaki boşluk
- **Izgara sistemi (Grid system)** — duyarlı tasarım için sütun düzeni
- **Bileşen boşluğu (Component spacing)** — kullanıcı arayüzü öğeleri arasındaki mesafe

### 4. Görsel Öğeler {#4-visual-elements}

- **Düğme stilleri (Button styles)** — ana, ikincil ve üçüncül düğme tasarımları
- **Kart tasarımları (Card designs)** — içerik blokları için düzenler
- **İkonlar (Icons)** — ikon stili ve boyutlandırma
- **Kenarlıklar ve gölgeler (Borders and shadows)** — zarif görsel derinlik
- **Üzerine gelme efektleri (Hover effects)** — etkileşimli durum stillendirmesi

## Önizleme Gösterimi (Preview Rendering) {#preview-rendering}
...
### Duyarlı Ekran Boyutları (Responsive Breakpoints) {#responsive-breakpoints}

Tema Oluşturucu, önizlemeleri bu noktalarda oluşturur:
...
## Tasarımınızı İyileştirme {#refining-your-design}

### Ayarlamalar Yapmak {#making-adjustments}

Tasarım yönünüzü aşağıdaki yollarla iyileştirebilirsiniz:

1. **Renkleri ayarlama** — paletinizdeki herhangi bir rengi değiştirebilirsiniz
...
- Masaüstü önizlemesi anında güncellenir
- Mobil önizlemesi anında güncellenir
- Tüm duyarlı ekran boyutları güncellenir
...
4. **Düzeni onaylayın** — boşlukları ve hizalamayı kontrol edin
...
1. Tema Oluşturucu, eksiksiz temanızı oluşturur
2. Tema, WordPress sitenize kurulur
3. WordPress özelleştirici'yi kullanarak daha fazla özelleştirme yapabilirsiniz
...
- **İçeriği planlayın** — düzenin gerçek içeriğinizle çalıştığından emin olun
...
- Farklı ekran boyutlarında yazı boyutunu kontrol edin
