---
title: Özel Geliştirme Yardımı
sidebar_position: 10
_i18n_hash: 92675ef1c1dfa981cb05f3e328042cbc
---
# Codeable'dan Özel Geliştirme Desteği Nasıl Alınır?

Bazen ağ akışlarımızı tam istediğimiz gibi yapılandırmak için harika özelleştirmelere ihtiyaç duyarız. Eğer aradığınız buysa, Ultimate Multisite konusunda deneyimli geliştiricileri bulabileceğiniz [Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE) platformunu öneriyoruz.

[Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE), müşterileri her ölçekteki projede çalışabilecek nitelikli WordPress freelance geliştiricilerle buluşturan bir platformdur. Titiz bir başvuru süreciyle, aklınızdaki görev için özel olarak uzmanlaşmış kişilerle eşleşmenizi sağlarlar.

### Codeable'da nasıl proje oluşturulur? {#how-to-create-a-project-in-codeable}

Öncelikle [Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE) sitesine gidin ve **Start a Project** butonuna tıklayın.

<!-- Screenshot unavailable: Codeable homepage with Start a Project button -->

Şimdi proje göndermek için soldaki **Start a Project** seçeneğine tıklayın.

<!-- Screenshot unavailable: Codeable project submission page with Start a Project option -->

"I need help with" bölümünde **Customizing** seçeneğini seçin:

<!-- Screenshot unavailable: Codeable project form with Customizing selected -->

"My" bölümünde **Plugin** seçeneğini seçin:

<!-- Screenshot unavailable: Codeable project form with Plugin selected -->

Ardından **Continue** butonuna tıklayın.

<!-- Screenshot unavailable: Codeable project form Continue button -->

Şimdi projenizi ve ihtiyaçlarınızı mümkün olan en iyi şekilde açıklamanın zamanı. Olabildiğince fazla bilgi vermeye çalışın!

<!-- Screenshot unavailable: Codeable project description form -->

Fiyatlandırmaya yardımcı olması için aşağıdaki soruları yanıtlayın.

<!-- Screenshot unavailable: Codeable project pricing questions -->

Bundan sonra, henüz bir [Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE) hesabınız yoksa bir hesap oluşturun ve **Publish My Project** butonuna tıklayın.

<!-- Screenshot unavailable: Codeable account creation and Publish My Project button -->

[Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE) projenizi yayınladıktan sonra, platform sizi projenizin gerektirdiği becerilere sahip geliştiricilerle eşleştirecektir.

Bu süreç, WordPress ve Ultimate Multisite deneyimine sahip, işiniz için doğru kişiyi bulmanıza yardımcı olacaktır!

Codeable'ın minimum ücreti saatlik 70–120 USD arasındadır. Tahminler, projenin kapsamına, karmaşıklığına ve aciliyetine göre değişir.

Teklifinizi aldıktan sonra, ihtiyaçlarınıza en uygun geliştiriciyi işe alabilir ve projenize birlikte başlayabilirsiniz!
