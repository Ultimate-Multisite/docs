...
---
# CyberPanel İntegrasiyası {#cyberpanel-integration}
...
## Tələb Olunan Kodlar {#requirements}
...
### 1. CyberPanel API-ni Aktivləşdirin {#1-enable-the-cyberpanel-api}
...
### 2. wp-config.php-yə Sabitlər Əlavə Edin {#2-add-constants-to-wp-configphp}
...
### 3. İntegrasiyanı Aktivləşdirin {#3-enable-the-integration}
...
### 4. Bağlantını Yoxlayın {#4-verify-connectivity}
...
### Domen Xəritələmə (Domain Mapping) {#domain-mapping}
...
### Avtomatik SSL (Auto-SSL) {#auto-ssl}
...
### E-poçt Yönləndiriciləri (Email Forwarders) {#email-forwarders}
...
## Konfiqurasiya Referansı {#configuration-reference}
...
### API Bağlantısı Rədd Edildi (API Connection Refused) {#api-connection-refused}
...
### Autentifikasiya Xətaları (Authentication Errors) {#authentication-errors}
...
### Domen Yaratılmadı (Domain Not Created) {#domain-not-created}
...
### SSL Sertifikatı Verilmədi (SSL Certificate Not Issued) {#ssl-certificate-not-issued}
