...
### الوصول إلى الـ API {#api-access}
خاصك تكون عندك صلاحيات المسؤول (administrator access) في Enhance Control Panel باش تصنع رموز الـ API (API tokens).

## الحصول على بيانات الـ API ديالك {#getting-your-api-credentials}

### 1. إنشاء رمز API (Create an API Token) {#1-create-an-api-token}
...
### ۴. دریافت API URL (آدرس API) {#4-get-your-api-url}
...
### راه‌اندازی از طریق راهنمای یکپارچه‌سازی (Setup via Integration Wizard) {#additional-wordpress-configuration}
...
## تنظیمات اضافی وردپرس (Additional WordPress Configuration) {#htaccess-configuration}
...
### تنظیمات فایل .htaccess {#cookie-constants}
...
### ثابت‌های کوکی (Cookie Constants) {#how-it-works}
...
## نحوه کارکرد (How It Works) {#when-a-domain-is-mapped}

### وقتی یک دامنه نگاشت می‌شود (When a Domain is Mapped) {#when-a-domain-is-removed}
...
### زمانی که یک دامنه حذف می‌شود {#dns-and-ssl-checking}
...
### بررسی DNS و SSL {#verifying-setup}
...
## تأیید تنظیمات {#test-the-connection}

### تست اتصال {#after-mapping-a-domain}
...
### بعد از نگاشت یک دامنه {#troubleshooting}
...
## Maganar da Matala (Troubleshooting) {#api-connection-issues}

### Matsalolin Ha Sambungan API (API Connection Issues) {#domain-not-added}
...
### Ba A ƙara Domain (Domain Not Added) {#ssl-certificate-issues}
...
### Matsalolin Sertifika na SSL (SSL Certificate Issues) {#dns-check-interval}
...
### Takaratayar Binciken DNS (DNS Check Interval) {#authentication-errors}
...
### Kuskuren Samun Bayan Aiki (Authentication Errors) {#log-analysis}
...
### Bincike na Logs (Log Analysis) {#api-reference}
...
## Bayanai na API (API Reference) {#authentication}

### Samun Bayan Aiki (Authentication) {#common-endpoints-used}
Dukiyar buƙatun API suna amfani da Bearer token authentication:
```
Authorization: Bearer YOUR_TOKEN_HERE
```

### Wurin Amfani da Kowa (Common Endpoints Used) {#full-api-documentation}
...
### Bayanai na API na Tsafi (Full API Documentation) {#best-practices}
Bayanai na API na cikakken tsari: [https://apidocs.enhance.com](https://apidocs.enhance.com)

## Amfani da Shawara Na Musamman (Best Practices) {#security}
...
### Inganci (Performance) {#monitoring}
...
### Bincike (Monitoring) {#additional-resources}
...
## Kayayyakin Tambayar (Additional Resources) {#support}
