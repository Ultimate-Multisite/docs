...
### Hoton Wato (Product Image) {#product-image}
...
### Zaɓuwar Wato (Product Options) {#product-options}
...
#### ارتقاء و پایین آوردن (Up & Downgrades) {#up--downgrades}
...
#### تغییرات قیمت (Price Variations) {#price-variations}
...
#### Site Templates (نموونەی شوێنەکان) {#site-templates}
...
#### فضای دیسک (Disk Space) {#disk-space}
...
#### دامنه سفارشی (Custom Domain) {#custom-domain}
...
#### Canjin Ilimin (Reset Limitations) {#reset-limitations}
...
## Gyara, Kuno ko Cire Product {#edit-duplicate-or-delete-product}
