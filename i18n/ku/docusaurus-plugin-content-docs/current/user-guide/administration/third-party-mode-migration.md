...
## کیان affected (کێ دەگۆڕێت؟) {#who-is-affected}

### جێبەجێکردنی نوێکان (WordPress 7.0+) {#new-installations-wordpress-70}
...
### وەرگرتەی WordPress پێش 7.0 {#wordpress-versions-before-70}
...
## تێگەیشتن لە Mode'ەکاندا (Understanding the Modes) {#understanding-the-modes}

### Auto Mode (پیش‌دۆزەوەی نوێ) {#auto-mode-new-default}
...
## مزایای حالت Auto Mode (حالت خودکار) {#benefits-of-auto-mode}
...
### قابلیت‌ها در حالت Auto بارگذاری نمی‌شوند {#abilities-arent-loading-in-auto-mode}
...
### توانیاساتی تایبەکانی من نیشان ناکرێن {#my-custom-abilities-arent-showing}
...
### من ئاگاداری "ability"ی نیشان دەبینم {#im-getting-unclassified-ability-notices}
...
## ڕەتکردنەوەی پێشتر (Backward Compatibility) {#backward-compatibility}

### ڕێکخستنەکان کە پێشتر هەبووین (Existing Configurations) {#existing-configurations}
...
### کاتی کۆنکردنەوەی کارکردن (Deprecation Timeline) {#deprecation-timeline}
...
### بۆ دامەزراندنی نوێکان (For New Installations) {#for-new-installations}
...
### بۆ دامەزراندنی ئێستا (For Existing Installations) {#for-existing-installations}
...
### بۆ توانیاساتی تایبەت (For Custom Abilities) {#for-custom-abilities}
...
## هەنگاوەکانی داهات (Next Steps) {#next-steps}
