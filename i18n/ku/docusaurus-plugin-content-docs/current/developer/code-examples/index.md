---
title: Mînakên Kodê yên Pêşketî
sidebar_position: 1
...
## Motora Nirxdanê ya Dînamîk {#dynamic-pricing-engine}

Motorek nirxdanê ya li ser bingehê rêzikan ku daxistinên qebareyê, dilsozî û demsalî sepand dike:
...
## Amadekirina Malperê ya Pêşketî {#advanced-site-provisioning}

Li gor taybetmendiyên planê, malperên nû bi pêvekan, SSL, CDN, kopiyên parastinê û çavdêrîyê bixweber mîheng bike:
...
## Pergala Sînorkirinên Taybet {#custom-limitations-system}

Sînorên çavkaniyan bi hişyariyên bikaranînê bişopîne û bisepîne:
...
## Jimêrê atomîk ê BerlinDB bi `increment_item()` {#berlindb-atomic-counter-with-incrementitem}

Ultimate Multisite v2.6.1 rêbaza `increment_item()` li pola BerlinDB `Query` zêde kir. Wê bikar bîne da ku zêdekirinên ewle û atomîk li ser stûnên hejmarî bêyî pevçûnên xwendin-guhertin-nivîsînê pêk bînî — ji bo jimêran, kotayên bikaranînê, û kontrolên sînorkirina rêjeyê yên di bin daxwazên hevdem de dixebitin bi kêr tê.

### Îmzeya rêbazê {#method-signature}
...
### Şopandina bikaranîna API li gorî endamtiyê {#tracking-api-usage-per-membership}

Şêwazekî pratîk ji bo sepandina sînorên rêjeya API li gorî endamtiyê:
...
### Çima `increment_item()` li şûna `update_item()` {#why-incrementitem-instead-of-updateitem}

Nêzîkatiyekî sade ya xwendin-guhertin-nivîsînê di bin daxwazên hevdem de ne ewle ye:
...
Du daxwazên hevdem dikarin heman nirxê bixwînin û herdu encama heman zêdekirî vegerînin, bi vî awayî yek jimartin winda dibe. `increment_item()` hejmartinê bi yek daxuyaniya `UPDATE ... SET column = column + ?` radestî motora danegirê dike, lewma ev kiryar bi xwe atomîk e.
