---
id: wu_magic_links_enabled
title: سۈزگۈچ - wu_magic_links_enabled
sidebar_label: wu_magic_links_enabled
_i18n_hash: 98f65dc18013396be2f197fbbd49e773
---
# سۈزگۈچ: wu_magic_links_enabled {#filter-wumagiclinksenabled}

سېھىرلىك ئۇلانمىلارنىڭ قوزغىتىلغان-قوزغىتىلمىغانلىقىنى سۈزۈش.

## پارامېتىرلار {#parameters}

| نامى | تۈرى | چۈشەندۈرۈش |
|------|------|-------------|
| $enabled | `bool` | سېھىرلىك ئۇلانمىلارنىڭ قوزغىتىلغان-قوزغىتىلمىغانلىقى. |

### باشلاپ {#since}

- 2.0.0
### مەنبە {#source}

[`inc/sso/class-magic-link.php`](https://github.com/Ultimate-Multisite/ultimate-multisite/blob/main/inc/sso/class-magic-link.php#L377) دا 377-قۇردا ئېنىقلانغان
