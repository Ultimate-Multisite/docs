...
### 1. CyberPanel API'yi Etkinleştirin {#1-enable-the-cyberpanel-api}
...
## کار څنګه کار کوي؟ {#how-it-works}

### Domain Mapping (دامنی نقشہ سازی) {#domain-mapping}
...
## Конфигурацияге карау өчен {#configuration-reference}
...
## Зур маанилИ мәгълүматлар {#important-notes}
...
## Problemləri Həlli {#troubleshooting}

### API Bağlantısı Rədd Edilir (API Connection Refused) {#api-connection-refused}
...
### Autentifikasi Səhvləri (Authentication Errors) {#authentication-errors}
...
### Domen Yaradılmayıb (Domain Not Created) {#domain-not-created}
...
### SSL Sertifikatı Verilməyib (SSL Certificate Not Issued) {#ssl-certificate-not-issued}
