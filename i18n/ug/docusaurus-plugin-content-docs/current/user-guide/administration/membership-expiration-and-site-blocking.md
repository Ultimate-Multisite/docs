...
## Üyelik Süreleri (Membership Expiration) {#membership-status-lifecycle}
...
## Site Bloklama (Site Blocking) {#how-memberships-transition-to-expired}
...
## Teknik Detaylar (Technical Details) {#auto-renewing-vs-non-auto-renewing-memberships}
...
## Üyeliq Durum Döngüsü {#how-trials-end}
...
## Installation (O'rnatish) {#what-visitors-see-when-a-site-is-blocked}
...
## Core Concepts (Asosiy Tushunchalar) {#what-gets-blocked-and-when}
...
## Using Plugins (Plaginlardan Foydalanish) {#troubleshooting-sites-remaining-accessible-after-expiration}
...
## Multisite Setup (Multisite Sozlamalari) {#1-verify-the-block-frontend-access-setting-is-enabled}
...
## Custom Development (Maxsus Ishlash) {#2-check-the-frontend-block-grace-period}
...
## Security Best Practices (Xavfsizlik Eng Amaliyotlari) {#3-confirm-the-membership-status-has-actually-changed}
...
## Conclusion (Xulosa) {#4-verify-action-scheduler-is-running}
...
### A'zoliklar Qanday Tugadi (How Memberships Transition to Expired) {#5-check-for-gateway-webhook-issues-auto-renewing-memberships}
...
#### Otomatik Yenilenen ve Otomatik Yenilenmeyen Üyelikler Arasındaki Farklar {#6-check-the-expiration-grace-period-cron-level}
...
### قيد (Trials) کی آخری تاریخ کیا ہے؟ {#7-manually-expire-a-membership}
...
### Ayarları Yapılandırma {#developer-reference}
