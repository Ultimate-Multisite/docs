...
## ការយល់ដឹងអំពីរបៀបផ្សេងៗគ្នា (Understanding the Modes)
...
## Tsoij tias Koj yuav tuaj yeem (Checking Your Current Mode)
...
## Tsim khoom koj yuav tuaj yeem (Changing Your Mode)
...
## Auto Mode-nii Benefits (자동 모드의 장점들)

### Automatic Discovery (자동으로 능력들을 찾아줌)
...
## Migration Path (이동 경로)

### If You're on WordPress 6.x (WordPress 6.x를 사용 중이라면)
...
### If You're on WordPress 7.0+ (WordPress 7.0+를 사용 중이라면)
...
### Abilities aren't loading in auto mode (자동 모드에서 능력들이 로드되지 않을 때)
...
### I want to keep legacy mode (레거시 모드를 유지하고 싶을 때)
...
### Kuv ra "unclassified ability" notices
...
## Backward Compatibility (Khả năng tương thích ngược)
...
### Thời gian ngừng hỗ trợ (Deprecation Timeline)
...
## ຫົວຂໍ້ທີ່ກ່ຽວຂ້ອງ (Related Topics)
