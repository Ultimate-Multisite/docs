...
## இயக்குх команд (Commands to run)
...
## පොදු අසාර්ථකතා (Common failures)

### Database grants do not match the host (Database හි දීමනා සේවා ස්ථානය නොගැලපීම)
...
### Bedrock or local installs cannot connect (Bedrock හෝ local socket ස්ථාපන සම්බන්ධ විය නොහැකි වීම)
...
### Async push queue does not drain (Async push කූඩය හිස් නොවීම)
...
### Tenant user count is wrong (테넌트 පරිශීලක ගණන වැරදි වීම)
...
### SSO visit is rejected (SSO පිවිසුම ප්‍රතික්ෂේප වීම)
