...
## ຂັ້ນຕອນທີ 2: ການຕັ້ງຄ່າ Network (Network Configuration)
...
### Network Admin Email (Email Ua Thawm Network)
...
## Step 3: Installation (Qhov Chaw Ua Install)
...
## 手動設定備用方案 (Manual Setup Fallback)
...
### wizard 話 wp-config.php 無法寫入 (The wizard says wp-config.php is not writable)
...
### 設定完之後網站無法存取（subdomains）(Sites are not accessible after setup (subdomains))
...
### 設定完之後出現認證問題（Authentication issues after setup）
...
### 安裝過程中某一步失敗咗 (A step failed during installation)
