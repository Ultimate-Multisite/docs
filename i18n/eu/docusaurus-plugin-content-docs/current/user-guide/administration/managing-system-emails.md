---
title: Sistema-emailak eskaintzea
sidebar_position: 13
...
Hau partirdu gabe, **System Emails** (Sistema-emailak) lapurialdiari aurkitu nahi duzu email template guztiek ikusteko.

![System Emails list](/img/admin/system-emails-list.png)

Euskal sistema-email editori erabiliz email template batzuk edite dezakezu:
...
Berri email templateak importatzeko:

1. **System Emails** (Sistema-emailak) lapurialdiari jarraitu
2. Lapurialdiaren üstaldean dagoen **Reset & Import** (Eskatu eta importatu) botoia klik egin
...
1. **Delete and re-import** (Bideratu eta importatu berri): Sistema-emaila bideratu eta berri importatu (horrek aurretik iragilduak ez daudien metrikak borratzen du).
2. **Reset via the tool** (Eskatu araz): **Reset & Import** (Eskatu eta importatu) arazak erabilu, **Reset** (Eskatu) opsioa onartu eta eskatzea nahi duzu emailak aukeratu

Reset opsioak preferatu da, karena kontuentuak default-eko itxarri eta beste metadata hauek ontzen ditu.
