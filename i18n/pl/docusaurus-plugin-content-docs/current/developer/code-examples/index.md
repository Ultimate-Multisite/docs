---
title: Zaawansowane przykłady kodu
sidebar_position: 1
...
## Silnik Dynamicznego Ustalania Cen {#dynamic-pricing-engine}

Silnik cenowy oparty na regułach, który stosuje zniżki za ilość, lojalność i sezonowe:
...
## Zaawansowane Uruchamianie Strony (Site Provisioning) {#advanced-site-provisioning}

Automatyczne konfigurowanie nowych stron za pomocą pluginów, SSL, CDN, backupów i monitoringu, bazując na funkcjach planu:
...
class Advanced_Site_Provisioner {
    ...
    public function provision_site($site, $membership) {
        ...
        // Instalacja pluginów na podstawie planu
...
## System Ograniczeń (Custom Limitations System) {#custom-limitations-system}

Śledź i egzekwuj limity zasobów z ostrzeżeniami o wykorzystaniu:
...
## Atomic Counter BerlinDB z `increment_item()` {#berlindb-atomic-counter-with-incrementitem}

Ultimate Multisite v2.6.1 dodał metodę `increment_item()` do klasy `Query` BerlinDB. Użyj jej do bezpiecznego, atomowego zwiększania wartości w kolumnach numerycznych, bez ryzyka wyścigu (race condition) typu odczyt-modyfikacja-zapis — jest to przydatne do liczników, kwot użycia i sprawdzania limitów żądań, które działają w przypadku równoczesnych żądań.

### Sygnatura metody {#method-signature}

```php
/**
 * Atomowo zwiększa wartość numerycznej kolumny dla określonego elementu.
 *
 * @param int    $item_id   Klucz główny wiersza do aktualizacji.
 * @param string $column    Nazwa kolumny do zwiększenia (musi być numeryczna).
 * @param int    $amount    Wartość do dodania. Użyj wartości ujemnej, aby zmniejszyć.
 *                          Domyślnie 1.
 * @return bool True w przypadku sukcesu, false w przypadku błędu lub jeśli kolumna jest nieprawidłowa.
 */
...
### Śledzenie użycia API dla każdego członkostwa {#tracking-api-usage-per-membership}

Praktyczny wzorzec egzekwowania limitów żądań API na poziomie członkostwa:

```php
class Membership_API_Limiter {

    /** Maksymalna liczba wywołań API dozwolona w cyklu rozliczeniowym. */
    const LIMIT = 500;
    ...
    /**
     * Odrzuca żądanie, jeśli członkostwo przekroczy limit;
     * w przeciwnym razie atomowo zwiększa licznik wywołania.
     *
     * @param bool   $enabled
     * @param object $context  Obiekt z metodą get_membership_id().
     * @return bool
     */
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        if ( (int) $membership->api_calls >= self::LIMIT ) {
            return false;  // Przekroczono kwotę — odrzuć.
        }

        // Atomowe zwiększenie: bezpieczne w przypadku równoczesnych żądań.
...
### Dlaczego `increment_item()` zamiast `update_item()` {#why-incrementitem-instead-of-updateitem}

Proste podejście odczyt-modyfikacja-zapis jest niebezpieczne w przypadku równoczesnych żądań:

```php
// NIEBEZPIECZNE — warunek wyścigu między odczytem a zapisem.
$membership = $memberships->get_item( $membership_id );
$new_count  = (int) $membership->api_calls + 1;
$memberships->update_item( $membership_id, [ 'api_calls' => $new_count ] );
```

Dwa równoczesne żądania mogą odczytać tę samą wartość i obie zapisać ten sam zwiększony wynik, co spowoduje utratę jednego licznika. `increment_item()` deleguje arytmetykę do silnika bazy danych za pomocą pojedynczego zapytania `UPDATE ... SET column = column + ?`, co sprawia, że operacja jest z natury atomowa.
