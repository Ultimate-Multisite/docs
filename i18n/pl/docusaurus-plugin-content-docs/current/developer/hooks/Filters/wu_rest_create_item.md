---
id: wu_rest_create_item
title: Filtr - wu_rest_create_item
sidebar_label: wu_rest_create_item
_i18n_hash: b35924a8414367a9d0c2f3c21f1a1b91
---
# Filter: wu_rest_create_item {#filter-wurestcreateitem}

Filtruje, czy można kontynuować żądanie, czy nie.

## Parametry {#parameters}

| Name | Type | Description |
|------|------|-------------|
| $allowed | `bool` | Wartość zwracana na początku. |
| $rest_base | `array` | Slug encji. |
| $this | `\Base_Manager` | Instancja obiektu. |

### Since {#since}

- 2.0.0
### Source {#source}

Zdefiniowany w [`inc/apis/trait-rest-api.php`](https://github.com/Ultimate-Multisite/ultimate-multisite/blob/main/inc/apis/trait-rest-api.php#L362) w linii 362
