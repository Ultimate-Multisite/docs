---
id: wu_page_before_render
title: Дзея - wu_page_before_render
sidebar_label: wu_page_before_render
_i18n_hash: fc447057ea50e758d4cf83bef375f7e6
---
# Action: wu_page_before_render {#action-wupagebeforerender}

Выклікаецца пачатку рэндэрынгу старонкі.

## Паметры {#parameters}

| Name | Type | Description |
|------|------|-------------|
| $page_id | `string\|\WP_Ultimo_Site_Exporter\Admin_Pages\Base_Admin_Page` | ID старонкі. |
| $this | `\WP_Ultimo_Site_Exporter\Admin_Pages\Base_Admin_Page` | Экземпляры Base Admin Page. |

### З {#since}

- 1.8.2
### Выкідзенне {#source}

Вызначана ў [`inc/admin-pages/class-base-admin-page.php`](https://github.com/Ultimate-Multisite/ultimate-multisite-site-exporter/blob/main/inc/admin-pages/class-base-admin-page.php#L356) на 356-й парэдзе.
