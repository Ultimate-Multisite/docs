...
---
# Пагляд выкарыстання гэтыхкаў {#hooks-usage-guide}
...
## Акцыйныя гэтыкі (Action Hooks) {#action-hooks}

### Жыцдарбы страўніка (Customer Lifecycle) {#customer-lifecycle}

#### Пасля стварэння страўніка {#after-customer-creation}
...
#### Змяненне статусу страўніка {#customer-status-change}
...
### Гэтыкі сайта (Site Hooks) {#site-hooks}

#### Пасля публікацыі сайта {#after-site-published}
...
#### Педыставя пакінування шаблону {#before-template-application}
...
### Гэтыкі Членства (Membership Hooks) {#membership-hooks}

#### Пераходы статусу {#status-transitions}
...
### Гэтыкі Плацёжаў (Payment Hooks) {#payment-hooks}

#### Аплатамнасць забіта / Неуможна {#payment-completed--failed}
...
### Гэтыкі Карыквання (Checkout Hooks) {#checkout-hooks}

#### Педыставя апрацоўкі / Пасля завяршэння {#before-processing--after-completion}
...
### Гэтыкі Доменаў (Domain Hooks) {#domain-hooks}
...
## Фільтрацыйныя гэтыкі (Filter Hooks) {#filter-hooks}

### Фільтры Цэнаўтварэння (Pricing Filters) {#pricing-filters}

#### Паганая карыквання (Cart Total) {#cart-total}
...
#### Стаўкі падатку (Tax Rates) {#tax-rates}
...
### Фільтры Шаблонаў (Template Filters) {#template-filters}
...
### Фільтры Абмежаванняў (Limitation Filters) {#limitation-filters}
...
### Фільтры Варот (Gateway Filters) {#gateway-filters}
...
### Паляў формы карыквання (Checkout Form Fields) {#checkout-form-fields}
...
### Змест электроннай пашты (Email Content) {#email-content}
