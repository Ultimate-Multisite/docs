---
title: Пашыраваныя прыклады кода
sidebar_position: 1
...
class Advanced_Site_Provisioner {
    ...
    public function provision_site($site, $membership) {
        ...
        // Устаноўка плагінаў на аснове плана
...
```php
/**
 * Атмарна павелічыць лічбавы слупац для пэўнага элемента.
 *
 * @param int    $item_id   Пермарынёк рытка, які трэба абнавіць.
 * @param string $column    Назва слупца для павелічэння (паўнога лічбавы).
 * @param int    $amount    Суме, якое трэба дадаць. Выкарыстоўвайце ад negatіўнага значэння для зменшэння.
 *                          Па спісы па замаўчанні 1.
 * @return bool True пры поспеху, false пры нешта адбываецца не так або калі слупац невазрашчальны.
 */
...
class Membership_API_Limiter {
    ...
    /** Максімум API выклікаў, дазволеных за billing cycle. */
    const LIMIT = 500;
    ...
    /**
     * Адмаўляе патрабаванне, калі абнававанне перавышае ліміт;
     * інакш атмарна адлічвае выклік.
     *
     * @param bool   $enabled
     * @param object $context  Об'ект з мэтай get_membership_id().
     * @return bool
     */
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        if ( (int) $membership->api_calls >= self::LIMIT ) {
            return false;  // Перавышае квоту — адмаўляе.
        }

        // Атмарнае павелічэнне: бяспечна пры адначасовасці патрабаванняў.
...
```php
// НЕБЯСПЕЧНЫ — умова гонкі паміж прачытаннем і запісам.
$membership = $memberships->get_item( $membership_id );
$new_count  = (int) $membership->api_calls + 1;
$memberships->update_item( $membership_id, [ 'api_calls' => $new_count ] );
```

Два адначасовыя патрабаванні могуць прачытаць адно і тое ж значэнне і абедзме папісаць адно павелічэнне, губляючы адзін лік. `increment_item()` перадае арытметыку ў мадыляцыйны рухавік базы дадзеных адным павелічэннем `UPDATE ... SET column = column + ?`, што робіць дзеянне ў тым імгненні атмарным.
