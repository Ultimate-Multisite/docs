---
id: wu_contains_element
title: Filter - wu_contains_element
sidebar_label: wu_contains_element
_i18n_hash: 5254f5cdc4fb5e3992e8987cc4c5d82b
---
# Filter: wu_contains_element

Membenarkan pembangun mengubah hasil carian awal.

Ini berguna untuk *builder* pihak ketiga dan lain-lain.

## Parameters

| Name | Type | Description |
|------|------|-------------|
| $contains_elements | `bool` | Jika elemen terkandung dalam kandungan. |
| $content | `string` | Kandungan yang diperiksa. |
| $element | `self` | Elemen semasa. |
| $post | `null\|\WP_Post` | pos yang perlu diperiksa. |

### Since

- 2.0.0
### Source

Defined in [`inc/ui/class-base-element.php`](https://github.com/Ultimate-Multisite/ultimate-multisite/blob/main/inc/ui/class-base-element.php#L534) at line 534
