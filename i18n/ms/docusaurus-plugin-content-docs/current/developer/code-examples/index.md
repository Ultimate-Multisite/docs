...
class Advanced_Site_Provisioner {
    ...
    public function provision_site($site, $membership) {
        ...
        // Pasang plugin berdasarkan pelan
...
```php
/**
 * Meningkatkan lajur numerik secara atomik untuk item tertentu.
 *
 * @param int    $item_id   Kunci utama baris yang akan dikemas kini.
 * @param string $column    Nama lajur yang akan ditingkatkan (mesti numerik).
 * @param int    $amount    Jumlah yang akan ditambah. Gunakan nilai negatif untuk mengurangkan.
 *                          Secara lalai adalah 1.
 * @return bool True jika berjaya, false jika gagal atau jika lajur tidak sah.
 */
...
// Tambah 5 kepada pengira penggunaan.
...
class Membership_API_Limiter {
    ...
    /** Had panggilan API yang dibenarkan bagi setiap kitaran bil. */
    const LIMIT = 500;
    ...
    /**
     * Menolak permintaan jika keahlian melebihi had;
     * jika tidak, kira panggilan secara atomik.
     *
     * @param bool   $enabled
     * @param object $context  Objek dengan kaedah get_membership_id().
     * @return bool
     */
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        // Peningkatan atomik: selamat di bawah permintaan serentak.
...
```php
// TIDAK SELAMAT — risiko perlumbaan antara baca dan tulis.
$membership = $memberships->get_item( $membership_id );
$new_count  = (int) $membership->api_calls + 1;
$memberships->update_item( $membership_id, [ 'api_calls' => $new_count ] );
```

Dua permintaan serentak boleh membaca nilai yang sama dan kedua-duanya menulis semula hasil peningkatan yang sama, menyebabkan satu kiraan hilang. `increment_item()` mendelegasikan aritmetik kepada enjin pangkalan data dengan satu pernyataan `UPDATE ... SET column = column + ?`, menjadikan operasi itu secara semula jadi atomik.
