---
id: wu_domain_seller_register_capabilities
title: କାର୍ଯ୍ୟ - wu_domain_seller_register_capabilities
sidebar_label: wu_domain_seller_register_capabilities
_i18n_hash: 2d0410d7e082c8a8d8f1fb970ccb40e2
---
# କାର୍ଯ୍ୟ: wu_domain_seller_register_capabilities {#action-wudomainsellerregistercapabilities}

ଅନ୍ୟ ପ୍ଲଗଇନଗୁଡ଼ିକୁ ଅତିରିକ୍ତ ଡୋମେନ ବିକ୍ରୟ କ୍ଷମତା ପଞ୍ଜିକରଣ କରିବାକୁ ଦେଇଥାଏ।

## ପାରାମିଟରଗୁଡ଼ିକ {#parameters}

| ନାମ | ପ୍ରକାର | ବର୍ଣ୍ଣନା |
|------|------|-------------|
| $registry | `\WP_Ultimo\Integrations\Integration_Registry` | ଇଣ୍ଟିଗ୍ରେସନ୍ ରେଜିଷ୍ଟ୍ରି। |

### ଯେଉଁ ସଂସ୍କରଣରୁ {#since}

- 2.1.0
### ସ୍ରୋତ {#source}

[`inc/class-domain-seller.php`](https://github.com/Ultimate-Multisite/ultimate-multisite-domain-seller/blob/main/inc/class-domain-seller.php#L320) ରେ ଲାଇନ 320ରେ ପରିଭାଷିତ।
