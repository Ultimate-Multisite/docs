---
id: wu_membership_update_forms
title: ଫିଲ୍ଟର - wu_membership_update_forms
sidebar_label: wu_membership_update_forms
_i18n_hash: b6890286b21a639011b037b0b06a11b6
---
# Filter: wu_membership_update_forms {#filter-wumembershipupdateforms}

ଡେଭେଲପରମାନଙ୍କୁ ନୂଆ ଅଦ୍ୟତନ ଫର୍ମ ସ୍ଲଗ୍‌ ଯୋଡ଼ିବାକୁ ଅନୁମତି ଦିଅନ୍ତୁ।

## ପାରାମିଟରଗୁଡ଼ିକ {#parameters}

| ନାମ | ପ୍ରକାର | ବିବରଣୀ |
|------|------|-------------|
| $slugs | `array` | ଏଡ଼ାଇବା ପାଇଁ ଫର୍ମ ସ୍ଲଗ୍‌ର ଏକ ତାଲିକା। |

### ଉତ୍ସ {#source}

[`inc/ui/class-checkout-element.php`](https://github.com/Ultimate-Multisite/ultimate-multisite/blob/main/inc/ui/class-checkout-element.php#L709) ରେ ୭୦୯ ନମ୍ବର ଧାଡ଼ିରେ ପରିଭାଷିତ।
