...
## Cap Captcha: そのまま使えるセルフホスト型保護 {#cap-captcha-self-hosted-protection-that-just-works}
...
## WooCommerce カードテスト保護 {#woocommerce-card-testing-protection}
...
## Google reCAPTCHA や hCaptcha がまだ必要ですか？ {#still-need-google-recaptcha-or-hcaptcha}
...
#### IP ホワイトリストフィルター {#ip-whitelist-filter}
...
## クライアント IP 検出と信頼済みプロキシ {#client-ip-detection--trusted-proxies}
...
#### Cloudflareヘッダーの信頼 {#cloudflare-header-trust}
...
#### 信頼済みプロキシ設定 {#trusted-proxy-configuration}
...
#### 初回有効化時の自動検出 {#first-enable-auto-detection}
