...
同時に発生した2つのリクエストが同じ値を読み取り、どちらも同じ増分後の結果を書き戻す可能性があり、その結果1回分のカウントが失われます。`increment_item()` は、単一の `UPDATE ... SET column = column + ?` 文で算術処理をデータベースエンジンに委ねるため、操作は本質的にアトミックになります。
