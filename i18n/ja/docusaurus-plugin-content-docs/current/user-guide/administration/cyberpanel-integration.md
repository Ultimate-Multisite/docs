...
### 1. CyberPanel APIの有効化 {#1-enable-the-cyberpanel-api}
...
### 2. wp-config.phpに定数を追加する {#2-add-constants-to-wp-configphp}
...
### 3. 連携機能の有効化 {#3-enable-the-integration}
...
### 4. 接続性の確認 {#4-verify-connectivity}
...
### API接続が拒否された場合 {#api-connection-refused}
...
### ドメインが作成されない場合 {#domain-not-created}
...
### SSL証明書が発行されない場合 {#ssl-certificate-not-issued}
