...
## ウィザードはいつ表示されますか？ {#when-does-the-wizard-appear}
...
## ステップ 2: ネットワーク設定 {#step-2-network-configuration}
...
## ステップ 3: インストール {#step-3-installation}
...
## 手動セットアップフォールバック {#manual-setup-fallback}
...
### ウィザードが wp-config.php が書き込み不可と表示する場合 {#the-wizard-says-wp-configphp-is-not-writable}
...
### 設定後にサイトがアクセスできない（サブドメイン） {#sites-are-not-accessible-after-setup-subdomains}
...
### 設定後の認証問題 {#authentication-issues-after-setup}
...
### インストール中にステップが失敗した場合 {#a-step-failed-during-installation}
