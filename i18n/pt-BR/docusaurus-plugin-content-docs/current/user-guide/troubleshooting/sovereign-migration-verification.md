...
---
# Verificação de Migração Soberana {#sovereign-migration-verification}
...
## Comandos a serem executados {#commands-to-run}
...
### Permissões do banco de dados não correspondem ao host {#database-grants-do-not-match-the-host}
...
### Instalações Bedrock ou locais não conseguem se conectar {#bedrock-or-local-installs-cannot-connect}
...
### Fila de push assíncrono não é esvaziada (drained) {#async-push-queue-does-not-drain}
...
### Contagem de usuários do inquilino está incorreta {#tenant-user-count-is-wrong}
...
### Visita SSO é rejeitada {#sso-visit-is-rejected}
...
## Quando tentar novamente {#when-to-retry}
