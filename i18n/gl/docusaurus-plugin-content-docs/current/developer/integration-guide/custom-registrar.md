---
title: Creación dunha integración de rexistrador personalizada
sidebar_position: 10
...
| Método | Desbloquea |
|---|---|
| `supports_whois_privacy(): bool` | Opción de privacidade WHOIS na configuración do produto |
| `enable_whois_privacy(string $domain_name): array` | Activa automaticamente a privacidade no rexistro |
| `get_dns_records(string $domain_name): array` | Pestana de vista DNS do cliente |
| `add_dns_record(string $domain_name, array $record_data): array` | O cliente pode engadir rexistros DNS |
| `update_dns_record(string $domain, string $record_id, array $record_data): array` | O cliente pode editar rexistros DNS |
| `delete_dns_record(string $domain, string $record_id, array $data): array` | O cliente pode eliminar rexistros DNS |
| `update_nameservers(string $domain_name, array $nameservers): array` | Xestión de nameservers |
...
O primeiro argumento de `add_capability()` é o **ID do provedor**: un slug en minúsculas usado para identificar o teu rexistrador na configuración, na configuración do produto e nas canles de rexistro. Usa algo único (por exemplo, o slug da túa empresa).
...
As credenciais gárdanse como opcións de rede usando os IDs dos campos como claves. Recupéraas na túa clase de capacidade con `wu_get_setting('my_registrar_api_key')`.
...
Os rexistros aparecen en **Administrador de rede › Ultimate Multisite › Rexistros › domain-seller-my-registrar**.
