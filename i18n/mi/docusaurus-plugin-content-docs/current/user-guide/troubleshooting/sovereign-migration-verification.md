...
## Command mo roto atu {#commands-to-run}
...
## Pākehā (Common failures) {#common-failures}

### He whakatere database (Database grants) ka whakahouhia i te host {#database-grants-do-not-match-the-host}
...
### He Bedrock me local installs ka whakatika i te koneki (cannot connect) {#bedrock-or-local-installs-cannot-connect}
...
### Async push queue ka whakatika i te drain (does not drain) {#async-push-queue-does-not-drain}
...
### He hā (count) o user tenant ka pēnei {#tenant-user-count-is-wrong}
...
### He wāha SSO ka pēnei (is rejected) {#sso-visit-is-rejected}
...
## I te wā e taea te whakatika (When to retry) {#when-to-retry}
