...
## Ngā Tūnga Whakawhanaungatanga (Prerequisites) {#prerequisites}
...
## Ngā Wāhi Whakawhanaungatanga (Requirements) {#requirements}
...
## Nereia Whakamahi (Setup Instructions) {#setup-instructions}

### 1. Whakawhanaungatanga API o CyberPanel (Enable the CyberPanel API) {#1-enable-the-cyberpanel-api}
...
### 2. Whakawhiti Constants ki wp-config.php (Add Constants to wp-config.php) {#2-add-constants-to-wp-configphp}
...
### 3. Whakawhanaungatanga (Enable the Integration) {#3-enable-the-integration}
...
### 4. Whakawhiti Whakamahi (Verify Connectivity) {#4-verify-connectivity}
...
## Whakamārama (Troubleshooting) {#configuration-reference}

### API Connection Refused {#important-notes}
...
### Domain Not Created {#api-connection-refused}
...
### SSL Certificate Not Issued {#authentication-errors}
