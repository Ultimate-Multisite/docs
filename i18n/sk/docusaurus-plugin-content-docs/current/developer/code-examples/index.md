...
Dve súčasné požiadavky môžu prečítať rovnakú hodnotu a obe zapísať späť rovnaký zvýšený výsledok, čím sa stratí jedno započítanie. `increment_item()` deleguje aritmetiku na databázový engine pomocou jediného príkazu `UPDATE ... SET column = column + ?`, vďaka čomu je operácia prirodzene atomická.
