...
## Wax-soo-saariyadaha (Features) {#features}
...
### 1. Hubi Furaha API-gaaga Closte (Verify Your Closte API Key) {#1-verify-your-closte-api-key}
...
### 2. Fur Isku-dhafka (Enable the Integration) {#2-enable-the-integration}
...
## Abuurista Diyaarka Domain-ka (Domain Record Creation) {#domain-record-creation}
...
### Masalahaat Ku Saabsan Isku-xirka API (API Connection Issues) {#api-connection-issues}
...
### Masalahaat Ku Saabsan Shahaadada SSL (SSL Certificate Issues) {#ssl-certificate-issues}
...
### Domain-ka Ma La Diiwaan gelin (Domain Not Added) {#domain-not-added}
...
### Marinta Hubinta DNS (DNS Check Interval) {#dns-check-interval}
