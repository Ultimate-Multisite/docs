---
title: Προηγμένα Παραδείγματα Κώδικα
sidebar_position: 1
...
Δύο ταυτόχρονα αιτήματα μπορούν να διαβάσουν την ίδια τιμή και και τα δύο να γράψουν πίσω το ίδιο αυξημένο αποτέλεσμα, καταστρέφοντας μία μέτρηση. Το `increment_item()` μεταβιβάζει τον υπολογισμό στην μηχανή της βάσης δεδομένων με μια ενιαία δήλωση `UPDATE ... SET column = column + ?`, κάνοντας τη λειτουργία εγγενώς ατομική.
