...
Ultimate Multisite: Multi-Tenancy 1.2.0 suporta isolamento de banco de dados e sistema de arquivos por subsite para inquilinos soberanos. Isso mantém os dados do inquilino separados, ao mesmo tempo que preserva o provisionamento em nível de rede, faturamento e administração.

## Estratégia de Isolamento

Use o isolamento soberano para clientes que necessitem de uma separação de dados mais forte, armazenamento de sistema de arquivos dedicado ou um limite de host separado.
...
3. Use `127.0.0.1` ou um nome de host de serviço apenas quando o servidor de banco de dados concede privilégios a esse host.
...
4. Provisionar os usuários do inquilino.
5. Configurar os caminhos do sistema de arquivos do inquilino.
6. Executar a verificação da migração.
7. Mudar o roteamento ou DNS após a verificação ser bem-sucedida.
...
## Fluxos de gerenciamento de clientes soberanos

O Ultimate Multisite v2.13.0 mantém as ações de gerenciamento de clientes no site principal quando o modo soberano está ativado. Um inquilino ainda pode funcionar como uma instalação do WordPress isolada, mas ações voltadas ao cliente que dependem de faturamento de rede, associação ou dados de conta compartilhada devem redirecionar o cliente para o site principal em vez de tentar concluir a ação dentro da execução do inquilino.

O fluxo do site principal se aplica a:

- Alterações no checkout e planos.
