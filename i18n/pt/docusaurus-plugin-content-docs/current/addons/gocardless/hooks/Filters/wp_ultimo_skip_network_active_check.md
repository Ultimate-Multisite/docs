---
id: wp_ultimo_skip_network_active_check
title: Filtro - wp_ultimo_skip_network_active_check
sidebar_label: wp_ultimo_skip_network_active_check
_i18n_hash: 9061f073ee77c38afc3cecfabe5edb50
---
# Filtro: wp_ultimo_skip_network_active_check

Permite que desenvolvedores interrompam antecipadamente esta verificação.

Isto é útil ao usar configurações baseadas em composer e outras configurações personalizadas, como Bedrock, por exemplo, onde usar plugins como mu-plugins é a norma.

### Desde

- 2.0.0
### Origem

Definido em [`inc/class-requirements.php`](https://github.com/Ultimate-Multisite/ultimate-multisite-gocardless/blob/main/inc/class-requirements.php#L229) na linha 229


## Retorna
