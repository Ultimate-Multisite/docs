...
Filtra a força mínima da palavra-passe exigida (pontuação zxcvbn).

Níveis de força: - 0, 1: Muito fraca - 2: Fraca - 3: Média - 4: Forte (predefinição)
...
| $strength_setting | `string` | O valor da definição de administrador (medium, strong, super_strong). |
