...
## Cap Captcha：开箱即用的自托管防护 {#cap-captcha-self-hosted-protection-that-just-works}
...
## WooCommerce 卡片测试防护 {#woocommerce-card-testing-protection}
...
## 仍然需要 Google reCAPTCHA 或 hCaptcha？ {#still-need-google-recaptcha-or-hcaptcha}
...
## 我需要配置任何内容吗？ {#do-i-need-to-configure-anything}

不需要！当你激活 addon 时，Cap Captcha 会自动启用。你的整个网络会立即受到保护——无需配置设置，无需输入 API 密钥。

## Cap Captcha 需要 API 密钥吗？ {#do-i-need-api-keys-for-cap-captcha}

不需要。Cap Captcha 完全自托管。无需外部账号、无需域名注册、没有配置烦恼。

## 这会拖慢我的结账流程吗？ {#will-this-slow-down-my-checkout}

Cap Captcha 只有 20KB，并在后台运行工作量证明挑战。你的客户不会察觉到任何延迟。

## 这能防护卡片测试攻击吗？ {#does-this-protect-against-card-testing-attacks}

可以！不同于只保护 HTML 表单的传统验证码，我们的 WooCommerce 集成会直接挂接到 Store API，在 API 层级阻止机器人攻击——这正是大多数卡片测试攻击发生的位置。

## 我可以在不同站点使用不同的验证码提供商吗？ {#can-i-use-different-captcha-providers-on-different-sites}

验证码提供商是在网络范围内设置的，但你可以根据需要调整难度设置。

## 这符合 GDPR 吗？ {#is-this-gdpr-compliant}
...
## Client IP 检测与受信任代理 {#client-ip-detection--trusted-proxies}
...
#### Cloudflare Header 信任 {#cloudflare-header-trust}
...
#### 受信任代理配置 {#trusted-proxy-configuration}
...
#### 首次启用自动检测 {#first-enable-auto-detection}
