...
## 访问 Agent Picker {#accessing-the-agent-picker}
...
## 五个内置 Agents {#the-five-built-in-agents}
...
## Superdav 自动化集成 {#superdav-automation-integrations}
...
## 自定义 Agent {#customising-agents}

每个内置 agent 都可以通过 `gratis_ai_agent_agents` filter 进行扩展或替换。

### 向现有 agent 添加自定义系统提示 {#adding-a-custom-system-prompt-to-an-existing-agent}
...
### 注册新 agent {#registering-a-new-agent}
...
### 移除内置 agent {#removing-a-built-in-agent}
