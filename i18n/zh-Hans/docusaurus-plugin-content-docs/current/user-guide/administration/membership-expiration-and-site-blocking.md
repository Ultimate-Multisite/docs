...
## 会员资格状态生命周期 {#membership-status-lifecycle}
...
### 会员资格如何转为过期 {#how-memberships-transition-to-expired}
...
#### 自动续费与非自动续费会员资格 {#auto-renewing-vs-non-auto-renewing-memberships}
...
### 网站被封禁时访客看到什么 {#what-visitors-see-when-a-site-is-blocked}
...
### 什么会被封禁以及何时封禁 {#what-gets-blocked-and-when}
...
## 故障排除：会员资格过期后网站仍可访问 {#troubleshooting-sites-remaining-accessible-after-expiration}

如果会员资格过期后网站仍然可以公开访问，请按顺序执行以下检查：

### 1. 确认 Block Frontend Access 设置已启用 {#1-verify-the-block-frontend-access-setting-is-enabled}

前往 **Ultimate Multisite > Settings > Memberships**，确认 **Block Frontend Access** 开关已打开。此设置 **默认关闭**，这意味着当会员资格不活跃时，只有 wp-admin 会被限制。

### 2. 检查前端封禁宽限期 {#2-check-the-frontend-block-grace-period}
...
### 3. 确认会员资格状态是否已实际改变 {#3-confirm-the-membership-status-has-actually-changed}
...
### 4. 验证 Action Scheduler 是否正在运行 {#4-verify-action-scheduler-is-running}
...
### 5. 检查网关 Webhook 问题（自动续费会员资格） {#5-check-for-gateway-webhook-issues-auto-renewing-memberships}
...
### 6. 检查过期宽限期（Cron 级别） {#6-check-the-expiration-grace-period-cron-level}
...
### 7. 手动过期会员资格 {#7-manually-expire-a-membership}
