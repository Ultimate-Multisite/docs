...
## Öňün Gerekli Şertler (Prerequisites) {#prerequisites}
...
## Kurmak Üçin Talbatlar {#setup-instructions}

### 1. CyberPanel API-ni İşleterlik {#1-enable-the-cyberpanel-api}
...
### 2. wp-config.php-e Konstanta goşmak {#2-add-constants-to-wp-configphp}
...
### 3. Integrasyonu Etkinleştirme {#3-enable-the-integration}
...
### 4. Bağlantıyı Doğrulama {#4-verify-connectivity}
...
### Email Forwarderleri {#email-forwarders}
...
## Sorunlary Gözläp Çyrma (Troubleshooting) {#troubleshooting}

### API Bağlanyp Bilmez (API Connection Refused) {#api-connection-refused}
...
### SSL Sertifikaty Berilmedi {#ssl-certificate-not-issued}
