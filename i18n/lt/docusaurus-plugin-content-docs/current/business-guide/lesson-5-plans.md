---
title: '5 pamoka: savo planų kūrimas'
sidebar_position: 6
_i18n_hash: 8c58dfd0a186bd585f43342bc4ea4f43
---
# 5 pamoka: savo planų kūrimas {#lesson-5-designing-your-plans}

Jūsų planų pakopos nėra tik kainų lygiai -- jos atspindi, kaip iš tikrųjų veikia jūsų nišos klientai. Šioje pamokoje sukursite produktų pakopas, atitinkančias tikruosius skirtinguose etapuose esančių fitneso verslų poreikius.

## Kur sustojome {#where-we-left-off}

FitSite turi paruoštus tris šablonus (Studio Essential, Gym Pro, Fitness Chain). Dabar sukursime planus, kurie nustato, prie ko klientai gauna prieigą ir kiek moka.

## Mąstymas pagal klientų segmentus {#thinking-in-customer-segments}

Klaida, kurią daro dauguma žmonių, yra planų kūrimas pagal technines funkcijas (saugykla, pralaidumas, puslapių skaičius). Jūsų nišos klientai apie tai negalvoja tokiais terminais. Sporto klubo savininkas galvoja apie tai, ko jam reikia verslui valdyti.

Fitneso studijoms yra trys natūralūs segmentai:

| Segmentas | Kas jie yra | Ko jiems reikia |
|---------|-------------|----------------|
| **Individualūs treneriai / mažos studijos** | 1-3 darbuotojai, viena vieta, pradedantieji | Profesionali svetainė, užsiėmimų informacija, kontaktinė forma |
| **Įsitvirtinę sporto klubai** | 5-20 darbuotojų, viena vieta, augantys | Viskas, kas aukščiau, ir rezervacijos, tinklaraštis, galerija, pasirinktinis domenas |
| **Fitneso tinklai** | Kelios vietos, įsitvirtinęs prekės ženklas | Viskas, kas aukščiau, ir kelių svetainių palaikymas, vietų puslapiai, darbuotojų katalogas |

Jūsų planai turėtų atitikti šiuos segmentus, o ne savavališkus funkcijų rinkinius.

## FitSite planų kūrimas {#creating-the-fitsite-plans}

Eikite į **Ultimate Multisite > Produktai > Pridėti produktą** kiekvienam planui.

### 1 planas: FitSite Starter -- 49 USD/mėn. {#plan-1-fitsite-starter----49month}

**Tikslinė auditorija**: individualūs treneriai ir mažos studijos

**Aprašymo skirtukas**:
- Pavadinimas: FitSite Starter
- Aprašymas: „Viskas, ko reikia asmeniniam treneriui ar mažai studijai, kad internete atrodytų profesionaliai.“

**Bendras skirtukas**:
- Produkto tipas: planas
- Kliento vaidmuo: administratorius

**Svetainių šablonų skirtukas**:
- Leisti svetainių šablonus: įjungta
- Galimi šablonai: tik Studio Essential

**Apribojimai**:
- Svetainės: 1
- Disko vieta: 1 GB
- Pasirinktinis domenas: išjungta (naudoja `studioname.fitsite.com`)

**Plugins skirtukas**:
- Kontaktinės formos plugin: priverstinai aktyvuoti
- SEO plugin: priverstinai aktyvuoti
- Rezervacijų plugin: nepasiekiamas (paskata atsinaujinti)

**Themes skirtukas**:
- Jūsų pasirinkta theme: priverstinai aktyvuoti
- Visos kitos themes: paslėptos

### 2 planas: FitSite Growth -- 99 USD/mėn. {#plan-2-fitsite-growth----99month}

**Tikslinė auditorija**: įsitvirtinę vienos vietos sporto klubai

**Aprašymo skirtukas**:
- Pavadinimas: FitSite Growth
- Aprašymas: „Įsitvirtinusiems sporto klubams, pasirengusiems auginti savo buvimą internete ir pritraukti naujų narių.“

**Svetainių šablonų skirtukas**:
- Galimi šablonai: Studio Essential ir Gym Pro

**Apribojimai**:
- Svetainės: 1
- Disko vieta: 5 GB
- Pasirinktinis domenas: įjungta

**Plugins skirtukas**:
- Viskas, kas yra Starter, ir:
- Rezervacijų plugin: priverstinai aktyvuoti
- Galerijos plugin: priverstinai aktyvuoti
- Tinklaraščio funkcionalumas: pasiekiamas

**Atnaujinimų ir grąžinimų į žemesnį planą skirtukas**:
- Plano grupė: FitSite Plans
- Produkto eilė: 2

### 3 planas: FitSite Pro -- 199 USD/mėn. {#plan-3-fitsite-pro----199month}

**Tikslinė auditorija**: kelių vietų fitneso tinklai

**Aprašymo skirtukas**:
- Pavadinimas: FitSite Pro
- Aprašymas: „Visapusiška platforma fitneso prekių ženklams, turintiems kelias vietas.“

**Svetainių šablonų skirtukas**:
- Galimi šablonai: visi trys šablonai

**Apribojimai**:
- Svetainės: 5 (po vieną kiekvienai vietai)
- Disko vieta: 20 GB
- Pasirinktinis domenas: įjungta

**Plugins skirtukas**:
- Viskas, kas yra Growth, ir:
- Visi premium plugins: priverstinai aktyvuoti

**Atnaujinimų ir grąžinimų į žemesnį planą skirtukas**:
- Plano grupė: FitSite Plans
- Produkto eilė: 3

## Plano grupės nustatymas {#setting-up-the-plan-group}

Plano grupė užtikrina, kad klientai galėtų atsinaujinti arba pereiti į žemesnį planą tik FitSite planų šeimoje. Kiekvieno plano skirtuke **Atnaujinimai ir grąžinimai į žemesnį planą**:

1. Nustatykite **Plano grupę** į „FitSite Plans“ visiems trims planams
2. Nustatykite **Produkto eilę** į 1 (Starter), 2 (Growth), 3 (Pro)

Tai sukuria aiškų atnaujinimo kelią: Starter → Growth → Pro.

## Užsakymo papildomų pasiūlymų pridėjimas {#adding-order-bumps}

Užsakymo papildomi pasiūlymai yra papildomi produktai, siūlomi atsiskaitymo metu. FitSite atveju apsvarstykite:

- **Papildomos saugyklos paketas** (19 USD/mėn.) -- papildoma 5 GB disko vieta
- **Prioritetinis palaikymas** (29 USD/mėn.) -- greitesnis atsakymo laikas
- **Papildoma svetainė** (39 USD/mėn.) -- klientams, kuriems reikia daugiau svetainių, nei leidžia jų planas

Sukurkite juos kaip **paketo** tipo produktus Ultimate Multisite ir susiekite su atitinkamais planais.

## Kodėl ši struktūra veikia {#why-this-structure-works}

- **Starter** pašalina įėjimo kliūtis -- maža kaina, paprastas pasiūlymas, treneriai greitai atsiduria internete
- **Growth** prideda funkcijas, kurių sporto klubai iš tikrųjų prašo -- rezervacijos, galerijos, pasirinktiniai domenai
- **Pro** aptarnauja didelės vertės segmentą, kuriam reikia kelių vietų palaikymo
- **Užsakymo papildomi pasiūlymai** leidžia klientams prisitaikyti paslaugą neapsunkinant pagrindinių planų
- **Aiškus atnaujinimo kelias** reiškia, kad klientai auga kartu su jumis, o ne išeina

## FitSite tinklas iki šiol {#the-fitsite-network-so-far}

```
FitSite Network
├── WordPress Multisite (subdomain mode)
├── Ultimate Multisite (configured)
├── Hosting with wildcard SSL + domain mapping
├── Site Templates (Studio Essential, Gym Pro, Fitness Chain)
├── Products
│   ├── FitSite Starter ($49/mo) → Studio Essential template
│   ├── FitSite Growth ($99/mo) → Studio Essential + Gym Pro templates
│   ├── FitSite Pro ($199/mo) → All templates
│   └── Order Bumps (Extra Storage, Priority Support, Additional Site)
└── Ready for checkout configuration (next lesson)
```

## Ką sukūrėme šioje pamokoje {#what-we-built-this-lesson}

- **Trys planų pakopos**, susietos su realiais fitneso verslo segmentais
- **Funkcijų ribojimas** naudojant Ultimate Multisite plugin ir šablonų valdiklius
- **Plano grupė** su aiškiu atnaujinimo keliu
- **Užsakymo papildomų pasiūlymų produktai** papildomoms pajamoms
- **Kainodaros struktūra**, pagrįsta kliento verte, o ne techninėmis specifikacijomis

---

**Toliau:** [6 pamoka: registracijos patirtis](lesson-6-checkout) -- sukursime atsiskaitymo srautą, kuris fitneso studijų savininkus paverčia mokančiais klientais.
