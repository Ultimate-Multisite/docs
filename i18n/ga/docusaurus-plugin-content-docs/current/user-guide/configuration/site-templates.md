...
Is é sin an t-amach: leat féidir leat a bheith ag curte site bunaithe, a chloisteanna (plugins) eile a chloisteanna, a theama (theme) activt a chur ar fáil, agus a chuidú go bith mar atá tú ag iarraidh. Ansin, nuair a bhíonn do chliacht ag curte account nua, níl tú ag iarraidh site WordPress de-default le aon curtha sinte ag anseo, ach beidh tú ag iarraidh cointinn (copy) de do site bunaithe le gcuid chuid cúiseanna agus curtha sinte atá ag curte.
...
Is é seo a bheith ag curte moal (modal window) a bhíonn ag iarraidh an **Site title, Site Domain/path,** agus an **Site type**. Ag cur le chéile ar an fáil ag an **Site Type** drop-down field, déan cinnte go bhfuil tú ag chur i bhfeic **Site Template** *.*

_![Add site template modal with site type dropdown](/img/config/site-templates-list.png)_

Is féidir leat a chur in áit a chur ar fáil do chliacht chun a gluthán a chur ar fáil go bhfuil an t-amach ag curte:

![Site template description field on the site template edit screen](/img/config/site-template-edit-description.png)

Arís an cur síneadh (toggle switch) **Copy Site** ag an t-amach is dearmad ar an cur fhoirmi. Beidh sé ag ghlúid leat a bheith agat inamh a new site template ar fáil tríd an site template atá ag taitneamh, mar chuid a bhfostála chun amáil a chur ar fáil ná a fháil é arís ó bhfostála nua.
...
Ar chuid na hé, tá de dualte ar chuid ar féidir leat a bheith agat ar fáil anois (gan bheith ag déanamh idirí shaothair):
...
Leat é seo a dhéanamh, bíodh tú ag cur isteach i **Ultimate Multisite > Products > Edit**.

![Edit product to assign site template](/img/config/product-site-templates.png)

Is féidir leis seo a bheith agat ar an cur síos **Edit Product** (Edit Product). Under an seachtrann **Product Options**, glac tú an tab **Site template** agus déan chun an option **Assign Site Template** a chur i bhfeicimseán drop-down. Beidh sé ag tairiscint an liosta deathtanna site templates atá ar fáil agus beidh sé ag ceart gointt ar an template atá ag úsáid ar chuid deán.
...
Ar an cur dia ar **Edit Product** page. Oibríonn tú i ndáir **Product Options**, agus bíodh an tab **Site template** agat, chun an option **Choose Available Site Template** a chur in áit ó leat. Beidh sé ag tairiscint an liosta de site templates atá ar fáil agus beidh sé ag ceart go leat a chur i bhfeicim an site template a bhfuil tú ag cur isteach. Is féidir leat é seo a dhéanamh ag tairiscint a chomhlaoiú: **Available** má tá tú ag iarraidh an site template a bheith ar liosta, _**Not Available**_ má níl tú ag iarraidh go bhfuil an site template ag tairiscint mar option, agus **Pre-selected** má tá tú ag iarraidh go bhfuil aon de site templates atá ar fáil ag bheith mar chomhlaoiú leat.
...
Beidh sé ag tairiscint an cur **Edit Checkout Form**. Find an field **Template Selection** agus cliceáil ar **Edit** ar é.

Beidh moal window ag tairiscint. Under an field **Template Sites** leat é seo a chur in áit agus a liostaiteáil gach site template atá ar fáil agat tríd an registration. Beidh an site templates a bhfuil tú ag cur isteach ó chuid seo ar fáil níl aon cheann de chuid product a bhí ag cur isteach.

![Template selection field in the checkout form editor](/img/config/checkout-form-template-step.png)

Ar an frontend, beidh na ghráisteoirí ag fheiceáil an chooser template tríd an checkout agus beidh siad ag ceart a bhaint as an deashbhlaiseacht atá ar fáil do chuid nua site.
...
Mar daccais, má tá tú ag iarraidh ghráinneán an teangail (company name) de do chuid úsáideor ar fáil ag ríoladh agus go dtí an teangail a chur ar an curte (home page), leat ag cur placeholders ar fáil ar do home page site template. Is féidir é seo a chur ar fáil, mar i an imáir a bhonraithe (image) a bhonraithe seo (caithfidh placeholders a bheith curtha ar fáil ar chuid d'aon chuid dubhair - {{placeholder_name}}).

![Homepage with placeholder text in curly braces](/img/config/site-templates-list.png)

Ansin, leat ag cur sóisín ríoladh (registration field) a bhaineann leis an form checkout atá agat chun dá dath a bhaint ar fáil. An amachasfóir form checkout a bhonraithe a bhonraithe seo chun feidrigh a chur ar fáil leat ag cur placeholders ar chuid ardánanna:

![Template selection and registration fields in the checkout form editor](/img/config/checkout-form-template-step.png)

Beidh do chuid ansin ag cur an fáil iad an fáil seo ag ríoladh.

![Frontend checkout form with template chooser](/img/config/frontend-template-chooser.png)

Ansin, beidh Ultimate Multisite ag gúntáil an placeholders le an dath a bhí cur isteach ag an chuid úsáideor go dtí an t-amhasgairt.
...
Si é seo a chuidíonn leis an talamh seo, ag cur isteach fáil ar fáil an option a chur ar fáil chuid fhísiúla (fake values) do theachdaí, agus a úsáidtear iad chun gníomhais an tairiscint agus an tairiscint deargadh (search and replace) ar na siti template ag curte chuid isteach go dtí an t-aon-chúirt a bhfaidh na custaimerí.

Is féidir leat fáil ar fáil ar an editor placeholder seo ag cur isteach ar **Ultimate Multisite > Settings > Sites**, agus ansin, ar an sidebar, chun an link **Edit Placeholders** a chluainteadh.

![Site template settings under the Sites settings page](/img/config/settings-sites-templates-section.png)

Beidh sé ag cur leat ar an editor deargadh na placeholderí, agus is féidir leat a chur isteach placeholders agus an t-aon-chúirt a bhfaidh siad.
