...
---
# Проверка миграции суверенного сайта {#sovereign-migration-verification}
...
## Команды для выполнения {#commands-to-run}
...
## Распространенные ошибки {#common-failures}

### Разрешения в базе данных не совпадают с хостом {#database-grants-do-not-match-the-host}
...
### Установки Bedrock или локальные сокеты не могут подключиться {#bedrock-or-local-installs-cannot-connect}
...
### Очередь асинхронного пуша не очищается (drain) {#async-push-queue-does-not-drain}
...
### Количество пользователей арендатора неверно {#tenant-user-count-is-wrong}
...
### Посещение SSO отклонено {#sso-visit-is-rejected}
...
## Когда повторять попытку {#when-to-retry}
