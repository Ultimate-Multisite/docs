...
## Cap Captcha : une protection auto-hébergée qui fonctionne tout simplement {#cap-captcha-self-hosted-protection-that-just-works}
...
## Protection contre les tests de cartes WooCommerce {#woocommerce-card-testing-protection}
...
## Vous avez toujours besoin de Google reCAPTCHA ou hCaptcha ? {#still-need-google-recaptcha-or-hcaptcha}
...
## Conçu pour évoluer {#built-for-scale}
...
## Dois-je configurer quelque chose ? {#do-i-need-to-configure-anything}
...
## Ai-je besoin de clés API pour Cap Captcha ? {#do-i-need-api-keys-for-cap-captcha}
...
## Cela ralentira-t-il mon checkout ? {#will-this-slow-down-my-checkout}
...
## Cela protège-t-il contre les attaques de test de cartes ? {#does-this-protect-against-card-testing-attacks}
...
## Puis-je utiliser différents fournisseurs de captcha sur différents sites ? {#can-i-use-different-captcha-providers-on-different-sites}
...
#### Temporisation du tarpit {#tarpit-timing}
...
#### Filtre de liste blanche IP {#ip-whitelist-filter}
...
#### Action avant blocage {#pre-block-action}
...
## Détection de l’IP client et proxys de confiance {#client-ip-detection--trusted-proxies}
...
### Modèle de confiance {#trust-model}
...
#### Confiance du header Cloudflare {#cloudflare-header-trust}
...
#### Configuration des proxy de confiance {#trusted-proxy-configuration}
...
#### Auto-détection lors de la première activation {#first-enable-auto-detection}
