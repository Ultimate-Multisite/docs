---
title: Pielāgotas reģistratora integrācijas izveide
sidebar_position: 10
...
| Metode | Atbloķē |
|---|---|
| `supports_whois_privacy(): bool` | WHOIS privātuma opcija produkta iestatījumos |
| `enable_whois_privacy(string $domain_name): array` | Automātiski iespējo privātumu reģistrācijas laikā |
| `get_dns_records(string $domain_name): array` | Klienta DNS skata cilne |
| `add_dns_record(string $domain_name, array $record_data): array` | Klients var pievienot DNS ierakstus |
| `update_dns_record(string $domain, string $record_id, array $record_data): array` | Klients var rediģēt DNS ierakstus |
| `delete_dns_record(string $domain, string $record_id, array $data): array` | Klients var dzēst DNS ierakstus |
| `update_nameservers(string $domain_name, array $nameservers): array` | Nameserver pārvaldība |
...
Pirmais `add_capability()` arguments ir **provider ID** — mazajiem burtiem rakstīts slug, ko izmanto, lai identificētu jūsu reģistratoru iestatījumos, produkta konfigurācijā un žurnālu kanālos. Izmantojiet kaut ko unikālu (piemēram, jūsu uzņēmuma slug).
...
Akreditācijas dati tiek glabāti kā tīkla opcijas, izmantojot lauku ID kā atslēgas. Iegūstiet tos savā capability klasē ar `wu_get_setting('my_registrar_api_key')`.
...
Žurnāli parādās sadaļā **Tīkla administrēšana › Ultimate Multisite › Žurnāli › domain-seller-my-registrar**.
