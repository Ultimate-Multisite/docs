...
---
# Guida all'uso dei Hooks {#hooks-usage-guide}

Ultimate Multisite fornisce oltre 200 hook di azione e oltre 280 hook di filtro. Questa pagina copre i hook più comunemente usati con esempi pratici. Per una referenza completa auto-generata, consulta le altre pagine di questa sezione.

## Hook di azione {#action-hooks}

### Ciclo di vita del cliente {#customer-lifecycle}

#### Dopo la creazione del cliente {#after-customer-creation}
...
#### Cambio di stato del cliente {#customer-status-change}
...
### Hook del sito {#site-hooks}

#### Dopo la pubblicazione del sito {#after-site-published}
...
#### Prima dell'applicazione del template {#before-template-application}
...
### Hook dell'iscrizione {#membership-hooks}

#### Transizioni di stato {#status-transitions}
...
### Hook di pagamento {#payment-hooks}

#### Pagamento completato / fallito {#payment-completed--failed}
...
### Hook del checkout {#checkout-hooks}

#### Prima della elaborazione / Dopo il completamento {#before-processing--after-completion}
...
## Hook di filtro {#filter-hooks}

### Filtri di prezzo {#pricing-filters}
...
### Filtri del template {#template-filters}
...
### Filtri di limitazione {#limitation-filters}
...
### Filtri del gateway {#gateway-filters}
...
### Campi del modulo di checkout {#checkout-form-fields}
...
### Contenuto dell'email {#email-content}
