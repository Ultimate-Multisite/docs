...
### 1. ჩართეთ CyberPanel API {#1-enable-the-cyberpanel-api}
...
### 2. დაამატეთ კონსტანტები wp-config.php-ში {#2-add-constants-to-wp-configphp}
...
### 4. გადაამოწმეთ კავშირი {#4-verify-connectivity}
...
### Domain Mapping (დომენის ერკვევა) {#domain-mapping}
...
### Auto-SSL (ავტომატური SSL) {#auto-ssl}
...
### www Subdomain (www ქვედომენი) {#www-subdomain}
...
### Email Forwarders (ელფოსტის გადამხდელები) {#email-forwarders}
...
## კონფიგურაციის მითითებები {#configuration-reference}
...
### API-ის კავშირის უარყოფა (API Connection Refused) {#api-connection-refused}
...
### ავთენტიფიკაციის შეცდომები (Authentication Errors) {#authentication-errors}
...
### დომეინის არარსებობა (Domain Not Created) {#domain-not-created}
...
### SSL სერტიფიკატის გამოცემა არ ხდება (SSL Certificate Not Issued) {#ssl-certificate-not-issued}
...
## დამატებითი ინფორმაცია (References) {#references}
