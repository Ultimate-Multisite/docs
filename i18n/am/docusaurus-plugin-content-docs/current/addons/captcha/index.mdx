...
## Cap Captcha: በራስ-የሚስተናገደ ጥበቃ፣ የሚሰራውም ብቻ ነው {#cap-captcha-self-hosted-protection-that-just-works}
...
## WooCommerce የካርድ ሙከራ ጥበቃ {#woocommerce-card-testing-protection}
...
## አሁንም Google reCAPTCHA ወይም hCaptcha ያስፈልጋል? {#still-need-google-recaptcha-or-hcaptcha}
...
## ለపెርፋለም (Scale) የተሰራ {#built-for-scale}
...
## በተለያዩ ድረ-ገጾች የተለያዩ captcha አቅራቢዎችን መጠቀም እችላለሁ? {#can-i-use-different-captcha-providers-on-different-sites}
...
## የጥቃት መጠን መገደብ (Rate Limiting) {#rate-limiting}
...
#### የትራፒት ጊዜ (Tarpit Timing) {#tarpit-timing}
...
### ሊሰፋ የሚችልበት አቅም (Extensibility) {#extensibility}

#### የIP ዝርዝር መብራት (IP Whitelist Filter) {#ip-whitelist-filter}
...
#### ከጥቃት በፊት የሚደረግ እርምጃ (Pre-Block Action) {#pre-block-action}
...
## የደንበኛ IP መለየት እና የተpercaya ፕሮክሲዎች (Client IP Detection & Trusted Proxies) {#client-ip-detection--trusted-proxies}
...
### የእምነት ሞዴል (Trust Model) {#trust-model}
...
### ቅንብር (Configuration) {#configuration-1}

#### የCloudflare ሄደር እምነት (Cloudflare Header Trust) {#cloudflare-header-trust}
...
#### የተpercaya ፕሮክሲ ቅንብር (Trusted Proxy Configuration) {#trusted-proxy-configuration}
...
#### የመጀመሪያ ጊዜ ማስጀመር በራስ-መለያየት (First-Enable Auto-Detection) {#first-enable-auto-detection}
...
### ይህ ለምን አስፈላጊ ነው? {#why-this-matters}
