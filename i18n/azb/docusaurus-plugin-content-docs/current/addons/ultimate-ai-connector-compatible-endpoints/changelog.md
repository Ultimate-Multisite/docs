---
title: Ultimate AI Connector for Compatible Endpoints Changelog
sidebar_position: 99
_i18n_hash: bc1e5acc3b2415defb863e64a18d271c
---
# Uyumlu Uç Noktalar için Ultimate AI Connector Değişiklik Günlüğü

## Version 2.0.0 — Yayınlandı: 2026-04-24

* Yeni: Çoklu sağlayıcı desteği — birden fazla AI uç noktasını yapılandırabilir ve sağlayıcılar arasında otomatik geri dönüş (fallback) ile istek yönlendirebilirsiniz.
* Düzeltme: Sağlayıcı başına doğru sağlayıcı kimlikleri, kayıt URL'leri ve HTTP filtre kapsam belirleme ile çoklu sağlayıcı SDK entegrasyonu yapıldı.
* Düzeltme: Yeni sağlayıcı kartı artık eklenirken otomatik olarak genişliyor; eklenti güncellendiğinde script önbellek temizleme (cache busting) yapılıyor.
* Düzeltme: `eval()` için dinamik sağlayıcı sınıf ad alanı ve yeni eklenen sağlayıcılar için otomatik genişleme davranışı sağlandı.
* Düzeltme: WordPress 6.9+ uyumluluğu için artık deneysel olmayan stabil Card/CardBody/CardHeader/CardDivider bileşenleri kullanıldı.
* Düzeltme: Sağlayıcı sürükle-bırak ile yeniden sıralama için tanımlanmamış DragHandle, Unicode tutma simgesi ile değiştirildi.
* Düzeltme: Her sayfa yüklenmesinde tetiklenen engelleme yapan HTTP isteği kaldırıldı.
* Geliştirildi: GitHub Actions iş akışları Node.js 24'e yükseltildi.

## Version 1.0.0 — İlk Yayın
