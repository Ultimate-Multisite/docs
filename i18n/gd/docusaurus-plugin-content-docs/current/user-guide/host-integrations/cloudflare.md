...
## Amachais (Setup Instructions)

### 1. Fháil an API Key a Chluainn (Get Your Cloudflare API Key)
...
### 2. Fháil an ID an Zone a Chluainn (Get Your Zone ID)
...
### 3. Add Constants i wp-config.php (Add Constants to wp-config.php)
...
### 4. Foirmiadh an Iompair (Enable the Integration)
...
## Conas Aird a Bhail (How It Works)

### Bhratadh Subdomains (Subdomain Management)
...
### Sealladh cridhe DNS (DNS Record Display)
...
## Hostnames a' chlàradh Cloudflare (Cloudflare Custom Hostnames)
...
## Notean Athramanta (Important Notes)
...
## Athrachadh (Troubleshooting)

### Earrachda a' chluich an API (API Connection Issues)
...
### Cha bhith an t-sìth (Subdomain Not Added)
