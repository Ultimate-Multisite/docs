...
## 會籍狀態生命週期 {#membership-status-lifecycle}
...
### 會籍如何過渡到「到期」 {#how-memberships-transition-to-expired}
...
#### 自動續訂與非自動續訂的會籍 {#auto-renewing-vs-non-auto-renewing-memberships}
...
### 網站被封鎖時，訪客會看到什麼 {#what-visitors-see-when-a-site-is-blocked}
...
### 什麼會被封鎖，以及何時被封鎖 {#what-gets-blocked-and-when}
...
## 疑難排解：會籍到期後網站仍可存取 {#troubleshooting-sites-remaining-accessible-after-expiration}

如果會籍到期後網站仍可公開存取，請依序檢查以下項目：

### 1. 確認「封鎖前端存取權限」設定是否已啟用 {#1-verify-the-block-frontend-access-setting-is-enabled}

前往 **Ultimate Multisite > Settings > Memberships**，確認 **Block Frontend Access** 開關是否為開啟狀態。此設定 **預設為關閉**，這意味著當會籍非活躍時，只有 wp-admin 後台會被限制。

### 2. 檢查「前端封鎖寬限期」 {#2-check-the-frontend-block-grace-period}
...
### 3. 確認會籍狀態是否已實際改變 {#3-confirm-the-membership-status-has-actually-changed}
...
### 4. 驗證 Action Scheduler 是否正在運行 {#4-verify-action-scheduler-is-running}
...
### 5. 檢查閘道 Webhook 問題（自動續訂會籍） {#5-check-for-gateway-webhook-issues-auto-renewing-memberships}
...
### 6. 檢查到期寬限期（Cron 層級） {#6-check-the-expiration-grace-period-cron-level}
...
### 7. 手動到期會籍 {#7-manually-expire-a-membership}
