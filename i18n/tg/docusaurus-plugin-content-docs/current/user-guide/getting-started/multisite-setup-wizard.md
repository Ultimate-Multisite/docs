...
---
# Руководства по настройке Multisite {#multisite-setup-wizard}
...
## Кашфи Мувофиат Качон Пешниҳод мешавад? {#when-does-the-wizard-appear}
...
## Қайдҳо (Prerequisites) {#prerequisites}
...
## Қадами 1: Саломӣ (Welcome) {#step-1-welcome}
...
## Қадами 2: Ташкилоти Шабака (Network Configuration) {#step-2-network-configuration}

Ин қадам аз шумо хоҳиш мекунад, ки калитуби шабакаи худ (network settings)-ро таҳия намоед.

### Сохтори Сайти (Site Structure) {#site-structure}
...
### Номи шабака (Network Title) {#network-title}
...
### Email-и администратори шабака (Network Admin Email) {#network-admin-email}
...
## Қадами 3: Насоз кардан (Installation) {#step-3-installation}
...
## Резервное копирование при ручной настройке (Manual Setup Fallback) {#manual-setup-fallback}
...
### Мастер говорит, что wp-config.php не может быть записан {#the-wizard-says-wp-configphp-is-not-writable}
...
### Сайты недоступны после настройки (субдомены) {#sites-are-not-accessible-after-setup-subdomains}
...
### Проблемы с аутентификацией после настройки {#authentication-issues-after-setup}
...
### Произошла ошибка на каком-либо этапе установки {#a-step-failed-during-installation}
