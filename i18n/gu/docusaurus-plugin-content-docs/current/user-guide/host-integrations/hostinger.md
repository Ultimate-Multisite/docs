...
## સંક્ષિપ્ત માહિતી (Overview) {#overview}
...
## સેટઅપ સૂચનાઓ (Setup Instructions) {#setup-instructions}

### 1. તમારું Hostinger API ટોકન બનાવો (Generate Your Hostinger API Token) {#1-generate-your-hostinger-api-token}
...
### 2. તમારો એકાઉન્ટ ID શોધો (Find Your Account ID) {#2-find-your-account-id}
...
### 3. wp-config.php માં કોન્સ્ટન્ટ્સ ઉમેરો (Add Constants to wp-config.php) {#3-add-constants-to-wp-configphp}
...
### 4. integration ને સક્ષમ કરો (Enable the Integration) {#4-enable-the-integration}
...
### એડ-ઓન ડોમેન્સ (Addon Domains) {#addon-domains}
...
### સબડોમેન્સ (Subdomains) {#subdomains}
...
## મહત્વપૂર્ણ નોંધો (Important Notes) {#important-notes}
...
## મુશ્કેલીનિવારણ (Troubleshooting) {#troubleshooting}

### API કનેક્શન સમસ્યાઓ (API Connection Issues) {#api-connection-issues}
...
### ડોમેન ઉમેરાયું નથી (Domain Not Added) {#domain-not-added}
...
### SSL Certificate સમસ્યાઓ (SSL Certificate Issues) {#ssl-certificate-issues}
