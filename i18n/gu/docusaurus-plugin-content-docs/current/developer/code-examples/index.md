...
class Membership_API_Limiter {
    ...
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        if ( (int) $membership->api_calls >= self::LIMIT ) {
            return false;  // Quota કરતાં વધુ — નકારી કાઢો.
        }

        // Atomic increment: concurrent requests હેઠળ સુરક્ષિત.
...
```php
// UNSAFE — read અને write વચ્ચે race condition.
$membership = $memberships->get_item( $membership_id );
$new_count  = (int) $membership->api_calls + 1;
$memberships->update_item( $membership_id, [ 'api_calls' => $new_count ] );
```

બે એકસાથે requests એક જ value વાંચી શકે છે અને બંને પાછા સમાન incremented result લખી શકે છે, જેના કારણે એક count ખોવાઈ જાય છે. `increment_item()` ગણિતને ડેટાબેઝ engine ને એક single `UPDATE ... SET column = column + ?` statement સાથે સોંપી દે છે, જે ઓપરેશનને મૂળભૂત રીતે એટોમિક બનાવે છે.
