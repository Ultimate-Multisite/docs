---
id: wu_wp_cli_enabled_sub_commands
title: ફિલ્ટર - wu_wp_cli_enabled_sub_commands
sidebar_label: wu_wp_cli_enabled_sub_commands
_i18n_hash: 2943f23ce68dd1fdabd034c45131295c
---
# Filter: wu_wp_cli_enabled_sub_commands

આ ફિલ્ટર નક્કી કરે છે કે આ એન્ટિટી માટે કયા સબ-કમાન્ડ્સ સક્ષમ (enabled) છે.

## પેરામીટર્સ {#parameters}

| Name | Type | વર્ણન |
|------|------|-------------|
| $sub_commands | `array` | ડિફોલ્ટ સબ-કમાન્ડ્સ. |
| $command_base | `string` | રૂટ પછી કમાન્ડમાં વપરાતું બેઝ. |
| $this | `\Base_Manager` | ઓબ્જેક્ટનું ઇન્સ્ટન્સ. |

### Since {#since}

- 2.0.0
### Source {#source}

Defined in [`inc/apis/trait-wp-cli.php`](https://github.com/Ultimate-Multisite/ultimate-multisite/blob/main/inc/apis/trait-wp-cli.php#L198) at line 198
