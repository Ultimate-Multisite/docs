...
## Cap Captcha: Samostátná ochrana, která prostě funguje {#cap-captcha-self-hosted-protection-that-just-works}
...
## Ochrana proti testování kart v WooCommerce {#woocommerce-card-testing-protection}
...
## Stále potřebujete Google reCAPTCHA nebo hCaptcha? {#still-need-google-recaptcha-or-hcaptcha}
...
## Navrženo pro škálovatelnost {#built-for-scale}
...
## Musím něco nakonfigurovat? {#do-i-need-to-configure-anything}
...
## Potřebuji API klíče pro Cap Captcha? {#do-i-need-api-keys-for-cap-captcha}

Ne. Cap Captcha je zcela samostatně hostovaný. Žádné externí účty, žádná registrace domén, žádné problémy s konfigurací.

## Zpomalí to můj pokladní proces? {#will-this-slow-down-my-checkout}

Cap Captcha má pouze 20KB a provádí výpočetní výzvy v pozadí. Vaši zákazníci nezaužistí žádné zpoždění.

## Chrká to proti útokům typu testování kart? {#does-this-protect-against-card-testing-attacks}
...
## Mohu na různých stránkách použít různé poskytovatele captcha? {#can-i-use-different-captcha-providers-on-different-sites}
...
#### IP Bílé seznamy {#ip-whitelist-filter}
...
#### Akce před blokováním {#pre-block-action}
...
## Detekce IP klienta a důvěryhodné proxy {#client-ip-detection--trusted-proxies}
...
#### Důvěra v hlavičky Cloudflare {#cloudflare-header-trust}
...
#### Konfigurace důvěryhodných proxy {#trusted-proxy-configuration}
...
#### Automatická detekce při prvním spuštění {#first-enable-auto-detection}
