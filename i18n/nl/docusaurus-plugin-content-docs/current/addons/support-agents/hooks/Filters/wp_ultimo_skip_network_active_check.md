---
id: wp_ultimo_skip_network_active_check
title: Filter - wp_ultimo_skip_network_active_check
sidebar_label: wp_ultimo_skip_network_active_check
_i18n_hash: 5965dcf90793f474105b7a5f6a005e8b
---
# Filter: wp_ultimo_skip_network_active_check

Laat ontwikkelaars deze controle overslaan.

Dit is handig bij het gebruik van composer-gebaseerde en andere aangepaste setups, zoals Bedrock, bijvoorbeeld, waarbij het gebruik van plugins als mu-plugins de norm is.

### Since

- 2.0.0

### Source

Defined in [`inc/class-requirements.php`](https://github.com/Ultimate-Multisite/ultimate-multisite-support-agents/blob/main/inc/class-requirements.php#L229) at line 229


## Returns
