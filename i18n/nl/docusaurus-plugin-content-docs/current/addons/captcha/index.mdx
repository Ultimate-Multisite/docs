...
## Cap Captcha: zelfgehoste bescherming die gewoon werkt {#cap-captcha-self-hosted-protection-that-just-works}
...
## WooCommerce-bescherming tegen card testing {#woocommerce-card-testing-protection}
...
## Nog steeds Google reCAPTCHA of hCaptcha nodig? {#still-need-google-recaptcha-or-hcaptcha}
...
## Gebouwd voor schaal {#built-for-scale}
...
## Moet ik iets configureren? {#do-i-need-to-configure-anything}
...
## Heb ik API keys nodig voor Cap Captcha? {#do-i-need-api-keys-for-cap-captcha}

Nee. Cap Captcha is volledig zelfgehost. Geen externe accounts, geen domeinregistratie, geen configuratiehoofdpijn.

## Vertraagt dit mijn checkout? {#will-this-slow-down-my-checkout}
...
## Beschermt dit tegen card-testing-aanvallen? {#does-this-protect-against-card-testing-attacks}
...
## Kan ik verschillende captcha-providers op verschillende sites gebruiken? {#can-i-use-different-captcha-providers-on-different-sites}
...
## Is dit AVG-conform? {#is-this-gdpr-compliant}
...
#### IP-whitelistfilter {#ip-whitelist-filter}
...
#### Pre-block-actie {#pre-block-action}
...
## Client-IP-detectie en vertrouwde proxies {#client-ip-detection--trusted-proxies}
...
#### Vertrouwen voor Cloudflare-header {#cloudflare-header-trust}
...
#### Configuratie van vertrouwde proxy {#trusted-proxy-configuration}
...
#### Automatische detectie bij eerste inschakeling {#first-enable-auto-detection}
