...
## Бу ничек исле? {#how-it-works}

### Domain Mapping (Домен Маппинг) {#domain-mapping}
...
### Auto-SSL (Автоматик SSL) {#auto-ssl}
...
### www Subdomain (www Поддомен) {#www-subdomain}
...
### Email Forwarders (Эпочта Перенаправлениеләре) {#email-forwarders}
...
### API bağlantысы диярлык булмады (API Connection Refused) {#api-connection-refused}
...
### Аутентификация сынулары (Authentication Errors) {#authentication-errors}
...
### Домен ясалмады (Domain Not Created) {#domain-not-created}
...
### SSL сертификаты чыгарылмады (SSL Certificate Not Issued) {#ssl-certificate-not-issued}
