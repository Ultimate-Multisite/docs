...
AI коннектор настройка перечисляет поддерживаемые пулы OAuth провайдеров: Anthropic Max, OpenAI ChatGPT/Codex и Google AI Pro. Каждая карточка провайдера позволяет суперадминам подключать учетные записи, обновлять сохраненные учетные записи, удалять учетные записи по email и использовать ручной резервный метод OAuth, когда среда с ограниченным доступом блокирует перенаправление браузера. Параметры настройки Cursor Pro были удалены из панели администратора.

Учетные записи ChatGPT/Codex поддерживают использование инструментов на основе коннектора, где операция позволяет использовать инструменты, поэтому рабочие процессы администратора, зависящие от операций с поддержкой коннектора, могут использовать поведение инструмента Codex после подключения учетной записи OpenAI.
...
И нижняя часть страницы настроек платежей:
