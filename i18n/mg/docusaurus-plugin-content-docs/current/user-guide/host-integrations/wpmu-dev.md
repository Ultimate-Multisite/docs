...
## Toetran'ny Fandraisana (Features) {#features}
...
### 1. Famantarana ny Hosting an'ny WPMU DEV {#1-verify-wpmu-dev-hosting}
...
### 2. Ampahamo ny Fandraisana (Enable the Integration) {#2-enable-the-integration}
...
### Fametrahana Domain ho Azy (Domain Syncing) {#domain-syncing}
...
### Fitantanana Sertifisikaran'ny SSL (SSL Certificate Management) {#ssl-certificate-management}
...
### Fandalana Domain (Domain Removal) {#domain-removal}
...
### Fandrefesana API (API Authentication) {#api-authentication}
...
## Fanamarihana (Troubleshooting) {#troubleshooting}

### Olana Amin'ny Fampifandraisana API {#api-connection-issues}
...
### Olana Amin'ny Sertifika SSL {#ssl-certificate-issues}
