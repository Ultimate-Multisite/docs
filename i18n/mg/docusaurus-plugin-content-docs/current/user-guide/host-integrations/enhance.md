...
## Toetran'ny Fahaizana (Features) {#features}
...
### Fepetra Ara-pitaovana (System Requirements) {#system-requirements}
...
### Fidirana Amin'ny API (API Access) {#api-access}
Tsy maintsy manana fahazoan-dalana ho administrator ianao ao amin'ny Enhance Control Panel mba hamoronana API tokens.

## Famoronana ny API Credentials Anao {#getting-your-api-credentials}

### 1. Mamorona API Token {#1-create-an-api-token}
...
### 2. Fandrefesana ny Organization ID {#2-get-your-organization-id}
...
### 3. Fandrefesana ny Server ID anao {#3-get-your-server-id}
...
### 4. Fandrefesana ny API URL anao {#4-get-your-api-url}
...
### Constants Ilaina {#required-constants}
...
### Fanaovana Fanamboarana amin'ny alalan'ny Wizard (Wizard) {#setup-via-integration-wizard}
...
## Fampahafantarana WordPress Fanampiny {#additional-wordpress-configuration}
...
### Fanaovana Configuration .htaccess {#htaccess-configuration}
...
### Rehefa Mipetraka Domain iray (When a Domain is Mapped) {#when-a-domain-is-mapped}
...
### Rehefa Esorina Domain iray {#when-a-domain-is-removed}
...
### Fanamarihana DNS sy SSL {#dns-and-ssl-checking}
...
## Fanamarinana ny Fandrafetana (Verifying Setup) {#verifying-setup}

### Manandrana ny Fifandraisana (Test the Connection) {#test-the-connection}
...
### Aorian'ny Fametrahana Domain {#after-mapping-a-domain}
...
### Olana Fifandra API Connection Issues {#api-connection-issues}
...
### Fandrefesana ny fotoana (DNS Check Interval) {#dns-check-interval}
...
### Fahasamihafana amin'ny Fanamarinana (Authentication Errors) {#authentication-errors}
...
### Famakafakana ny Logs (Log Analysis) {#log-analysis}
...
## API Reference (Fanondroana API) {#api-reference}

### Fanamarinana (Authentication) {#authentication}
Ny fangatahana API rehetra dia mampiasa fanamarinana Bearer token:
```
Authorization: Bearer YOUR_TOKEN_HERE
```

### Endpoints Mahazava Ampiasaina (Common Endpoints Used) {#common-endpoints-used}
...
### Fanazavana API Feno (Full API Documentation) {#full-api-documentation}
...
### Fahaizana (Performance) {#performance}
...
### Fanaraha-maso (Monitoring) {#monitoring}
...
## Loharanon-kevitra Fanampiny (Additional Resources) {#additional-resources}
