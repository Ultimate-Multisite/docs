...
### 1. CyberPanel API-ды қосыңыз {#1-enable-the-cyberpanel-api}
...
### 2. wp-config.php-ке константаларды қосыңыз {#2-add-constants-to-wp-configphp}
...
### 3. Интеграцияны іске қосыңыз {#3-enable-the-integration}
...
### 4. Байланысты тексеріңіз {#4-verify-connectivity}
...
### Домен картасы (Domain Mapping) {#domain-mapping}
...
### www Поддомен (Subdomain) {#www-subdomain}
...
### Электрондық пошта жіберушілері (Email Forwarders) {#email-forwarders}
...
## Конфигурацияға сілтеме {#configuration-reference}
...
### API الاتصال жоқ (API Connection Refused) {#api-connection-refused}
...
### Аутентификация қателері (Authentication Errors) {#authentication-errors}
...
### Домен жасалмаған (Domain Not Created) {#domain-not-created}
...
### SSL сертификаты шығарылмаған (SSL Certificate Not Issued) {#ssl-certificate-not-issued}
