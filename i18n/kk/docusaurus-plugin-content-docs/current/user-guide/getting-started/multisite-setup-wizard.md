...
## Жоспар Қашан Көрінеді? {#when-does-the-wizard-appear}
...
## 1-қадам: Қош келдіңі бе? (Welcome) {#step-1-welcome}
...
## 2-қадам: Желі Конфигурациясы (Network Configuration) {#step-2-network-configuration}

Бұл қадам сізге желі параметрлеріңізді реттеуді сұрайды.

### Сайт Құрылымы (Site Structure) {#site-structure}
...
### Желі атауы (Network Title) {#network-title}
...
### Желі администратор маңызды электрондық хабары (Network Admin Email) {#network-admin-email}
...
## 3-ші қадам: Орнату (Installation) {#step-3-installation}
...
## Қолдану арқылы орнатудың қосымша жолы (Manual Setup Fallback) {#manual-setup-fallback}
...
### Wizard wp-config.php жазуға болатыны туралы айтады {#the-wizard-says-wp-configphp-is-not-writable}
...
### Орнатылғаннан кейін сайттар қолжетімсіз (субдомендер) {#sites-are-not-accessible-after-setup-subdomains}
...
### Орнатылғаннан кейін аутентификация мәселелері {#authentication-issues-after-setup}
...
### Орнату кезінде бір қадам сәтсіз болды {#a-step-failed-during-installation}
