---
title: Кеңейтілген код мысалдары
sidebar_position: 1
_i18n_hash: 98addf6b26f4c19754fed6e4f64a1704
---
# Кеңейтілген код мысалдары {#advanced-code-examples}

Бұл мысалдар Ultimate Multisite-пен кеңейтілген интеграция үлгілерін көрсетеді.

## Динамикалық баға белгілеу механизмі {#dynamic-pricing-engine}

Көлемдік, адалдық және маусымдық жеңілдіктерді қолданатын ережелерге негізделген баға белгілеу механизмі:
...
## Кеңейтілген сайтты дайындау {#advanced-site-provisioning}

Жоспар мүмкіндіктеріне қарай жаңа сайттарды plugin-дермен, SSL, CDN, сақтық көшірмелермен және мониторингпен автоматты түрде конфигурациялау:
...
## Таңдамалы шектеулер жүйесі {#custom-limitations-system}

Пайдалану туралы ескертулермен ресурс шектеулерін бақылау және мәжбүрлеп орындату:
...
## `increment_item()` бар BerlinDB атомдық санауышы {#berlindb-atomic-counter-with-incrementitem}

Ultimate Multisite v2.6.1 BerlinDB `Query` класына `increment_item()` әдісін қосты. Оны сандық бағандарда read-modify-write жарыстарынсыз қауіпсіз, атомдық арттыруларды орындау үшін пайдаланыңыз — бұл санауыштар, пайдалану квоталары және қатарлас сұраулар кезінде орындалатын rate-limiting тексерістері үшін пайдалы.

### Әдіс сигнатурасы {#method-signature}
...
### Әр membership бойынша API пайдалануын қадағалау {#tracking-api-usage-per-membership}

Әр membership үшін API rate limit-терін қолданудың практикалық үлгісі:
...
### Неліктен `update_item()` орнына `increment_item()` {#why-incrementitem-instead-of-updateitem}

Қарапайым read-modify-write тәсілі қатарлас сұраулар кезінде қауіпсіз емес:
...
Бір уақытта келген екі сұрау бірдей мәнді оқып, екеуі де бірдей арттырылған нәтижені қайта жазуы мүмкін, сондықтан бір санау жоғалады. `increment_item()` арифметиканы дерекқор қозғалтқышына бір `UPDATE ... SET column = column + ?` нұсқауымен тапсырады, осылайша операцияны табиғатынан атомдық етеді.
