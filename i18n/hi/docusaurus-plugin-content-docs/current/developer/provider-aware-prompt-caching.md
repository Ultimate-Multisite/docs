...
---
# प्रदाता-जागरूक प्रॉम्प्ट कैशिंग {#provider-aware-prompt-caching}
...
### कैश किया गया प्रॉम्प्ट बनाना (Creating a Cached Prompt) {#creating-a-cached-prompt}
...
### कैश किया गया प्रॉम्प्ट का उपयोग करना (Using a Cached Prompt) {#using-a-cached-prompt}
...
### कैश लाइफसाइकिल (Cache Lifecycle) {#cache-lifecycle}
...
### Gemini के लिए सर्वोत्तम अभ्यास (Best Practices for Gemini) {#best-practices-for-gemini}
...
## Azure OpenAI: प्रॉम्प्ट कैशिंग {#azure-openai-prompt-caching}

Azure OpenAI स्वचालित TTL प्रबंधन के साथ प्रॉम्प्ट कैशिंग का समर्थन करता है।

### कॉन्फ़िगरेशन (Configuration) {#configuration-1}
...
### कैशिंग सक्षम करना (Enabling Caching) {#enabling-caching}
...
### कैश हेडर (Cache Headers) {#cache-headers}
...
### कैश उपयोग की निगरानी करना (Monitoring Cache Usage) {#monitoring-cache-usage}
...
### Azure OpenAI के लिए सर्वोत्तम अभ्यास (Best Practices for Azure OpenAI) {#best-practices-for-azure-openai}
...
## OpenRouter: प्रदाता-विशिष्ट कैशिंग {#openrouter-provider-specific-caching}

OpenRouter अंतर्निहित प्रदाताओं (OpenAI, Anthropic, आदि) के माध्यम से कैशिंग का समर्थन करता है।

### कॉन्फ़िगरेशन (Configuration) {#configuration-2}
...
### OpenRouter कैशिंग का उपयोग करना (Using OpenRouter Caching) {#using-openrouter-caching}
...
### प्रदाता-विशिष्ट विकल्प (Provider-Specific Options) {#provider-specific-options}
...
### OpenRouter के लिए सर्वोत्तम अभ्यास (Best Practices for OpenRouter) {#best-practices-for-openrouter}
...
## Vertex Anthropic: कैश कंट्रोल के साथ प्रॉम्प्ट कैशिंग {#vertex-anthropic-prompt-caching-with-cache-control}

Vertex Anthropic (Google Cloud) स्पष्ट कैश कंट्रोल के साथ प्रॉम्प्ट कैशिंग का समर्थन करता है।

### कॉन्फ़िगरेशन (Configuration) {#configuration-3}
...
### Vertex Anthropic कैशिंग का उपयोग करना (Using Vertex Anthropic Caching) {#using-vertex-anthropic-caching}
...
### कैश कंट्रोल प्रकार (Cache Control Types) {#cache-control-types}

- **ephemeral**: अनुरोध की अवधि के लिए कैश करें (डिफ़ॉल्ट)
- **persistent**: कई अनुरोधों में कैश करें (यदि समर्थित है)

### कैश उपयोग की निगरानी करना (Monitoring Cache Usage) {#monitoring-cache-usage-1}
...
### Vertex Anthropic के लिए सर्वोत्तम अभ्यास (Best Practices for Vertex Anthropic) {#best-practices-for-vertex-anthropic}
...
## क्रॉस-प्रदाता कैशिंग रणनीति (Cross-Provider Caching Strategy) {#cross-provider-caching-strategy}

### एकीकृत कॉन्फ़िगरेशन (Unified Configuration) {#unified-configuration}
...
### प्रदाता का पता लगाना (Provider Detection) {#provider-detection}
...
### फॉलबैक रणनीति (Fallback Strategy) {#fallback-strategy}
...
## लागत अनुकूलन (Cost Optimization) {#cost-optimization}

### बचत की गणना करना (Calculate Savings) {#calculate-savings}
...
### अनुकूलन सुझाव (Optimization Tips) {#optimization-tips}
...
### कैश का उपयोग नहीं हो रहा है (Cache not being used) {#cache-not-being-used}
...
### कैश निर्माण विफल होना (Cache creation failing) {#cache-creation-failing}
...
### अप्रत्याशित लागतें (Unexpected costs) {#unexpected-costs}
