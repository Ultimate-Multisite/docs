---
id: wp_ultimo_ptm_page_list_after_title
title: Igikorwa - wp_ultimo_ptm_page_list_after_title
sidebar_label: wp_ultimo_ptm_page_list_after_title
_i18n_hash: 10c518d81a33e5d7d9eb0885d5179484
---
# Action: wp_ultimo_ptm_page_list_after_title

Emerera abategura plugin kongeramo buto z’inyongera ku mapaji y’urutonde

## Ibipimo {#parameters}

| Izina | Ubwoko | Ibisobanuro |
|------|------|-------------|
| $ | `\WU_Page` | Urugero rwa paji ya Multisite Ultimate |

### Guhera {#since}

- 1.8.2
### Inkomoko {#source}

Bisobanuwe muri [`views/base/list.php`](https://github.com/Ultimate-Multisite/ultimate-multisite-plugin-and-theme-manager/blob/main/views/base/list.php#L47) ku murongo wa 47
