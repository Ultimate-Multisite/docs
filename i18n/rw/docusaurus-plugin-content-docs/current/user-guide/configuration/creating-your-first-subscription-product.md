---
title: Gukora Icyemezo Cyako Cy'ibanze
sidebar_position: 6
_i18n_hash: a4a36ed896f6691eebb099208a746cd7
---
# Gukora Icyemezo Cyako Cy'Uruhuriro (v2) {#creating-your-first-subscription-product-v2}

_**IKUNDI KUGUCUZE: Urugero uyu ari ukwiko kwisi Ultimate Multisite version 2.x. Uziyo ugiye n'version 1.x,**_ **__gukora uyu wese***_ **._

Kugenzura ukugenzura uburyo bwa network yakwa no kumenya imbere imbere ubucuruzi bwawe ku bashobora bafite, ushobora kuba n'ubwoko bw'ibintu bwa subscription. Ugiye gutera ibintu byo gukora ibi? Ubwoko bw'ibintu w'ibyo mushobora kubona? Mu gihe cyacu, twagira uburyo bwawe bose bwo kumenya ibintu by'ibintu.

## Ubwoko bw'Icyemezo (Product Type) {#product-type}

N'Ultimate Multisite, ushobora gukoresha ibintu bibiri mu buryo bwa clienta wako: **plans** n'**add-ons (Order Bump)**. Add-ons mushobora guhindurwa mu byiciro bibiri: **packages** n'**services**. Twagira uburyo bwabo kandi twakwibona ibyo bifite.

  * **Plans** : Icyemezo cyiza cyane cy'Ultimate Multisite. Umuco wako ashobora kuba umukoresha (membership) gusa niba yari yakwiye n'plan. Plan igira imbere muco yawe uyu mukozi (ikiye kishobora guhindurwa ku buryo bwo kubyara) n'ibintu byo kwizera ku gihe cyo gukoresha ibyo bwa product editing page.

  * **Packages** : Add-ons y'ibintu bifite urugero mu gutera imbere uburyo bwa plan ya Ultimate Multisite. Bishobora guhindura imbere n'ubwoko bw'ibintu cyangwa gukoresha ibintu by'ingenzi (resources), plugins cyangwa themes zishobora kubyara ku plan y'umukozi wako yakwiriye. Urugero, plan ya mbere ishobora gufasha uwo muntu kugira umubare wa ibitabo 1,000 mu gihe cyo gukoresha, kandi ushobora gutera imbere package yishobora gufasha uwo nshya umubare kugeza 10,000.

  * **Services:** Add-ons atari ibintu byogera imbere ubucuruzi bwa Ultimate Multisite. Ni ibintu byo gukora ku mukozi wako kandi yari yakwiye n'plan yashobanye. Urugero, umukoresha wako ashobora gukora plan igira site runaka cyangwa yari yakwiye gukora service ikomeye cyo kubyara uwo site.

## Gukoma Icyemezo (Managing Products) {#managing-products}

Buriwe, uyu **Products** tab mu Ultimate Multisite **(Ultimate Multisite > Products)** mushobwa n'ubwoko bw'ibizigirwa (plans) mu gihe cyo gukoresha hosting w'umunsi.

Mu Ultimate Multisite, tab ya Products igaragaza uburyo bwo gukora kandi ibindi byiza bikorwa ku bijyanye n'ibizigirwa cyangwa ibikorwa by'ubwoko runaka. Ibi byiza byitera ku gukoresha ibisubizo by'ibizigirwa, uburo mu gihe cyo gukora, amafaranga, uburyo bwo gukoresha (permissions), n'ibindi.

Iki cyiciro kizabigize inama ku kumenya uburyo bwo gukoresha uburyo buhitamo mu Ultimate Multisite.

![Products list page](/img/config/products-list.png)

## Gufasha Ibibizigirwa (Adding Products) {#adding-products}

Icyiza cyangwa ibyiza, plan, package, cyangwa ikintu runaka kumenya uburyo bwo gukora ibiri mu gihe cyo gushyira umuntu ku bijyanye n'ikintu runaka ni **Ultimate Multisite > Products > Add Product**.

![Add Product button](/img/config/product-add-button.png)

Icyemezo kishobora gukoresha ibiri mu gihe cyo gushyira umuntu ku bijyanye n'ikintu runaka. Muwe, hari tabs nyinshi zishobora gufasha mu gukora ikintu runaka, kandi mu gihe kera hari sections nyinshi zishobora gukoresha mu gukora amafaranga y'ibizigirwa by'ikintu runaka, uburyo bwo kuba ari mu gihe cyiza (active state), n'isali ya ikintu runaka.

![Product edit page overview](/img/config/product-edit-full.png)

### Uburyo Bwo Gukoresha (Description) {#description}

Uburyo buhitamo ibyerekeye ubijyanye n'ikintu runaka bishobora gushyirwa binyuze mu gukoresha izina ry'ikintu runaka n'ubwoko bw'ibyo. Ibi byiza bikorwa mu gihe cyose aho umuntu ushaka kumenya ibyerekeye ikintu runaka, nk'uko mu gukora plan na amafaranga, inyandiko z'inshuro (invoices), uburyo bwo gutera ubwoko (upgrades), n'ibindi.

![Product description section](/img/config/product-description.png)

### Uburyo Bwo Gukoresha Amafaranga (Pricing Type) {#pricing-type}

Mu gihe kera mu cyemezo, amafaranga y'ibizigirwa bishobora gushyirwa.

![Pricing and save section](/img/config/product-pricing-save.png)

Ultimate Multisite ishyira uburyo buhitamo ibi byiza bikorwa mu gihe cyose. Uburyo **paid** (uhoze amafaranga) bishobora gushyira umuntu ku ngarwaho ibindi by'ibizigirwa n'uburyo bw'ibyo bishobora gukoreshwa.

### Amafaranga (Pricing) {#pricing}

Uburyo buhitamo amafaranga bishobora gushyirwa mu gihe cyose uburyo bwo gukora ikintu runaka no gihe cyo gutera amafaranga.

![Pricing and save section](/img/config/product-pricing-save.png)

dona uwo nkuru ya $29.99 n'ubwumvikana bwa 1 mwe, izakora ubwumvikana buro mu gihe cyo gukoresha. Uburyo bwa mbere, umuntu uzakora ubwumvikana buro ku gihe cyo gukoresha. Uburyo bwa mbere, umuntu uzakora ubwumvikana buro ku gihe cyo gukoresha.

### Icyemezo cyo Ubwumvikana (Billing Cycles) {#billing-cycles}

Icyemezo cyo Ubwumvikana kigisha uburyo bw'ubwumvikana bwa mbere, kandi bikorwa mu buryo bwo kwiga ku by'ibanga cyangwa imyaka itarashobora gukoreshwa.

![Pricing and save section](/img/config/product-pricing-save.png)

Urugero, umuntu yashobora kugabanya ubwumvikana buro ku gihe cyo gukoresha mu mwe kandi uburyo bwa mbere bwa 12. Ibi bikora ko umuntu uzakora ubwumvikana buro ku mwe $29.99 ku gihe cyose cy'amavu y'imyaka itarashobora gukoreshwa mu myaka 12. Bindi, uburyo bwa mbere bwa 12 bishobora gutera ubwumvikana buro ku mwe $29.99 ku gihe cyose cy'amavu y'imyaka itarashobora gukoreshwa mu myaka 12 kandi bikorwa.

### Icyemezo cyo Gukoresha (Trial Period) {#trial-period}

Gukoresha ubyumva bwa "offer trial" bikora umunyamakuru w'ubwumvikana kwibanda igihe cyo gukoresha ku kintu.

![Pricing and save section](/img/config/product-pricing-save.png)

Mu gihe cyo gukoresha, abantu bashobora gukoresha ibiri kandi batakora ubwumvikana dukoresha mu gihe cyose cy'amavu y'imyaka itarashobora gukoreshwa mu gihe cyo gukoresha.

### Icyemezo cyo Gukorera (Setup Fee) {#setup-fee}

Ushobora kandi gukoresha ubwumvikana bwa "setup fee" ku gihe cyo gukoresha ku plan yawe.

![Pricing and save section](/img/config/product-pricing-save.png)

Ibi rero, umunyamakuru w'ubwumvikana uzakora ubundi mu gihe cyo mbere (ku giti cyangwa ku gihe cyo gukoresha) (ku giti cyangwa ku gihe cyo gukoresha) kandi ubu ni igice cyo ubwumvikana w'uwo wumva wakoze mu gihe cyo gukoresha.

### Icyemezo Cy'Ubwumvikana (Active) {#active}

Icyemezo cyo "active" kigisha uko ibiri bishobora gukoreshwa ku muntu w'ubwumvikana bwa mbere.

![Active toggle](/img/config/product-active.png)

Nta hari uyu mwanya, niba hari abantu bafite ku muryango uyu, gukora toggle mu uko yashyize mu uko yashyize (disabled state) bituma muryango w'ibyo kumenya ko umuryango utagira ubushobozi bwo kuboneka mu gihe cy'ubwujere. **Abantu bafite ku muryango uyu bazuzuye** kugera ku gihe bafite uburyo bwo guhindura mu muryango w'ibyo, cyangwa bakagurwa mu muryango.

### Icyemezo cy'Isaliro (Product Image) {#product-image}

Icyemezo cy'Isaliro (**Upload Image**) bituma umukozi w'uburenganzira (network administrator) yashobora gukoresha ibyo mu media library kugira ngo atangire cyangwa yagire isaliro ry'ibintu by'ibintu byo mu kintu.

![Product image section](/img/config/product-image.png)

### Guhagarara (Delete) {#delete}

Icyemezo cy'Ushobora Guhagarara (**Delete Product**) bituma umuryango w'ibyo guhagarara mu sistema. Biba biba bifite mu gihe uburyo bwo kuboneka mu muryango w'ibyo byashyize (published).

![Delete product section](/img/config/product-delete.png)

Mu buryo bukwiye ku gukora ibyo, umuryango w'ibyo utagira mu gihe cyo guhagarara mu trash. Bityo niba uhoze guhagarara, igikorwa cyo guhagarara si cyo gutera ku muryango.

### Icyemezo cy'Icyemezo (Product Options) {#product-options}

Niba umuryango w'ibintu by'ubwujere bwo mu kintu bishobora gushyirwa mu gihe, icyemezo cy'ibintu byo mu kintu bituma umukozi w'uburenganzira yashobora guhindura ibyo byo mu kintu.

#### Icyemezo Cy'Uburyo (General) {#general}

Icyemezo cy'Iburyo (**General**) bituma ibintu byo gushyirwa mu gihe bishobora gukoreshwa mu muryango w'ibintu byo mu kintu, ntabwo bifite mu gihe cyo kuboneka mu gihe cyo guhindura ibyo byo mu kintu by'ibintu byo mu kintu.

![General tab](/img/config/product-general-tab.png)

Icyemezo cy'Isaliro ry'Uburyo (**product slug**) bituma isaliro rishobora gukoreshwa mu gihe cyo kuboneka mu URLs n'ibindi byo mu Ultimate Multisite.

Ultimate Multisite yashobora gushyira ibintu byo mu kintu bishobora kuba ari Plan, Package, na Service. Icyemezo cy'Icyemezo (**Product Options**) bituma bifite mu gihe cyo guhindura bifite mu buryo bwo mu kintu wiboneka ku muryango uyu.

Icyemezo cy'Uburyo (Customer Role) bituma umuryango w'abantu bakoresha mu gihe uburyo bwo kuboneka mu gihe cyo gukora site. Mu gihe cyo guhindura, niba hari abantu benshi bafite ku muryango w'ibintu byo mu kintu, izwiho itangwa mu gihe cyo guhindura (default) cyangwa Administrator. Icyemezo cyo default mu Ultimate Multisite cyashobora gukorwa mu **Ultimate Multisite > Settings > Login & Registration**.

![Customer role settings](/img/config/product-customer-role-settings.png)

#### Kumenyi no Gukoresha n'Ukurusha (Up & Downgrades) {#up--downgrades}

Icyo ubutabashwa bishobora gukoresha n'ukoresha (upgrade and downgrade paths) byo mu kundenye umuntu muri uburenganzira bwe.

Kugushyira ngo wumve uburyo bwo kuyitondera ibyo, gusoma uburyo bw'ibyo: uburyo umuntu wumva uburyo n'ukoresha (upgrade and downgrade) mu kintu cyiza cyane cyane. Umeze gukoresha uburyo bwo kugushyira ngo umuntu yitondera inyigisho z'ubwumvikane (learning management solutions) ku bumwe bw'ibyo: mu gihe cyose, ibi bitandukanye (Basic, Plus, n'Premium) bigenzwa kandi ibyerekeye uburyo bwo gukoresha plugins by'ingenzi kuri buri gihe (bura mu gihe cyane muri iki cyiciro, uza kumenya inama ku buryo bwo gukoresha plugins).

Niba Ultimate Multisite yitondera ibyo bikoreshwa no mu bikorwa by'ubizina cyangwa eCommerce websites, ibi gihe bigenzwa bitewe n'ibindi plugins byerekana kandi byerekana.

Mu gihe cyo, bitabwire kandi bitera ikibazo gukora umuntu wumva uburyo bwo guhagarara ku gihe cy'eCommerce (eLearning) kuko ibi gihe bigenzwa n'ubwoko bw'ikoresho, uburo mu gihe, n'ibindi byiza bitabwire.

Bityo kugira ngo twizige umuntu kandi twihagarire ikibazo, umunyamakuru w'uburenganzira (network administrator) ashobora gukora "plan group" kandi mu gihe cyo kumenya ibi plan byose umuntu yashobora guhagarara.

![Up and Downgrades tab](/img/config/product-upgrades.png)

Kugenzura "plan group", uburyo bwo gukoresha ibi plan byose muri listi ya **plan group**. Uburyo bwo gutangira (product order) bishobora kwerekana uko ibi plan bigenzwa kandi bigenzwa ku buryo bw'ibyo cyiza cyane, kuva mu gihe cyinhinduro kugeza mu gihe cyinhinduro.

Ultimate Multisite yitondera n'ubwoko bwo "order bump" (order bump feature) aho ibindi byiza bitandukanye byashobora gushyirwa kuri plan. Ibi bitangwa umuntu nk'ibindi byiza bigenzwa ku gihe cyo guhagarara cyangwa mu gihe cyinhinduro.

#### Ubwoko bw'Icyemezo (Price Variations) {#price-variations}

Urugero mu gihe cy'umwihariko, umunyarwanda w'ubwumvikane y'urwego rwa WordPress.

Urugero mu gihe cy'ubwujobozi bwo gukoresha ubwoko bw'ibyo (Price variations) bishobora guhindura uburyo bwo kwerekana imyera ya imyera ku muntu w'uburyo, aho umukozi w'urwego rwa WordPress yashobora gukoresha ibyo mu gihe cyose. Icyo kishobora kugira ngo umwe mu gihe (monthly), mu gihe cy'umwuka (quarterly), mu gihe cy'umwuka w'intaka (annual), cyangwa uburyo bw'ibindi byiza binyuze mu gihe cyose. Urugero, ushobora gukoresha duka imyera ya duka ku gihe cy'umwuka (monthly) yari $29.99, kandi ukoresha duka imyera ya intaka (annual) yari $249.99 yari yashyira umutera w'intaka.

![Price Variations tab](/img/config/product-price-variations-tab.png)

Kugira ngo uburyo bwo gukoresha ubwoko bw'ibyo (price variations), gukora **Enable Price Variations** kugira ngo yari yakoreshwa kandi ugukora **Add new Price Variation**.

![Product price variations](/img/config/product-price-variations.png)

Kugira ngo uburyo bwo gukoresha ubwoko bw'ibyo (variation), ugomba gukora ibi: gukora gihe cyangwa umwuka, igihe, n'ubwoko bw'imyera ya ubwoko. Ushobora gukoresha ubwoko bukwirwa mu gihe cyose binyuze mu gihe cyo gukora **Add new Price Variation**.

Urugero, niba duka imyera ya duka ku gihe cy'umwuka yari $29.99, ushobora gushyira:

  * **Imyaka 3** yari $79.99 (duka kintu cyane ku gihe cyo gukoresha mu gihe cyose)
  * **Imyaka 1** yari $249.99 (duka kintu cyane ku gihe cyo gukoresha mu gihe cyose)

:::tip Gukoresha Toggle ya Icyemezo Cy'umwuka ku Frontend

Ubwoko bw'ibyo (price variations) bishobora guhindura uburyo bwo kwerekana imyera y'uburyo ku frontend checkout. Kugira ngo abakoresha bashobore gukoresha ibintu mu gihe cyose (monthly / annual), ushobora gushyira **Period Selection** ikoranabuhinduka mu form ya checkout yawe. Ubwumvikane uburyo bwo gukora iki, ugomba gusoma [Checkout Forms: Adding a Period Selection Toggle](checkout-forms#adding-a-period-selection-toggle) kugira ngo ushije ibyo.
:::

#### Icyemezo (Taxes) {#taxes}

Icyemezo cy'ibyo (Taxes) bishobora gukoresha ibyo mu gihe cyose byo kwerekana imyera ya cyerekeza ku gihe cyose **Ultimate Multisite > Settings > Taxes** kandi mu gihe cyose byo gukoresha ubwoko bw'ibyo (price variations) binyuze mu gihe cyose. Kugira ngo ushije ibi, ugomba gusoma uburyo bwo gukora iki mu gihe cyose **Ultimate Multisite: Settings**.

![Taxes tab](/img/config/product-taxes.png)

Mu gihe, twaboneye uburyo mu gihe cyo gishobora kuba hari ibyemezo by'ubwoko (tax rate) bwa 7.25% bitanga abakoresha mu California (United States of America).

Iyo ubwoko bw'ubwoko (tax rate) bugaragazwa mu **Ultimate Multisite > Settings > Manage Tax Rates**, igaragaza ko gushobora guhindurwa ku rwego rwo product.

![Taxes tab](/img/config/product-taxes.png)

Kugira ngo uburyo rwo product rikoho cy'ubwoko (taxable item), gukora **Is Taxable** toggle kugira ngo yari ikoreshwa kandi ugira ngo utandire ubwoko bw'ubwoko mu dropdown ya Tax Category.

#### Site Templates {#site-templates}

Mu gihe cyiza, site templates ni WordPress websites zikurikiranwa (cloned) ku site ya umukoresha mu gihe cyo gushyiraho uburyo bwo kuboneka mu gihe cyo guhuza (subscription).

![Site Templates tab](/img/config/product-site-templates.png)

Umukozi w'ubwoko (network administrator) yitera kandi yigira ngo yifashwe site template nk'umwebwe WordPress w'ibyo byari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari yari.

Iyo **Allow Site Templates** ni ikoreshwa, abakoresha babiri bishobora gushyira templates ntabwo hari ko hari uburyo bwa checkout form, shareable link cyangwa URL parameter byo bishobora gufasha gushyira templates mu gihe cyo guhuza. Ultimate Multisite yitera uburyo bwa ibi bikoreshwa ku gihe cyo guhuza binyuze mu gukoresha ubwoko (fallback chain) mu buryo bw'ibyo byari byari yari yari yari yari yari yari yari. Icyemezo byubwoko (plan settings) bigera mbere, nyuma ibyemezo by'isohamuni ya checkout form, nyuma templates zikoreshwa cyangwa zishoborwa gukoresha binyuze mu URL. Ibi bikoreshwa byitera ko ibi byemezo byubwoko ni igihe cyiza kandi bigira ngo templates zikoreshwa ku product zitagira ngo zikoreshe.

**site template selection mode** yifashisha uburyo bw'ibyo byari byari yari yari yari yari yari yari mu gihe cyo guhuza (subscription process).

Icyo **D** **default** ni gukoresha mu gihe cyo gukora checkout form. Niba umunyamakuru w'ubwoko (network administrator) yagize igice cyo gukoresha template mu gihe cyo checkout kandi igice cyo hozeho n'ibyo, iki gihe iki setting cyozahoza ibyo byiza mu gihe cyo checkout.

Gukoresha **A** **Assign Site Template** bikora ko ugiye gukoresha template yose wiyumvirewe. Icyo ni cyo kera ko ibice byo gukoresha template mu gihe cyo checkout byihagararirwa.

Nyuma, **C** **Choose Available Site Templates** bishobora guhindura (override) templates yagizeho mu gihe cyo checkout n'ibyo utera mu gihe iki setting. Umo gukoresha template yose wiyumvirewe kandi uba umuntu ashobora gukoresha template zikoreshwa mu gihe iki.

Mu gihe cyiza, niba umunyamakuru w'ubwoko yashaka ko gukoresha template mu gihe cyo checkout, setting cyo ' _default_ ' kizaba kera. Mu gihe cyiza kandi kugira ngo utaboneho gukoresha template kandi uba umuntu ashobora gukoresha ibyo byo kwiga ku plan settings, uza kuba nziza ko ugize option ya ' _assign new template_ ' cyangwa ' _choose available site templates_ '.

#### Sites (Icyemezo) {#sites}

Icyiciro cyo **Sites** ni igice cyo gukoresha ubwoko bwa Ultimate Multisite.

![Sites tab](/img/config/product-sites.png)

Icyo setting kigaragaza indwo ya mbere y'ubwoko (maximum number of sites) uzi umuntu ashobora gukora mu gihe cyo kwiga kuwe.

Kugira ngo uboneho iki cyemezo, gizeye **limit sites** toggle kugira ngo yaba ari mu gihe cyiza kandi gizeye indwo ya mbere y'ubwoko (site allowance).

#### Visits (Ubwumvikane) {#visits}

Icyiciro cyo **Visits** ni igice cyo gukoresha uburyo bwa Ultimate Multisite bw'ubwoko. Icyo setting kigaragaza uko umuntu w'ubwoko ashobora gufata no gukoresha abantu bashobora kuba bafite amakuru (unique visitors) kuwe.

![Visits tab](/img/config/product-visits.png)

Kutera umukono w'ubucuruzi (marketing perspective), abanyamwuga b'ubwoko (network administrators) bashobora gukoresha uburyo bwa setting iryo kugira ngo babone abakiriya bakomeza kubaka plan yabo iyo ubwoko bwabaye. Icyo cyo gukora cyangwa gukemura umwihariko, ibi settings byashobora gufasha umukono w'ubwoko kuzuza no gukemura uburyo bwa traffic bukurikirana ku sites kugira ngo uburenganzira bw'ubwoko bishoboke.

Kugira ngo ushije uburyo bwa feature iryo, shyira **limit unique visits** cyangwa toggle iyo itangiraho mu gihe cy'ibyo byiza kandi ugisobanura indamiri ya abakiriya babaye mu field **unique visits quota**.

Iyo limit iryo ikoreshwa, Ultimate Multisite izagira ubushobozi bwo guhagarara kwigisha site y'abakiriya bakoresha cyangwa igisubizo kugaragaza ko limit yashobye.

#### Abakoresha (Users) {#users}

Uburyo bw'Ultimate Multisite mu 'Users' bufasha umukono w'ubwoko gufasha gukora ibi ngingo ku ko hari uburyo bwo kugabanya indamiri ya abakoresha bashobora gukoresha no kubyara.

![Users tab](/img/config/product-users.png)

Kugira ngo ushije uburyo bwa limitation, shyira **limit user** toggle mu gihe cy'ibyo byiza ukubaka ku kera (slide) ku gice cy'ibyo byiza.

Icyo cyo gukora kuri buri role wumwe washobora gufasha, shyira toggle yayo ishyiraho mu gihe cy'ibyo byiza kandi usobanura indamiri ya mbere (maximum upper limit) mu field igaragaza.

#### Post Types {#post-types}

Icyo cyo 'Post Types' kigaragaza umukono w'ubwoko gufasha gukora ibi ngingo ku ko hari uburyo bwo kubyara abantu bashobora gukoresha mu post types zikomeye muri WordPress.

![Post Types tab](/img/config/product-post-types.png)

Kugira ngo umwe y'ibyo byiza, ibyemezo n'abantu (posts and post types) ni igice cyiza cyane cyane mu gukora uburyo bwa WordPress, kandi dushobora gukoresha uburyo bwa limitation bwa Ultimate Multisite kugira ngo umukono w'ubwoko yashoboke gufasha gukemura no kubyara.

Kugira ngo ushije uburyo bwa subsystem bwa limitation, shyira **limit post types** toggle mu gihe cy'ibyo byiza ukubaka ku kera (slide) ku gice cy'ibyo byiza.

Icyo, ku gihe, kugira umuntu wese uburyo bwo gukemura ibyo byiza mu rurimi rw'ubwumvikane.

#### Disk Space (Urururo Rw'Ubwoko) {#disk-space}

Tabu ya **Disk Space** ituma abashobozi b'ubwoko bwawe bwo gukora umuntu wese uburyo bwo kwizera ubwoko bwawe bwawe.

![Disk Space tab](/img/config/product-disk-space.png)

Mu gihe cyane mu WordPress multisite, ibyerekezo by'uburyo (core files) byari yihagaragaje ku nshingano yawe yose kandi ibintu by'ubwoko bwa media n'ibyo byo gukoreshwa byari bifite uburyo bwo kwizera ibi bikorwa.

Kugira ngo ushake gukora uburyo bwo kwizera ubwoko, wumva **limit disk size per site** uburyo bwo gukoresha mu gihe cy'ubwoko (toggle) kandi ubufashe ku kinyagurura ku gice cy'ibyo byiza.

Kandi, ushaka gukora uburyo bwo kwizera uburyo bwawe bwawe mu **disk space allowance** uburyo bwo gukoresha megabytes (MB).

#### Custom Domain (Urururo Rw'Ubwoko Bw'Umuryango) {#custom-domain}

Binyuze mu gihe cy'ubwoko (toggle), ushaka gukora uburyo bwo kwizera ibi byiza ku ruhande (custom domains) kuri iyo plan igomba.

![Custom Domains tab](/img/config/product-custom-domains.png)

#### Themes (Urururo Rw'Icyemezo) {#themes}

Tabu ya **Themes** mu gihe cyo gukora ibyo byiza ku product ituma umuntu wese uburyo bwo kwizera ibi byiza ku ruhande, kandi ushaka gukora uburyo bwo kwizera uburyo bwawe.

![Themes tab](/img/config/product-themes.png)

_**Urugero: Iyo themes zishobora gufite umuntu wese uburyo bwo kwizera, abashobozi b'uburyo (network administrator) bashobora gufite ibyo byiza ku ruhande.**_

![Network themes page](/img/config/product-themes-network-enabled.png)

Uburyo bwa **visibility** buvura uburyo bwo gukoresha iyo theme kuri umuntu wese uburyo bwo kwizera, mu gihe yisobanura ibyo byiza (Appearance > Themes) mu site yawe. Kugira ngo ushake gukora uburyo bwo kwizera **Hidden**, ubufashe ku kuzuza iyo theme kandi ubwoko bwawe bwo gukoresha.

![Themes tab](/img/config/product-themes.png)

Uburyo bwa **behavior** buvura umuntu wese uburyo bwo gukora ibyo byiza mu gihe cyo gukira uburyo bwa theme ku gihe cyo gukoresha site yawe.

Mu gihe **A** **vuba** n'ubwo, uburyo bwo theme ni mu gihe cyashobora kugabanyijwe umukoresha w'umuryango (customer) kugira ngo yisobanure mu buryo bw'ibyo. Mu gihe **Nta vuba**, ibyo byitera umukoresha w'umuryango ubushobozi bwo gukorera theme. Mu gihe **Gukora Gukora** (Force Activate), uburyo bwa "Force Activate" rishobora gutera ko theme yisobanura kandi yirashobora kugira umwanya mu gihe cyerekana ku site.

#### Plugins {#plugins}

Nko mu gihe cy'Themes, Ultimate Multisite yishobora guhindurira umunyamakuru w'umuryango (network administrator) gukora ibyo plugin byitera umukoresha w'umuryango kugira ngo yisobanure kandi yirashobora kuba mu gihe cyo gukora site yashyirwe.

![Plugins tab](/img/config/product-plugins.png)

Ururu wa **visibility** (ubwumvikane) rishobora gutera ko plugin yitera umukoresha w'umuryango kugira ngo yerekane cyangwa yisobanure mu gihe yerekana ku site yashyirwe binyuze mu menu ya Plugins.

Umunyamakuru w'umuryango yashobora gukora ibyo plugin byitera umuntu kandi binyuze mu ruru wa **behavior** (uburyo bw'ibyo) kugira ngo yisobanure cyangwa yishobora guhindurwa.

![Plugins tab](/img/config/product-plugins.png)

Ururu wa **Default** rishobora gukoresha uburyo bwo plugin byitera mu gihe cyo site yashyirwe (site template) umukoresha w'umuryango yashyiraho. Bityo plugins zikoreshwa mu template yisobanura zizishobora kuze kwibuka mu site ya umukoresha w'umuryango yashyiramoho, ntabwo zishobora guhindurwa iyo template yitera (cloned) mu site ya umukoresha w'umuryango.

Ururu wa **Force Activate** rishobora gukora plugin yisobanura mu gihe cyo site yashyirwe, kandi mu gihe **Force Inactivate**, rishobora guhindura ko plugin yisobanure mu gihe cyo site yashyirwe. Mu gihe cyo gukora ibyo, umukoresha w'umuryango yashobora guhindura isobanuro ry'plugin mu menu ya WordPress Plugins.

Icyo **Force Activate & Lock** ni ifite nko mu buryo bwa mbere, ariko yituma uburyo bw'ibyo plugin cyangwa umenye bwa customer bishobora guhindura. Bityo ibyo setting ya Force Activate and Lock ituma plugin ikomeye kandi ituma customer atagushobora gukomeza (deactivate) uko. Uburyo bwa Force Inactivate & Lock, niho, ituma plugin ikomeye kandi ituma umuntu atagushobora gukomeza (activate).

Umuvugizi w'ubwoko bw'ubwoko (network administrator) yashobora gutera ku byo Force Activate & Lock n'Force Inactivate & Lock mu buryo bwa site templates. Iyo uziho, ibyo plugin cyangwa umenye bwa plugin mu templates bishobora guhindurwa n'ibyo setting bituma ari byo bikorwa.

#### Guhindura Uburyo Bwa Gukomeza (Reset Limitations) {#reset-limitations}

Icyo tab ya **Reset Limitations** ituma uburyo bwose bwa mbere bwa product bishobora gushyirwa mu gihe cy'ibyo plugin cyangwa ibindi byo. Kugira ngo utone ibyo, ugomba gukora igikorwa cy'ubwoko **reset limitations**.

![Reset Limitations tab](/img/config/product-reset-limitations.png)

Kugira ngo utone ikoranabuhindura, ugomba gukoresha toggle ya **confirm reset** kugira ngo itegeze mu gihe cy'ibyo (active state) ku kera kandi ukoreshe **reset limitations**.

![Reset Limitations tab](/img/config/product-reset-limitations.png)

## Guhindura, Gukora Uburyo Bwa Product Cyangwa Gutera Ku Rugero {#edit-duplicate-or-delete-product}

Icyo product cy'ibindi byo bishobora guhindurwa, gukorwa rugero (duplicate) cyangwa gutera ku rugero (delete) bikora mu gihe **Ultimate Multisite > Products** kandi uziho ibyo product cy'ibindi byo.

![Product hover actions](/img/config/product-hover-actions.png)
