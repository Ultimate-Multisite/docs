...
## Amagambo yo gukora (Commands to run)
...
## Uburyo bwa Gukora (Common failures)

### Ubwumvikane bw'uburyo bwa database bwa host batagira uburyo bwa host (Database grants do not match the host)
...
### Bedrock cyangwa install yashyirwaho mu rwego rwawe (Local installs) batagira ubushobozi bwa gutanga (cannot connect)
...
### Async push queue itagira ubushobozi bwa gufata (Async push queue does not drain)
...
### Icyo cyo ku rwego rwa tenant (Tenant user count) ni ryiza (wrong)
...
### Uburyo bwa SSO batagira ubushobozi (SSO visit is rejected)
