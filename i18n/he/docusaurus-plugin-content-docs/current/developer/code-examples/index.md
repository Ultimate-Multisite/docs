...
class Membership_API_Limiter {
    ...
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        if ( (int) $membership->api_calls >= self::LIMIT ) {
            return false;  // חרג מהמכסה — נדחה.
        }

        // הגדלה אטומיות: בטוח תחת בקשות מקבילות.
...
שתי בקשות בו-זמניות יכולות לקרוא את אותה ערך ולכתוב בחזרה את אותה תוצאה מוגדלת, מה שגורם לאובדן של ספירה אחת. `increment_item()` מעבירה את החישוב למנוע המידע באמצעות הצהרת `UPDATE ... SET column = column + ?` יחידה, מה שהופך את הפעולה לאטומה מטבעה.
