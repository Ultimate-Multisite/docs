...
### מחזור חיי הלקוח (Customer Lifecycle) {#customer-lifecycle}

#### לאחר יצירת לקוח (After Customer Creation) {#after-customer-creation}
...
#### שינוי סטטוס לקוח (Customer Status Change) {#customer-status-change}
...
#### לאחר פרסום האתר (After Site Published) {#after-site-published}
...
#### לפני יישום תבנית (Before Template Application) {#before-template-application}
...
#### מעברי סטטוס (Status Transitions) {#status-transitions}
...
#### תשלום הושלם / נכשל (Payment Completed / Failed) {#payment-completed--failed}
...
#### לפני עיבוד / לאחר השלמה (Before Processing / After Completion) {#before-processing--after-completion}
...
#### סך הקנייה (Cart Total) {#cart-total}
