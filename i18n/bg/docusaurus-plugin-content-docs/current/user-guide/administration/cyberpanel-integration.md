...
### 1. Активиране на CyberPanel API {#1-enable-the-cyberpanel-api}
...
### 2. Добавяне на константи в wp-config.php {#2-add-constants-to-wp-configphp}
...
### 3. Активиране на интеграцията {#3-enable-the-integration}
...
### 4. Проверка на свързаността {#4-verify-connectivity}
...
## Референтен справочник за конфигурация {#configuration-reference}
...
### API Connection Refused (Отказ на свързването с API) {#api-connection-refused}
...
### Authentication Errors (Грешки при аутентикация) {#authentication-errors}
...
### Domain Not Created (Домейнът не е създаден) {#domain-not-created}
...
### SSL Certificate Not Issued (SSL сертификатът не е издаден) {#ssl-certificate-not-issued}
