...
### Domain Mapping (Мапирање на домен)
...
### Email Forwarders (Пренасочувачи на е-пошта)
...
### Отказ од API врска (API Connection Refused)
...
### Грешки при автентикација (Authentication Errors)
...
### Доминот не е креиран (Domain Not Created)
...
### SSL сертификат не е издаден (SSL Certificate Not Issued)
