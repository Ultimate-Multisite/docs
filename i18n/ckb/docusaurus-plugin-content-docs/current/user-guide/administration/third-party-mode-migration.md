...
## چی گۆڕاوە؟ {#what-changed}

### پێش v1.12.0 {#before-v1120}
...
## کی کاریگەری دەکات؟ {#who-is-affected}

### جێبەجێکردنی نوێ (WordPress 7.0+) {#new-installations-wordpress-70}

**هیچ کارێک پێویست نییە.** مۆدی تری ئێستا بە ئۆتۆماتیک بۆ "auto" دیار دەکرێت، و تواناکان بە شێوەیەکی سروشتی کار دەکەن.

### جێبەجێکردنی ئێستا (Existing Installations) {#existing-installations}
...
### وەرگرتنی وۆرسپریدەکانی پێش 7.0 {#wordpress-versions-before-70}
...
## تێگەیشتن لە مۆدەکاندا {#understanding-the-modes}

### مۆدی ئۆتۆماتیک (Auto Mode) (پێشنی نوێ) {#auto-mode-new-default}
...
### Modela Manavur (Manual Mode) {#manual-mode}
...
### Modela کۆن (Legacy Mode) {#legacy-mode}
...
## چاودێریکردنی مۆدێل ئێستای تۆ {#checking-your-current-mode}

### لە ڕێگەی پۆینتی ئیدارە (Admin Panel) {#via-admin-panel}
...
### لە ڕێگەی کۆد (Via Code) {#via-code}
...
## گۆڕینی مۆدێلەکەت {#changing-your-mode}

### گواستنەوە بۆ Mode Auto (Auto Mode) {#switch-to-auto-mode}
...
### گواستنەوە بۆ Mode Manual (Manual Mode) {#switch-to-manual-mode}
...
### گواستنەوە بۆ Mode Legacy (Legacy Mode) {#switch-to-legacy-mode}
...
## مزایای حالت Auto Mode (خودکار) {#benefits-of-auto-mode}

### کشف خودکار قابلیت‌ها {#automatic-discovery}
...
### یکپارچگی بومی (Native Integration) {#native-integration}
...
### مدیریت ساده‌تر {#simplified-management}
...
## مسیر مهاجرت (Migration Path) {#migration-path}

### اگر روی WordPress 6.x هستید {#if-youre-on-wordpress-6x}
...
### اگر روی WordPress 7.0+ هستید {#if-youre-on-wordpress-70}
...
### قابلیت‌ها در حالت auto mode بارگذاری نمی‌شوند {#abilities-arent-loading-in-auto-mode}
...
### من می‌خواهم از حالت legacy استفاده کنم {#i-want-to-keep-legacy-mode}
...
### تواناکانی تایبەکانی خۆم نیشان ناکرێت {#my-custom-abilities-arent-showing}
...
### زه "unclassified ability" اطلاعاتي ترلاسه کوم {#im-getting-unclassified-ability-notices}
...
## پښتو ساتل (Backward Compatibility) {#backward-compatibility}

### اوسني تنظیمات {#existing-configurations}
...
### د اوسني تثبیتونو لپاره {#for-existing-installations}
...
### د ځانګړو تواناکانو لپاره {#for-custom-abilities}
