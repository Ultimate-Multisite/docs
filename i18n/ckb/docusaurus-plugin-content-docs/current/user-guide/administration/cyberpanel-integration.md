...
## Pêşkinîyên lazîk {#prerequisites}
...
## Pêşwendiyên lazîk {#requirements}
...
## چطور کار می‌کند {#setup-instructions}

### Domain Mapping (نقشه‌برداری دامنه) {#1-enable-the-cyberpanel-api}
...
### Auto-SSL (خودکار SSL) {#2-add-constants-to-wp-configphp}
...
### www Subdomain (زیردامنه www) {#3-enable-the-integration}
...
### Email Forwarders (پریدن ایمیل‌ها) {#4-verify-connectivity}
...
## مشکلي حلول (Troubleshooting) {#domain-mapping}

### API Connection Refused (پاسخ ورکړل د API اتصال) {#auto-ssl}
...
### Authentication Errors (تاییدولو غلطۍ) {#www-subdomain}
...
### Domain Not Created (دامنه نه جوړه شوې) {#email-forwarders}
...
### SSL Certificate Not Issued (SSL سند ورکړل نه شوې) {#configuration-reference}
