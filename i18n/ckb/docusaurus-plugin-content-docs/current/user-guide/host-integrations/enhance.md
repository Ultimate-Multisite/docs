...
---
# باشترکردنی ئەگرۆپ کنترۆڵ پینلەکە {#enhance-control-panel-integration}
...
### پێداویستییەکانی سیستەم {#system-requirements}
...
## گرتۆڕی API خۆت وەربگرە {#getting-your-api-credentials}
...
### ڕێکخستن لە ڕێگەی ئامێرە یەکگرتوو (Integration Wizard) {#setup-via-integration-wizard}
...
## ڕێکخستنی زیاتر بۆ WordPress {#additional-wordpress-configuration}
...
### constants-ی Cookie {#cookie-constants}
...
### کاتێک ناوچێک مۆدێل دەکرێت (When a Domain is Mapped) {#when-a-domain-is-mapped}
...
### जेव्हा डोमेन काढला जातो {#when-a-domain-is-removed}
...
### DNS आणि SSL तपासणी {#dns-and-ssl-checking}
...
## सेटअपची पडताळणी करणे {#verifying-setup}

### कनेक्शनची चाचणी घेणे {#test-the-connection}
...
### डोमेन मॅप केल्यानंतर {#after-mapping-a-domain}
...
## Çözümleme (Troubleshooting) {#troubleshooting}
...
### Alan Adı Eklenmemiş Olması {#domain-not-added}
...
### هەڵەکانی پەسەندکردن (Authentication Errors) {#authentication-errors}
...
### پەسەندکردن (Authentication) {#authentication}
هەموو داواکارییەکانی API بە ڕێگەی Bearer token یان پەسەندکردنەوەی توکن بەکاردێن:
```
Authorization: Bearer YOUR_TOKEN_HERE
```

### پۆلە باوەکان کە بەکاردێن (Common Endpoints Used) {#common-endpoints-used}
...
## پراکتیکە باشەکان (Best Practices) {#best-practices}

### భద్రత (Security) {#security}
...
### పర్యవేక్షణ (Monitoring) {#monitoring}
...
## అదనపు వనరులు (Additional Resources) {#additional-resources}
