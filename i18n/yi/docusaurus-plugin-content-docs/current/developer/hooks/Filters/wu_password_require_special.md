---
id: wu_password_require_special
title: פילטער - wu_password_require_special
sidebar_label: wu_password_require_special
_i18n_hash: 87f845abf068a92161fb7db01f88ee99
---
# פֿילטער: wu_password_require_special

פֿילטערט צי צו פֿאָדערן ספּעציעלע צייכנס אין פּאַראָלן.

## פּאַראַמעטערס {#parameters}

| נאָמען | טיפּ | באַשרײַבונג |
|------|------|-------------|
| $require | `bool` | צי צו פֿאָדערן ספּעציעלע צייכנס. ברירת־מחדל איז true ווען כּללים ווערן דורכגעפֿירט. |
| $defender_active | `bool` | צי Defender Pro Strong Password איז אַקטיוו. |

### זינט {#since}

- 2.4.0
### מקור {#source}

דעפֿינירט אין [`inc/class-scripts.php`](https://github.com/Ultimate-Multisite/ultimate-multisite/blob/main/inc/class-scripts.php#L583) בײַ שורה 583
