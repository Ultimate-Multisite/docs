---
title: 'ਪਾਠ ੫: ਆਪਣੀਆਂ ਯੋਜਨਾਵਾਂ ਦਾ ਡਿਜ਼ਾਈਨ ਕਰਨਾ'
sidebar_position: 6
_i18n_hash: 8c58dfd0a186bd585f43342bc4ea4f43
---
# Lesson 5: ਆਪਣੀਆਂ ਯੋਜਨਾਵਾਂ ਤਿਆਰ ਕਰਨਾ

ਤੁਹਾਡੇ ਪਲਾਨ ਦੇ ਪੱਧਰ ਸਿਰਫ਼ ਕੀਮਤ ਦੇ ਲੈਵਲ ਨਹੀਂ ਹਨ—ਇਹ ਇਸ ਗੱਲ ਦਾ ਪ੍ਰਤੀਬਿੰਬ ਹਨ ਕਿ ਤੁਹਾਡੇ ਖਾਸ ਗਾਹਕ (niche customers) ਅਸਲ ਵਿੱਚ ਕਿਵੇਂ ਕੰਮ ਕਰਦੇ ਹਨ। ਇਸ ਲੈਸਨ ਵਿੱਚ, ਤੁਸੀਂ ਅਜਿਹੇ ਪ੍ਰੋਡਕਟ ਪੱਧਰ ਤਿਆਰ ਕਰੋਗੇ ਜੋ ਵੱਖ-ਵੱਖ ਪੜਾਵਾਂ 'ਤੇ ਫਿਟਨੈਸ ਕਾਰੋਬਾਰਾਂ ਦੀ ਅਸਲ ਲੋੜਾਂ ਨਾਲ ਮੇਲ ਖਾਂਦੇ ਹੋਣ।

## ਜਿੱਥੇ ਅਸੀਂ ਰੁਕੇ ਸੀ

FitSite ਕੋਲ ਤਿੰਨ ਤਿਆਰ ਟੈਂਪਲੇਟ ਹਨ (Studio Essential, Gym Pro, Fitness Chain)। ਹੁਣ ਅਸੀਂ ਉਹ ਪਲਾਨ ਬਣਾਵਾਂਗੇ ਜੋ ਇਹ ਨਿਰਧਾਰਤ ਕਰਦੇ ਹਨ ਕਿ ਗਾਹਕਾਂ ਨੂੰ ਕਿਹੜੀ ਚੀਜ਼ ਤੱਕ ਪਹੁੰਚ ਮਿਲੇਗੀ ਅਤੇ ਉਹ ਕਿੰਨਾ ਭਰੋਗੇ।

## ਗਾਹਕ ਵਰਗਾਂ ਵਿੱਚ ਸੋਚਣਾ

ਜ਼ਿਆਦਾਤਰ ਲੋਕਾਂ ਦੀ ਗਲਤੀ ਇਹ ਹੁੰਦੀ ਹੈ ਕਿ ਉਹ ਤਕਨੀਕੀ ਖੇਤਰਾਂ (storage, bandwidth, pages ਦੀ ਗਿਣਤੀ) ਦੇ ਆਲੇ-ਦੁਆਲੇ ਪਲਾਨ ਬਣਾਉਂਦੇ ਹਨ। ਤੁਹਾਡੇ ਖਾਸ ਗਾਹਕ ਇਨ੍ਹਾਂ ਸ਼ਬਦਾਂ ਵਿੱਚ ਨਹੀਂ ਸੋਚਦੇ। ਇੱਕ ਜਿਮ ਮਾਲਕ ਇਸ ਬਾਰੇ ਸੋਚਦਾ ਹੈ ਕਿ ਉਸਨੂੰ ਆਪਣਾ ਕਾਰੋਬਾਰ ਚਲਾਉਣ ਲਈ ਕੀ ਚਾਹੀਦਾ ਹੈ।

ਫਿਟਨੈਸ ਸਟੂਡੀਓਜ਼ ਲਈ, ਤਿੰਨ ਕੁਦਰਤੀ ਵਰਗ (segments) ਮੌਜੂਦ ਹਨ:

| Segment | ਉਹ ਕੌਣ ਹਨ | ਉਹਨਾਂ ਨੂੰ ਕੀ ਚਾਹੀਦਾ ਹੈ |
|---------|-------------|----------------|
| **Solo trainers / small studios** | 1-3 ਸਟਾਫ਼, ਇੱਕ ਥਾਂ, ਸ਼ੁਰੂਆਤ ਕਰ ਰਹੇ | ਇੱਕ ਪੇਸ਼ੇਵਰ ਸਾਈਟ, ਕਲਾਸ ਦੀ ਜਾਣਕਾਰੀ, ਸੰਪਰਕ ਫਾਰਮ |
| **Established gyms** | 5-20 ਸਟਾਫ਼, ਇੱਕ ਥਾਂ, ਵਧ ਰਹੇ | ਉਪਰੋਕਤ ਸਭ ਕੁਝ ਪਲੱਸ ਬੁਕਿੰਗ, ਬਲੌਗ, ਗੈਲਰੀ, ਕਸਟਮ ਡੋਮੇਨ |
| **Fitness chains** | ਕਈ ਥਾਵਾਂ, ਸਥਾਪਿਤ ਬ੍ਰਾਂਡ | ਉਪਰੋਕਤ ਸਭ ਕੁਝ ਪਲੱਸ ਮਲਟੀ-ਸਾਈਟ, ਲੋਕੇਸ਼ਨ ਪੇਜ, ਸਟਾਫ਼ ਡਾਇਰੈਕਟਰੀ |

ਤੁਹਾਡੇ ਪਲਾਨ ਇਨ੍ਹਾਂ ਵਰਗਾਂ ਨਾਲ ਜੁੜੇ ਹੋਣੇ ਚਾਹੀਦੇ ਹਨ, ਨਾ ਕਿ ਮਨਮantra ਖੇਤਰਾਂ ਦੇ ਗੁੱਛੇ (arbitrary feature bundles) ਨਾਲ।

## FitSite ਪਲਾਨ ਬਣਾਉਣਾ

ਹਰੇਕ ਪਲਾਨ ਲਈ **Ultimate Multisite > Products > Add Product** 'ਤੇ ਜਾਓ।

### Plan 1: FitSite Starter -- $49/month

**Target**: ਸੋਲੋ ਟ੍ਰੇਨਰ ਅਤੇ ਸਟੂਡੀਓਜ਼

**Description tab**:
- Name: FitSite Starter
- Description: "ਹਰ ਚੀਜ਼ ਜੋ ਇੱਕ ਪਰਸਨਲ ਟ੍ਰੇਨਰ ਜਾਂ چھوٹے ਸਟੂਡੀਓ ਨੂੰ ਆਨਲਾਈਨ ਪੇਸ਼ੇਵਰ ਦਿਖਣ ਲਈ ਚਾਹੀਦੀ ਹੈ।"

**General tab**:
- Product type: Plan
- Customer role: Administrator

**Site Templates tab**:
- Allow site templates: Enabled
- Available templates: Studio Essential only

**Limitations**:
- Sites: 1
- Disk space: 1 GB
- Custom domain: Disabled (uses `studioname.fitsite.com`)

**Plugins tab**:
- Contact form plugin: Force Activate
- SEO plugin: Force Activate
- Booking plugin: Not Available (upgrade incentive)

**Themes tab**:
- Your selected theme: Force Activate
- All other themes: Hidden

### Plan 2: FitSite Growth -- $99/month

**Target**: ਸਥਾਪਿਤ ਇੱਕ-ਸਥਾਨ ਵਾਲੇ ਜਿਮ

**Description tab**:
- Name: FitSite Growth
- Description: "ਉਹ ਸਥਾਪਿਤ ਜਿਮਾਂ ਲਈ ਜੋ ਆਪਣੀ ਆਨਲਾਈਨ ਪਹਾੜੀ ਨੂੰ ਵਧਾਉਣ ਅਤੇ ਨਵੇਂ ਮੈਂਬਰ ਆਕਰਸ਼ਿਤ ਕਰਨ ਲਈ ਤਿਆਰ ਹਨ।"

**Site Templates tab**:
- Available templates: Studio Essential and Gym Pro

**Limitations**:
- Sites: 1
- Disk space: 5 GB
- Custom domain: Enabled

**Plugins tab**:
- Starter ਵਿੱਚ ਸਭ ਕੁਝ, ਪਲੱਸ:
- Booking plugin: Force Activate
- Gallery plugin: Force Activate
- Blog functionality: Available

**Up & Downgrades tab**:
- Plan group: FitSite Plans
- Product order: 2

### Plan 3: FitSite Pro -- $199/month

**Target**: ਕਈ-ਸਥਾਨ ਵਾਲੀਆਂ ਫਿਟਨੈਸ 체ਨਾਂ

**Description tab**:
- Name: FitSite Pro
- Description: "ਕਈ ਸਥਾਨਾਂ ਵਾਲੇ ਫਿਟਨੈਸ ਬ੍ਰਾਂਡਾਂ ਲਈ ਪੂਰਾ ਪਲੇਟਫਾਰਮ।"

**Site Templates tab**:
- Available templates: ਸਾਰੇ ਤਿੰਨ ਟੈਂਪਲੇਟ

**Limitations**:
- Sites: 5 (ਹਰੇਕ ਸਥਾਨ ਲਈ ਇੱਕ)
- Disk space: 20 GB
- Custom domain: Enabled

**Plugins tab**:
- Growth ਵਿੱਚ ਸਭ ਕੁਝ, ਪਲੱਸ:
- All premium plugins: Force Activate

**Up & Downgrades tab**:
- Plan group: FitSite Plans
- Product order: 3

## Plan Group ਸੈੱਟ ਕਰਨਾ

ਪਲਾਨ ਗਰੁੱਪ ਇਹ ਯਕੀਨੀ ਬਣਾਉਂਦਾ ਹੈ ਕਿ ਗਾਹਕ ਸਿਰਫ਼ FitSite ਪਲਾਨ ਫੈਮਿਲੀ ਦੇ ਅੰਦਰ ਅਪਗ੍ਰੇਡ ਜਾਂ ਡਾਊਨਗ੍ਰੇਡ ਕਰ ਸਕਣ। ਹਰੇਕ ਪਲਾਨ ਦੇ **Up & Downgrades** ਟੈਬ 'ਤੇ:

1. ਸਾਰੇ ਤਿੰਨ ਪਲਾਨਾਂ ਲਈ **Plan Group** ਨੂੰ "FitSite Plans" ਸੈੱਟ ਕਰੋ
2. **Product Order** ਨੂੰ 1 (Starter), 2 (Growth), 3 (Pro) ਸੈੱਟ ਕਰੋ

ਇਹ ਇੱਕ ਸਪੱਸ਼ਟ ਅਪਗ੍ਰੇਡ ਰਸਤਾ ਬਣਾਉਂਦਾ ਹੈ: Starter → Growth → Pro।

## Order Bumps ਜੋੜਨਾ

ਆਰਡਰ ਬੰਪਸ (Order bumps) ਚੈੱਕਆਉਟ ਦੌਰਾਨ ਦਿੱਤੇ ਜਾਣ ਵਾਲੇ ਐਡ-ਆਨ ਪ੍ਰੋਡਕਟ ਹਨ। FitSite ਲਈ, ਇਹ ਵਿਚਾਰ ਕਰੋ:

- **Extra Storage Pack** ($19/month) -- ਵਾਧੂ 5 GB ਡਿਸਕ ਸਪੇਸ
- **Priority Support** ($29/month) -- ਤੇਜ਼ ਰਿਸਪਾਂਸ ਟਾਈਮ
- **Additional Site** ($39/month) -- ਗਾਹਕਾਂ ਲਈ ਜਿਨ੍ਹਾਂ ਨੂੰ ਆਪਣੇ ਪਲਾਨ ਨਾਲੋਂ ਵੱਧ ਸਾਈਟਾਂ ਦੀ ਲੋੜ ਹੈ

ਇਹਨਾਂ ਨੂੰ Ultimate Multisite ਵਿੱਚ **Package** ਕਿਸਮ ਦੇ ਪ੍ਰੋਡਕਟ ਵਜੋਂ ਬਣਾਓ ਅਤੇ ਉਹਨਾਂ ਨੂੰ ਸਬੰਧਤ ਪਲਾਨਾਂ ਨਾਲ ਜੋੜੋ।

## ਇਹ ਢਾਂਚਾ ਕਿਉਂ ਕੰਮ ਕਰਦਾ ਹੈ

- **Starter** ਦਾ ਕੰਮ ਦਾਖਲੇ ਦੀਆਂ ਰੁਕਾਵਟਾਂ ਹਟਾਉਣਾ ਹੈ -- ਘੱਟ ਕੀਮਤ, ਸਧਾਰਨ ਆਫਰਿੰਗ, ਟ੍ਰੇਨਰਾਂ ਨੂੰ ਜਲਦੀ ਆਨਲਾਈਨ ਕਰਨਾ।
- **Growth** ਉਹ ਖੇਤਰਾਂ ਜੋ ਜਿਮ ਅਸਲ ਵਿੱਚ ਮੰਗਦੇ ਹਨ, ਉਹ ਜੋੜਦਾ ਹੈ -- ਬੁਕਿੰਗ, ਗੈਲਰੀ, ਕਸਟਮ ਡੋਮੇਨ।
- **Pro** ਉੱਚ-ਮੁੱਲ ਵਾਲੇ ਵਰਗ ਨੂੰ ਸੇਵਾ ਦਿੰਦਾ ਹੈ ਜਿਸਨੂੰ ਕਈ-ਸਥਾਨ ਸਪੋਰਟ ਦੀ ਲੋੜ ਹੁੰਦੀ ਹੈ।
- **Order bumps** ਗਾਹਕਾਂ ਨੂੰ ਮੁੱਖ ਪਲਾਨਾਂ ਨੂੰ ਗੁੰਝਲਦਾਰ ਬਣਾਏ ਬਿਨਾਂ ਕਸਟਮਾਈਜ਼ ਕਰਨ ਦਿੰਦੇ ਹਨ।
- **ਸਪੱਸ਼ਟ ਅਪਗ੍ਰੇਡ ਰਸਤਾ** ਦਾ ਮਤਲਬ ਹੈ ਕਿ ਗਾਹਕ ਤੁਹਾਡੇ ਨਾਲ ਵਧਦੇ ਹਨ, ਨਾ ਕਿ ਕਿ ਉਹ ਚਲੇ ਜਾਂਦੇ ਹਨ।

## FitSite Network ਹੁਣ ਤੱਕ

```
FitSite Network
├── WordPress Multisite (subdomain mode)
├── Ultimate Multisite (configured)
├── Hosting with wildcard SSL + domain mapping
├── Site Templates (Studio Essential, Gym Pro, Fitness Chain)
├── Products
│   ├── FitSite Starter ($49/mo) → Studio Essential template
│   ├── FitSite Growth ($99/mo) → Studio Essential + Gym Pro templates
│   ├── FitSite Pro ($199/mo) → All templates
│   └── Order Bumps (Extra Storage, Priority Support, Additional Site)
└── Ready for checkout configuration (next lesson)
```

## ਇਸ ਲੈਸਨ ਵਿੱਚ ਅਸੀਂ ਕੀ ਬਣਾਇਆ

- **ਤਿੰਨ ਪਲਾਨ ਪੱਧਰ** ਜੋ ਅਸਲ ਫਿਟਨੈਸ ਕਾਰੋਬਾਰ ਵਰਗਾਂ ਨਾਲ ਜੁੜੇ ਹਨ
- Ultimate Multisite ਦੇ ਪਲੱਗਇਨ ਅਤੇ ਟੈਂਪਲੇਟ ਕੰਟਰੋਲ ਦੀ ਵਰਤੋਂ ਕਰਕੇ **ਖੇਤਰਾਂ ਨੂੰ ਸੀਮਤ ਕਰਨਾ** (Feature gating)
- ਇੱਕ ਸਪੱਸ਼ਟ ਅਪਗ੍ਰੇਡ ਰਸਤੇ ਵਾਲਾ **ਪਲਾਨ ਗਰੁੱਪ**
- ਵਾਧੂ ਆਮਦਨ ਲਈ **Order bump ਪ੍ਰੋਡਕਟ**
- ਗਾਹਕ ਦੇ ਮੁੱਲ 'ਤੇ ਅਧਾਰਿਤ **ਕੀਮਤ ਢਾਂਚਾ**, ਨਾ ਕਿ ਤਕਨੀਕੀ ਵਿਸ਼ੇਸ਼ਤਾਵਾਂ 'ਤੇ।

---

**ਅਗਲਾ:** [Lesson 6: The Signup Experience](lesson-6-checkout) -- ਅਸੀਂ ਇੱਕ ਚੈੱਕਆਉਟ ਫਲੋ ਬਣਾਵਾਂਗੇ ਜੋ ਫਿਟਨੈਸ ਸਟੂਡੀਓ ਮਾਲਕਾਂ ਨੂੰ ਭੁਗਤਾਨ ਕਰਨ ਵਾਲੇ ਗਾਹਕਾਂ ਵਿੱਚ ਬਦਲ ਦੇਵੇਗਾ।
