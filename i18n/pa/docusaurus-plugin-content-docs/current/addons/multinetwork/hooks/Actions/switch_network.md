---
id: switch_network
title: Action - switch_network
sidebar_label: switch_network
_i18n_hash: 40e813a787afb90d335c7dd7b8c93eb2
---
# Action: switch_network

ਇਹ ਟ੍ਰਿਗਰ ਹੁੰਦਾ ਹੈ ਜਦੋਂ ਮੌਜੂਦਾ ਨੈੱਟਵਰਕ ਕੰਟੈਕਸਟ ਬਦਲਿਆ ਜਾਂਦਾ ਹੈ।

## Parameters {#parameters}

| Name | Type | Description |
|------|------|-------------|
| $new_network_id | `int` | ਉਸ ਨੈੱਟਵਰਕ ਦੀ ID ਜਿਸ 'ਤੇ ਬਦਲਾਅ ਹੋ ਰਿਹਾ ਹੈ। |
| $old_network_id | `int` | ਪਹਿਲਾਂ ਮੌਜੂਦਾ ਨੈੱਟਵਰਕ ਦੀ ID। |

### Since {#since}

- 1.3.0
### Source {#source}

Defined in [`inc/functions/network.php`](https://github.com/Ultimate-Multisite/ultimate-multisite-multinetwork/blob/main/inc/functions/network.php#L859) at line 859
