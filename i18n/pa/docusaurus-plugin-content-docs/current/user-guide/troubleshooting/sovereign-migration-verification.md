...
## चलाने वाले कमांड (Commands to run) {#commands-to-run}
...
## सामान्य विफलताएँ (Common failures) {#common-failures}

### डेटाबेस ग्रांट्स मेल नहीं खाती हैं (Database grants do not match the host) {#database-grants-do-not-match-the-host}
...
### बेडरोक या लोकल इंस्टॉलेशन कनेक्ट नहीं हो पाते (Bedrock or local installs cannot connect) {#bedrock-or-local-installs-cannot-connect}
...
### एसिंक पुश क्यू ड्रेन नहीं होती (Async push queue does not drain) {#async-push-queue-does-not-drain}
...
### टेनेंट उपयोगकर्ता की संख्या गलत है (Tenant user count is wrong) {#tenant-user-count-is-wrong}
...
### SSO विज़िट अस्वीकृत हो जाती है (SSO visit is rejected) {#sso-visit-is-rejected}
...
## दोबारा कोशिश कब करें {#when-to-retry}
