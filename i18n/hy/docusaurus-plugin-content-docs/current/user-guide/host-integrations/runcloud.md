...
### 1. Ստացեք RunCloud API ինտերֆեյսը (Get Your RunCloud API Credentials) {#1-get-your-runcloud-api-credentials}
...
### 2. Ստացեք ձեր Server ID-ն և App ID-ն (Get Your Server and App IDs) {#2-get-your-server-and-app-ids}
...
### 3. Ավելացրեք հաստատուն արժեքները wp-config.php-ին (Add Constants to wp-config.php) {#3-add-constants-to-wp-configphp}
...
### დომენი არ დაემატა {#domain-not-added}
