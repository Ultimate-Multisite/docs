...
### Համակարգային պահանջներ {#system-requirements}
...
### API Մուտք {#api-access}
Դուք պետք է ունենաք Enhance Control Panel-ի վրա ադմինիստրատոր մուտք, որպեսզի ստեղծեք API token-ներ։

## Ձեր API Տվյալների Ստացումը {#getting-your-api-credentials}

### 1. API Token Ստեղծել {#1-create-an-api-token}
...
### 2. Ստացեք ձեր Organization ID-ն (Կազմակերպության ID) {#2-get-your-organization-id}
...
### 3. Ստացեք ձեր Server ID-ն (Սերվերի ID) {#3-get-your-server-id}
...
### 4. Ստացեք ձեր API URL-ը {#4-get-your-api-url}
...
## Կազմաձևում (Configuration) {#configuration}

### Անհրաժեշտ հաստատուններ (Required Constants) {#required-constants}
...
### Ինտեգրացիայով Կարգավորում {#setup-via-integration-wizard}
...
### Cookie Հաստատուններ (Constants) {#cookie-constants}
...
## Ինչպես աշխատում է {#how-it-works}

### Երբ Տիրույթը Կարտեզագրվում է (When a Domain is Mapped) {#when-a-domain-is-mapped}
...
### Երբ Տիրույթը Հեռացվում է {#when-a-domain-is-removed}
...
### DNS և SSL Ստուգումներ {#dns-and-ssl-checking}
...
## Սահմանագրության Հաստատում {#verifying-setup}

### Կապի Ստուգում {#test-the-connection}
...
### Տիրույթի Կապումից Առաջ {#after-mapping-a-domain}
...
### SSL ցուցադրման խնդիրներ (SSL Certificate Issues) {#ssl-certificate-issues}
...
### DNS Ստուգման ինտերվալ {#dns-check-interval}
...
### Հաստատման սխալներ (Authentication Errors) {#authentication-errors}
...
### Գրանցումների վերլուծություն (Log Analysis) {#log-analysis}
...
## API Հղումներ (API Reference) {#api-reference}

### Հաստատում (Authentication) {#authentication}
Բոլոր API հարցումները օգտագործում են Bearer token հաստատում.
```
Authorization: Bearer YOUR_TOKEN_HERE
```

### Օգտագործվող ընդհանուր էնդպոինտներ (Common Endpoints Used) {#common-endpoints-used}
...
### Ամբողջական API Փաստաթուղթ (Full API Documentation) {#full-api-documentation}
Ամբողջական API փաստաթուղթ: [https://apidocs.enhance.com](https://apidocs.enhance.com)

## Ամենալավ պրակտիկաները (Best Practices) {#best-practices}
...
### Արագություն (Performance) {#performance}
...
### Մոնիթորինգ (Monitoring) {#monitoring}
...
## Ավելի լավ ռեսուրսներ {#additional-resources}
