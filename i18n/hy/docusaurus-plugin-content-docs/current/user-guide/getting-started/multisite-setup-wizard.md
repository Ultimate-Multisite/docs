...
## Ե՞րբ է հայտնվում wizard-ը {#when-does-the-wizard-appear}
...
## Քայլ 1. Բարի գալուստ (Welcome) {#step-1-welcome}
...
## Քայլ 2. Ցանցային կոնֆիգուրացիա (Network Configuration) {#step-2-network-configuration}

Այս քայլում խնդրվում է ձեզ կազմաձևել ձեր ցանցային կարգավորումները։

### Տեղադրման կառուցվածքը (Site Structure) {#site-structure}
...
### Ցանցի անուն (Network Title) {#network-title}
...
### Ցանցի ադմինիայի էլ․ հասցե (Network Admin Email) {#network-admin-email}
...
## Քայլ 3. Տեղադրում (Installation) {#step-3-installation}
...
## Քայլ 4. Ավարտել {#step-4-complete}
...
## Մասնակի սահմանման անվտանգության տարբերակ (Manual Setup Fallback) {#manual-setup-fallback}
...
### Վիզարդը ասում է, որ wp-config.php-ը գրելի չէ {#the-wizard-says-wp-configphp-is-not-writable}
...
### Սայթերը հասանելի չեն սահմանելուց հետո (սոբդոմեններ) {#sites-are-not-accessible-after-setup-subdomains}
...
### Ավտենտիկացիոն խնդիրներ սահմանումից հետո {#authentication-issues-after-setup}
...
### Տեղադրման ընթացքում ձախողվեց մի քայլ {#a-step-failed-during-installation}
