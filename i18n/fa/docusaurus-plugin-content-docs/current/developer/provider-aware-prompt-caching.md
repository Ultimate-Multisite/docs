...
### ایجاد یک Prompt کش شده (Creating a Cached Prompt) {#creating-a-cached-prompt}
...
### استفاده از یک Prompt کش شده (Using a Cached Prompt) {#using-a-cached-prompt}
...
### چرخه عمر کش (Cache Lifecycle) {#cache-lifecycle}
...
### بهترین شیوه‌ها برای Gemini (Best Practices for Gemini) {#best-practices-for-gemini}
...
## Azure OpenAI: ذخیره‌سازی Prompt {#azure-openai-prompt-caching}

Azure OpenAI از ذخیره‌سازی Prompt با مدیریت خودکار TTL پشتیبانی می‌کند.

### پیکربندی (Configuration) {#configuration-1}
...
### فعال‌سازی کش (Enabling Caching) {#enabling-caching}
...
### هدرهای کش (Cache Headers) {#cache-headers}
...
### نظارت بر استفاده از کش (Monitoring Cache Usage) {#monitoring-cache-usage}
...
### بهترین شیوه‌ها برای Azure OpenAI (Best Practices for Azure OpenAI) {#best-practices-for-azure-openai}
...
## OpenRouter: کش اختصاصی ارائه‌دهنده (Provider-Specific Caching) {#openrouter-provider-specific-caching}

OpenRouter از طریق ارائه‌دهندگان زیربنایی (OpenAI، Anthropic و غیره) از قابلیت کش پشتیبانی می‌کند.

### پیکربندی (Configuration) {#configuration-2}
...
### استفاده از کش OpenRouter (Using OpenRouter Caching) {#using-openrouter-caching}
...
### گزینه‌های اختصاصی ارائه‌دهنده (Provider-Specific Options) {#provider-specific-options}
...
### بهترین شیوه‌ها برای OpenRouter (Best Practices for OpenRouter) {#best-practices-for-openrouter}
...
## Vertex Anthropic: ذخیره‌سازی Prompt با کنترل کش {#vertex-anthropic-prompt-caching-with-cache-control}

Vertex Anthropic (Google Cloud) از ذخیره‌سازی Prompt با کنترل کش صریح پشتیبانی می‌کند.

### پیکربندی (Configuration) {#configuration-3}
...
### استفاده از کش Vertex Anthropic (Using Vertex Anthropic Caching) {#using-vertex-anthropic-caching}
...
### انواع کنترل کش (Cache Control Types) {#cache-control-types}

- **ephemeral**: کش برای مدت زمان درخواست (پیش‌فرض).
- **persistent**: کش در طول چندین درخواست (در صورت پشتیبانی).

### نظارت بر استفاده از کش (Monitoring Cache Usage) {#monitoring-cache-usage-1}
...
### بهترین شیوه‌ها برای Vertex Anthropic (Best Practices for Vertex Anthropic) {#best-practices-for-vertex-anthropic}
...
## استراتژی کش‌سازی بین ارائه‌دهندگان (Cross-Provider Caching Strategy) {#cross-provider-caching-strategy}

### پیکربندی یکپارچه (Unified Configuration) {#unified-configuration}
...
### تشخیص ارائه‌دهنده (Provider Detection) {#provider-detection}
...
### استراتژی بازگشت (Fallback Strategy) {#fallback-strategy}
...
## بهینه‌سازی هزینه (Cost Optimization) {#cost-optimization}

### محاسبه صرفه‌جویی (Calculate Savings) {#calculate-savings}
...
### نکات بهینه‌سازی (Optimization Tips) {#optimization-tips}
...
### کش استفاده نمی‌شود (Cache not being used) {#cache-not-being-used}
...
### ایجاد کش شکست می‌خورد (Cache creation failing) {#cache-creation-failing}
...
### هزینه‌های غیرمنتظره (Unexpected costs) {#unexpected-costs}
...
## مقایسه ارائه‌دهندگان (Provider Comparison) {#provider-comparison}
