---
title: نمونه‌های پیشرفته کد
sidebar_position: 1
_i18n_hash: 98addf6b26f4c19754fed6e4f64a1704
---
# مثال‌های پیشرفته کد {#advanced-code-examples}

این مثال‌ها الگوهای یکپارچه‌سازی پیشرفته با Ultimate Multisite را نشان می‌دهند.

## موتور قیمت‌گذاری پویا {#dynamic-pricing-engine}

موتوری قیمت‌گذاری مبتنی بر قوانین که تخفیف‌های حجمی، وفاداری و فصلی را اعمال می‌کند:
...
## راه‌اندازی پیشرفته سایت {#advanced-site-provisioning}

پیکربندی خودکار سایت‌های جدید با پلاگین‌ها، SSL، CDN، بک‌آپ و مانیتورینگ بر اساس ویژگی‌های پلن:
...
## سیستم محدودیت‌های سفارشی {#custom-limitations-system}

ردیابی و اعمال محدودیت منابع به همراه هشدارهای استفاده:
...
## شمارنده اتمیک BerlinDB با `increment_item()` {#berlindb-atomic-counter-with-incrementitem}

Ultimate Multisite v2.6.1 متد `increment_item()` را به کلاس `Query` مربوط به BerlinDB اضافه کرده است. از این متد برای انجام افزایش‌های ایمن و اتمیک روی ستون‌های عددی استفاده کنید؛ این کار از شرایط مسابقه (race condition) در عملیات خواندن-تغییر-نوشتن جلوگیری می‌کند و برای شمارنده‌ها، سهمیه‌بندی استفاده و بررسی محدودیت نرخ (rate-limiting) که تحت درخواست‌های همزمان اجرا می‌شوند، بسیار مفید است.

### امضای متد {#method-signature}

```php
/**
 * افزایش اتمیک یک ستون عددی برای یک آیتم خاص.
 *
 * @param int    $item_id   کلید اصلی ردیف مورد نظر برای به‌روزرسانی.
 * @param string $column    نام ستونی که باید افزایش یابد (باید عددی باشد).
 * @param int    $amount    مقدار مورد افزودن. برای کاهش، از مقدار منفی استفاده کنید.
 *                          پیش‌فرض ۱ است.
 * @return bool True در صورت موفقیت، false در صورت شکست یا اگر ستون نامعتبر باشد.
 */
...
### ردیابی استفاده از API برای هر عضویت {#tracking-api-usage-per-membership}

یک الگوی عملی برای اعمال محدودیت نرخ API در سطح هر عضویت:

```php
class Membership_API_Limiter {

    /** حداکثر تماس API مجاز در هر دوره صورت‌حساب. */
    const LIMIT = 500;
    ...
    /**
     * اگر عضویت از حد مجاز فراتر رفت، درخواست را رد می‌کند؛
     * در غیر این صورت، تماس را به صورت اتمیک شمارش می‌کند.
     *
     * @param bool   $enabled
     * @param object $context  شیئی با متد get_membership_id().
     * @return bool
     */
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        if ( (int) $membership->api_calls >= self::LIMIT ) {
            return false;  // از سهمیه فراتر رفت — رد شود.
        }

        // افزایش اتمیک: در برابر درخواست‌های همزمان ایمن است.
...
### چرا `increment_item()` به جای `update_item()` {#why-incrementitem-instead-of-updateitem}

رویکرد ساده خواندن-تغییر-نوشتن (read-modify-write) در شرایط درخواست‌های همزمان ایمن نیست:

```php
// ناامن — شرایط مسابقه بین خواندن و نوشتن وجود دارد.
$membership = $memberships->get_item( $membership_id );
$new_count  = (int) $membership->api_calls + 1;
$memberships->update_item( $membership_id, [ 'api_calls' => $new_count ] );
```

دو درخواست همزمان می‌توانند یک مقدار یکسان را بخوانند و هر دو نتیجه افزایش‌یافته یکسان را بنویسند، و در نتیجه یک شمارش از دست می‌رود. `increment_item()` عملیات ریاضی را به موتور دیتابیس با یک دستور واحد `UPDATE ... SET column = column + ?` واگذار می‌کند و این کار عملیات را ذاتاً اتمیک می‌سازد.
