---
id: wu_page_enqueue_scripts
title: Аракет - wu_page_enqueue_scripts
sidebar_label: wu_page_enqueue_scripts
_i18n_hash: 3ac07c0174f0c9930e32e5e28d72166f
---
# Аракет: wu_page_enqueue_scripts

Бул жерге ыңгайлаштырылган аракет кошулууда, ошентип керек болгондо скрипттерди жана стилдерди кезекке кошо турган жерибиз болот\ жана алардын туура барактарда гана жүктөлүшүн камсыз кылабыз

## Параметрлер {#parameters}

| Аты | Түрү | Сүрөттөмө |
|------|------|-------------|
| $ | `\WU_Page` | Учурдагы администратор барагы |

### Бери {#since}

- 1.3.0
### Булак {#source}

[`inc/class-wu-admin-pages.php`](https://github.com/Ultimate-Multisite/ultimate-multisite-admin-page-creator/blob/main/inc/class-wu-admin-pages.php#L1123) ичинде 1123-сапта аныкталган
