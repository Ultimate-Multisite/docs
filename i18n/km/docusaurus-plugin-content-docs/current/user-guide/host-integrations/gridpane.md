...
---
# ការរួមបញ្ចូលជាមួយ GridPane (GridPane Integration) {#gridpane-integration}
...
## តម្រូវការ (Requirements) {#requirements}
...
## របៀបដំឡើង (Setup Instructions) {#setup-instructions}

### ១. ទទួលបានព័ត៌មាន API របស់អ្នកពី GridPane (Get Your GridPane API Credentials) {#1-get-your-gridpane-api-credentials}
...
### ២. ទទួលបាន Server ID និង Site ID របស់អ្នក (Get Your Server and Site IDs) {#2-get-your-server-and-site-ids}
...
### ៣. បន្ថែមថេរទៅក្នុង wp-config.php (Add Constants to wp-config.php) {#3-add-constants-to-wp-configphp}
...
## ការគ្រប់គ្រងថេរ SUNRISE (SUNRISE Constant Management) {#sunrise-constant-management}
...
### បញ្ហានៅការតភ្ជាប់ API (API Connection Issues) {#api-connection-issues}
...
### បញ្ហាវញ្ញាបនបត្រ SSL (SSL Certificate Issues) {#ssl-certificate-issues}
