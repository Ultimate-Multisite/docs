...
## การรับข้อมูลประจำตัว API ของคุณ {#getting-your-api-credentials}

### 1. สร้าง API Token {#1-create-an-api-token}
...
### ២. ទទួលបាន Organization ID (ID អង្គភាព) {#2-get-your-organization-id}
...
### ៣. ទទួលបាន Server ID (ID ម៉ាស៊ីន) {#3-get-your-server-id}
...
### ៤. ទទួលបាន API URL {#4-get-your-api-url}
...
## ការកំណត់រចនាសម្ព័ន្ធ (Configuration) {#configuration}

### Constants ដែលត្រូវតម្រូវ (Required Constants) {#required-constants}
...
### ការកំណត់រចនាសម្ព័ន្ធតាមរយៈ Integration Wizard {#setup-via-integration-wizard}
...
## ការកំណត់រចនាសម្ព័ន្ធ WordPress បន្ថែម {#additional-wordpress-configuration}
...
### ការកំណត់រចនាសម្ព័ន្ធ .htaccess {#htaccess-configuration}
...
### នៅពេលដែលឈ្មោះដែនត្រូវបានបំប្លែង (When a Domain is Mapped) {#when-a-domain-is-mapped}
...
### នៅពេលដែលលុបឈ្មោះដែនចេញ {#when-a-domain-is-removed}
...
### ការពិនិត្យ DNS និង SSL {#dns-and-ssl-checking}
...
## ការផ្ទៀងផ្ទាត់ការរៀបចំ (Verifying Setup) {#verifying-setup}

### សាកល្បងការតភ្ជាប់ (Test the Connection) {#test-the-connection}
...
### បន្ទាប់ពីភ្ជាប់ឈ្មោះដែន (After Mapping a Domain) {#after-mapping-a-domain}
...
### បញ្ហាបណ្តាញ API មិនដំណើរការ (API Connection Issues) {#api-connection-issues}
...
### ដែនឈ្មោះមិនត្រូវបានបន្ថែម (Domain Not Added) {#domain-not-added}
...
### បញ្ហាប័ណ្ណ SSL (SSL Certificate Issues) {#ssl-certificate-issues}
...
### គម្លាតនៃការត្រួតពិនិត្យ DNS (DNS Check Interval) {#dns-check-interval}
...
### កំហុសនៃការផ្ទៀងផ្ទាត់អត្តសញ្ញាណ (Authentication Errors) {#authentication-errors}
...
## API Reference (ឯកសារយោង API) {#api-reference}

### ការផ្ទៀងផ្ទាត់អត្តសញ្ញាណ (Authentication) {#authentication}
ការស្នើសុំ API ទាំងអស់ប្រើការផ្ទៀងផ្ទាត់តាមរយៈ Bearer token៖
```
Authorization: Bearer YOUR_TOKEN_HERE
```

### Endpoint ដែលប្រើជាទូទៅ {#common-endpoints-used}
...
### ឯកសារ API ពេញលេញ {#full-api-documentation}
ឯកសារ API ពេញលេញ៖ [https://apidocs.enhance.com](https://apidocs.enhance.com)

## Best Practices (ការអនុវត្តល្អបំផុត) {#best-practices}

### សុវត្ថិភាព (Security) {#security}
...
### ប្រសិទ្ធភាព (Performance) {#performance}
...
### ការត្រួតពិនិត្យ (Monitoring) {#monitoring}
...
## ធនធានបន្ថែម (Additional Resources) {#additional-resources}
