---
title: Aap-custom Entwickligshëllef
sidebar_position: 10
_i18n_hash: 92675ef1c1dfa981cb05f3e328042cbc
---
# Wie kriege ich Custom Entwicklungsunterstützig vo Codeable? {#how-to-get-custom-development-help-from-codeable}

Manchmal bruchemer würkli e super Anpassige, um üsi Netzwerf-Flüssigkeit genau so z'mache wie mer wend. Wenn das isch, was Sie sueche, empfehlemer [Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE), en Ort, wo Sie erfahrni Entwickler mit Ultimate Multisite Expertise finde chönd.

[Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE) isch e Plattform, wo Kunde mit qualifizierte WordPress Freelancer zämmebringt und Projekte vo allne Grössi chönd mache. Sie händ en sorgfältige Bewerbigsprozess, damit Sie mit Experte verbunde sind, wo genau für d'Ufgab geeignet sind, wo Sie im Sinn händ.

### Wie erstellt mer es Projekt in Codeable? {#how-to-create-a-project-in-codeable}

Zuerst gaht Sie uf [Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE) und klicke uf de Knopf **Start a Project**.

<!-- Screenshot unavailable: Codeable homepage with Start a Project button -->

Jetzt, um es Projekt z'übermittle, klicke Sie links uf **Start a Project**.

<!-- Screenshot unavailable: Codeable project submission page with Start a Project option -->

In "I need help with", wähle Sie **Customizing** us:

<!-- Screenshot unavailable: Codeable project form with Customizing selected -->

In "My", wähle Sie **Plugin**:

<!-- Screenshot unavailable: Codeable project form with Plugin selected -->

Dänn klicke Sie uf de Knopf **Continue**.

<!-- Screenshot unavailable: Codeable project form Continue button -->

Jetzt isch d'Ziit, Ihr Projekt und Ihri Bedürfnis so guet wie möglich z'beschriebe. Versueche Sie, so viel Information wie möglich z'gäbe!

<!-- Screenshot unavailable: Codeable project description form -->

Antworte Sie uf die folgende Froge, um bi de Priisbestimmig z'helfe.

<!-- Screenshot unavailable: Codeable project pricing questions -->

Nachdem Sie ein [Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE) Konto noch nicht haben, erstellen Sie eines und klicken Sie auf die Schaltfläche **Publish My Project** (Mein Projekt veröffentlichen).

<!-- Screenshot unavailable: Codeable account creation and Publish My Project button -->

Sobald Sie Ihr [Codeable](https://codeable.io/partners/wpultimo/?ref=7wHwE) Projekt veröffentlicht haben, wird die Plattform Sie mit Entwicklern verbinden, die genau die Fähigkeiten haben, die für Ihr Projekt nötig sind.

Dieser Prozess hilft Ihnen dabei, die richtige Person für Ihre Arbeit zu finden, die Erfahrung mit WordPress und Ultimate Multisite hat!

Die Mindestrate von Codeable liegt bei 70–120 USD pro Stunde. Schätzungen können sich je nach Umfang der Arbeit, Komplexität und Dringlichkeit des Projekts ändern.

Sobald Sie Ihr Angebot haben, können Sie den Entwickler beauftragen, der am besten zu Ihren Bedürfnissen passt, und starten gemeinsam Ihr Projekt!
