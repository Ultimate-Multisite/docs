...
## Cap Captcha: Bảo vệ Tự lưu trữ Hoạt động Trơn tru {#cap-captcha-self-hosted-protection-that-just-works}
...
## Bảo vệ Kiểm tra Thẻ (Card Testing) của WooCommerce {#woocommerce-card-testing-protection}
...
## Vẫn cần Google reCAPTCHA hoặc hCaptcha? {#still-need-google-recaptcha-or-hcaptcha}
...
## Được xây dựng cho quy mô lớn {#built-for-scale}
...
## Tôi có cần cấu hình gì không? {#do-i-need-to-configure-anything}
...
## Tôi có cần khóa API cho Cap Captcha không? {#do-i-need-api-keys-for-cap-captcha}
...
## Điều này có làm chậm quá trình thanh toán của tôi không? {#will-this-slow-down-my-checkout}
...
## Nó có bảo vệ chống lại các cuộc tấn công kiểm tra thẻ không? {#does-this-protect-against-card-testing-attacks}
...
## Tôi có thể sử dụng các nhà cung cấp captcha khác nhau trên các trang khác nhau không? {#can-i-use-different-captcha-providers-on-different-sites}
...
## Giới hạn Tốc độ (Rate Limiting) {#rate-limiting}
...
#### Thời gian Tarpit {#tarpit-timing}
...
#### Bộ lọc Danh sách Trắng IP (IP Whitelist Filter) {#ip-whitelist-filter}
...
#### Hành động Trước khi Chặn (Pre-Block Action) {#pre-block-action}
...
## Phát hiện IP Máy khách & Proxy Tin cậy {#client-ip-detection--trusted-proxies}
...
#### Tin cậy Tiêu đề Cloudflare {#cloudflare-header-trust}
...
#### Cấu hình Proxy Tin cậy {#trusted-proxy-configuration}
...
#### Tự động Phát hiện Khi Kích hoạt Lần đầu {#first-enable-auto-detection}
