...
## Truy cập Settings {#accessing-settings}

1. Trong quản trị WordPress, đi tới **Gratis AI Agent → Settings**.
2. Nhấp vào tab **Advanced**.

## Cấu hình endpoint phản hồi {#feedback-endpoint-configuration}
...
### Payload JSON dự kiến {#expected-json-payload}
...
### Giá trị `triage_category` {#triagecategory-values}
...
### Tắt thu thập phản hồi {#disabling-feedback-collection}
...
## Dịch vụ Superdav được quản lý {#managed-superdav-service}
...
## Cấu hình Google Calendar {#google-calendar-configuration}
...
## Thông báo SMS TextBee {#textbee-sms-notifications}
...
### Truy cập Feature Flags {#accessing-feature-flags}

1. Trong quản trị WordPress, đi tới **Gratis AI Agent → Settings**.
2. Nhấp vào tab **Feature Flags**.

### Cờ kiểm soát truy cập {#access-control-flags}
...
### Cờ an toàn tự động hóa {#automation-safety-flags}
