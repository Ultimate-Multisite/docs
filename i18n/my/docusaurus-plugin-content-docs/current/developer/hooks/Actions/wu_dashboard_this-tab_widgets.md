---
id: wu_dashboard_this-tab_widgets
title: 'Action - wu_dashboard_{$this->tab}_widgets'
sidebar_label: 'wu_dashboard_{$this->tab}_widgets'
_i18n_hash: 375d02727169a29da6237374f0ad979a
---
# Action: wu_dashboard_\{$this->tab\}_widgets

ပလပ်အင် developer များအနေဖြင့် Network Dashboard Panel တွင် widget များ ထည့်သွင်းနိုင်ရန် ခွင့်ပြုပါသည်။

## Parameters

| Name | Type | Description |
|------|------|-------------|
| $tab | `string` | လက်ရှိ tab ၏ အမည်။ |
| $screen | `\WP_Screen` | မျက်နှာပြင် အရာဝတ္ထု (screen object)။ |
| $page | `\WP_Ultimo\Admin_Pages\Dashboard_Admin_Page` | Ultimate Multisite admin page ၏ instance။ |

### Since

- 2.0.0
### Source

Defined in [`inc/admin-pages/class-dashboard-admin-page.php`](https://github.com/Ultimate-Multisite/ultimate-multisite/blob/main/inc/admin-pages/class-dashboard-admin-page.php#L214) at line 214
