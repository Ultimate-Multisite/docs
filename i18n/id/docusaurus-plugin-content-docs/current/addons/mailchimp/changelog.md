---
title: Catatan Perubahan Mailchimp
sidebar_position: 99
_i18n_hash: 042b569887eb26599793e18914922c78
---
# Catatan Perubahan Mailchimp {#mailchimp-changelog}

Versi: 1.1.1 - Dirilis pada 2025-09-28

* Mengganti awalan (prefix) menjadi ultimate-multisite; memperbarui domain teks; peningkatan versi.

Versi 1.1.0 - 03/05/2018
- Perbaikan: Banyak masalah dan bug kecil;
- Tambahan: Kemampuan untuk menggunakan satu daftar (list) dan segmen melalui grup MailChimp;

Versi 1.0.0 - Rilis Awal
