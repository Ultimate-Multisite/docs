---
title: Contoh Kode Tingkat Lanjut
sidebar_position: 1
...
class Advanced_Site_Provisioner {
    ...
    public function provision_site($site, $membership) {
        ...
        // Install plugins berdasarkan plan
...
```php
/**
 * Meningkatkan kolom numerik secara atomik untuk item tertentu.
 *
 * @param int    $item_id   Primary key dari baris yang akan diperbarui.
 * @param string $column    Nama kolom yang akan ditingkatkan (harus numerik).
 * @param int    $amount    Jumlah yang akan ditambahkan. Gunakan nilai negatif untuk mengurangi.
 *                          Bawaan (default) adalah 1.
 * @return bool True jika berhasil, false jika gagal atau jika kolom tidak valid.
 */
...
// Menambahkan 5 ke penghitung penggunaan.
$memberships->increment_item( 42, 'api_calls', 5 );

// Mengurangi (mengurangi 1).
...
class Membership_API_Limiter {
    ...
    /** Batas panggilan API yang diizinkan per siklus penagihan. */
    const LIMIT = 500;
    ...
    /**
     * Menolak permintaan jika membership melebihi batas;
     * jika tidak, hitung panggilan secara atomik.
     *
     * @param bool   $enabled
     * @param object $context  Objek dengan metode get_membership_id().
     * @return bool
     */
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        if ( (int) $membership->api_calls >= self::LIMIT ) {
            return false;  // Melebihi kuota — ditolak.
        }

        // Increment atomik: aman di bawah permintaan bersamaan.
...
```php
// TIDAK AMAN — race condition antara baca dan tulis.
$membership = $memberships->get_item( $membership_id );
$new_count  = (int) $membership->api_calls + 1;
$memberships->update_item( $membership_id, [ 'api_calls' => $new_count ] );
```

Dua permintaan simultan dapat membaca nilai yang sama dan keduanya menulis kembali hasil peningkatan yang sama, sehingga kehilangan satu hitungan. `increment_item()` mendelegasikan aritmatika ke mesin database dengan pernyataan tunggal `UPDATE ... SET column = column + ?`, membuat operasi tersebut secara inheren atomik.
