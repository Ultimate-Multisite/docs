...
class Advanced_Site_Provisioner {
    ...
    public function provision_site($site, $membership) {
        ...
        // Pluginek telepítése a terv alapján
...
```php
/**
 * Atomikus növelés egy numerikus oszlopon egy adott elem számára.
 *
 * @param int    $item_id   Az frissíteni kívánt sor elsődleges kulcsa.
 * @param string $column    Az oszlop neve, amit növelni (számértéknek kell lennie).
 * @param int    $amount    Az hozzáadandó mennyiség. Negatív érték használata csökkentéshez.
 *                          Alapértelmezett értéke 1.
 * @return bool True a siker esetén, false a hiba vagy ha az oszlop nem érvényes.
 */
...
// Hozzáad 5-öt egy használati számlálóhoz.
$memberships->increment_item( 42, 'api_calls', 5 );

// Csökkentés (subtract 1).
...
class Membership_API_Limiter {
    ...
    /**
     * Elutasítja a kérést, ha a membership meghaladja a limitet;
     * különben atomikus módon számolja meg hívást.
     *
     * @param bool   $enabled
     * @param object $context  Objektum, amely get_membership_id() metódust tartalmaz.
     * @return bool
     */
    public function check_and_count( bool $enabled, $context ): bool {
        ...
        if ( (int) $membership->api_calls >= self::LIMIT ) {
            return false;  // Meghaladja a kvótát — elutasítva.
        }

        // Atomikus növelés: biztonságos párhuzamos kérések alatt.
...
```php
// NEM BIZTONSÁGOS — versenyhelyzet az olvasás és aírás között.
$membership = $memberships->get_item( $membership_id );
$new_count  = (int) $membership->api_calls + 1;
$memberships->update_item( $membership_id, [ 'api_calls' => $new_count ] );
```

Két egyidejű kérés ugyanazt az értéket olvashatja, és mindkettő ugyanazt a növelt eredményt írja vissza, így elveszítve egy számlálást. Az `increment_item()` a számításokat az adatbázis motorának küldi egyetlen `UPDATE ... SET column = column + ?` utasításával, így az művelet természeténél fogva atomikus.
