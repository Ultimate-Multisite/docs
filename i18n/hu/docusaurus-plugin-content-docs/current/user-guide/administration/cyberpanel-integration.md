...
### 1. Kapcsolja be a CyberPanel API-t {#1-enable-the-cyberpanel-api}
...
### 2. Adja hozzá a konstansokat a wp-config.php-hoz {#2-add-constants-to-wp-configphp}
...
### 3. Kapcsolja be az integrációt {#3-enable-the-integration}
...
### 4. Ellenőrizze a kapcsolatot {#4-verify-connectivity}
...
### Domain Mapping (Domain mappolás) {#domain-mapping}
...
### Auto-SSL (Automatikus SSL) {#auto-ssl}
...
### E-mail Forwarder-ek (E-mail továbbítók) {#email-forwarders}
...
## Konfigurációs hivatkozás {#configuration-reference}
...
### API Kapcsolat Elutasítása (API Connection Refused) {#api-connection-refused}
...
### Autentikációs Hibák (Authentication Errors) {#authentication-errors}
...
### Domain Nem Létrehozva (Domain Not Created) {#domain-not-created}
...
### SSL Tanúsítvány Nem Kiadva (SSL Certificate Not Issued) {#ssl-certificate-not-issued}
