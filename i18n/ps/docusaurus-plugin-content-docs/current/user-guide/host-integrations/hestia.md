...
## پیکربندی (Wizard → Integrations → Hestia) {#configuration-wizard--integrations--hestia}
...
## تنظیم کی تایید (Verifying Setup) {#verifying-setup}
...
## یادونه او نصیحتونه (Notes & Tips) {#notes--tips}
...
## مثال د API کال (cURL) {#example-api-call-curl}
...
## کورنی لینکونه (References) {#references}
