...
## د اجراsetClassلونه (Commands to run) {#commands-to-run}
...
## عام ناکامۍ (Common failures) {#common-failures}

### د ډیتابیس ګرانټونه سره توپیر لري (Database grants do not match the host) {#database-grants-do-not-match-the-host}
...
### Bedrock یا محلي هم設置 نه کولی شي وصل شي (Bedrock or local installs cannot connect) {#bedrock-or-local-installs-cannot-connect}
...
### Async push queue نه خالی کیږي (Async push queue does not drain) {#async-push-queue-does-not-drain}
...
### تانټنټ کارګرۍ شمېر غلط دی (Tenant user count is wrong) {#tenant-user-count-is-wrong}
...
### SSO وزمنه مراجعه رد شوې ده (SSO visit is rejected) {#sso-visit-is-rejected}
...
## کله بیا آزمویه गर्ने؟ {#when-to-retry}
