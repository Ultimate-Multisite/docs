---
title: Hook le Filters
sidebar_position: 13
_i18n_hash: 6da69219acbcab02df57343de4e3a145
---
# Hooks le Filters ea Ultimate Multisite

Ultimate Multisite e ntse le fona e mangata ea functions (funtšho) le database queries. E na le hooks le filters tse ka u tloha kapa ho nika code e ntlha le functions e fapaneng.

Re re ne re etsa litlhahlo tsa hooks le filters tse ntse ho ba le ho ba le sebope (reference) ho thusa batho ba nang le bohlokoa ho ba le ho u tloha ho nika ka network ea bona. U ka ba le ho ba le leliho la litlhahlo tse ka Github ka ho tsamaea link e fetang:

https://github.com/next-press/wp-ultimo-hooks-and-filters/blob/main/wp-ultimo.md
