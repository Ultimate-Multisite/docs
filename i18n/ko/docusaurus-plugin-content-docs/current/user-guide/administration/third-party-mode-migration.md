...
## 변경된 사항은 무엇인가요? {#what-changed}

### v1.12.0 이전 {#before-v1120}
...
### 신규 설치 (WordPress 7.0+) {#new-installations-wordpress-70}
...
### WordPress 7.0 이전 버전 {#wordpress-versions-before-70}
...
## 모드 이해하기 {#understanding-the-modes}

### Auto Mode (새로운 기본값) {#auto-mode-new-default}
...
## 현재 모드 확인하기 {#checking-your-current-mode}

### Admin Panel을 통해 {#via-admin-panel}
...
## 모드 변경하기 {#changing-your-mode}

### Auto Mode로 전환 {#switch-to-auto-mode}
...
### Manual Mode로 전환 {#switch-to-manual-mode}
...
### Legacy Mode로 전환 {#switch-to-legacy-mode}
...
## Auto Mode의 장점 {#benefits-of-auto-mode}

### 자동 감지 (Automatic Discovery) {#automatic-discovery}
...
### 네이티브 통합 (Native Integration) {#native-integration}
...
### 간소화된 관리 (Simplified Management) {#simplified-management}
...
### 향상된 성능 (Better Performance) {#better-performance}
...
### WordPress 6.x를 사용하는 경우 {#if-youre-on-wordpress-6x}
...
### WordPress 7.0+을 사용하는 경우 {#if-youre-on-wordpress-70}
...
### auto mode에서 abilities가 로드되지 않아요 {#abilities-arent-loading-in-auto-mode}
...
### legacy mode를 유지하고 싶어요 {#i-want-to-keep-legacy-mode}
...
### 커스텀 abilities가 표시되지 않아요 {#my-custom-abilities-arent-showing}
...
### "unclassified ability" 알림이 표시돼요 {#im-getting-unclassified-ability-notices}

- 이는 새로운 third-party abilities에 대해 정상적인 현상입니다.
- admin notice에서 검토하고 분류합니다.
- 분류에 대한 자세한 내용은 **Ability Visibility**를 확인하세요.

## 하위 호환성 (Backward Compatibility) {#backward-compatibility}

### 기존 설정 {#existing-configurations}
