...
## Livslífi Lauðningsstöðu {#membership-status-lifecycle}
...
### Hvernig Lauðningar Skipta í Lausa Stað {#how-memberships-transition-to-expired}
...
#### Sjálfuppfærandi og sjálfuppfærandi lánningarskapningar (Memberships) {#auto-renewing-vs-non-auto-renewing-memberships}
...
### Hvernig prófanir (Trials) stendja {#how-trials-end}
...
## Blokka frontend aðgang {#block-frontend-access}
...
### Innstilling af innstillingunni {#configuring-the-setting}
...
### Hvað gjöfendur sjá þegar síða er stengd {#what-visitors-see-when-a-site-is-blocked}
...
### Hva er stengt og hvornar {#what-gets-blocked-and-when}
...
## Launansaftur: Síður sem eru framkvæmdar aðgengilegar eftir lausa efni {#troubleshooting-sites-remaining-accessible-after-expiration}

Ef síður eru framkvæmdar aðgengilegar eftir það sem membership er laust, skoðaðu þessar skref í þessu forgangsordni:

### 1. Staða Block Frontend Access-setningu {#1-verify-the-block-frontend-access-setting-is-enabled}
...
### 2. Staða Block Frontend Grace Period {#2-check-the-frontend-block-grace-period}
...
### 3. Staða lausins er í raunni breytt? {#3-confirm-the-membership-status-has-actually-changed}
...
### 4. Staða Action Scheduler er í gangi? {#4-verify-action-scheduler-is-running}
...
### 5. Skennslu á vöndum við Gateway Webhook (Auto-Renewing Memberships) {#5-check-for-gateway-webhook-issues-auto-renewing-memberships}
...
### 6. Skennslu á gríðstíma fyrir lauxið (Cron Level) {#6-check-the-expiration-grace-period-cron-level}
...
### 7. Manually Expire a Membership (Manuallyykka samþykki) {#7-manually-expire-a-membership}
...
## Developer Reference (Vefirbók fyrir þróunarframkvæmdar) {#developer-reference}
