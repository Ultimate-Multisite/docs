---
title: 'Õppetund 14: Mis saab edasi'
sidebar_position: 15
...
```
FitSite võrgustik (täielik)
├── Taristu
│   ├── WordPress Multisite (alamdomeeni režiim)
│   ├── Ultimate Multisite (seadistatud + bränditud)
│   ├── Hosting wildcard SSL-iga + domeenide kaardistamine
│   └── CDN ja vahemällu salvestamine jõudluse jaoks
├── Toode
│   ├── Saidi mallid (Studio Essential, Gym Pro, Fitness Chain)
│   ├── Paketid (Starter $49/kuu, Growth $99/kuu, Pro $199/kuu)
│   ├── Aastane hinnastus 20% allahindlusega
│   ├── Seadistustasud, prooviperioodid ja tellimuse lisapakkumised
│   └── Sooduskoodid kampaaniate ja soovituste jaoks
├── Kogemus
│   ├── Nišispetsiifiline ostuvoog
│   ├── Bränditud sisselogimine, Dashboard ja suhtlus
│   ├── Juhendatud kasutuselevõtt Quick Start vidinaga
│   ├── Tervituskirjade seeria
│   └── Teadmistebaas ja videojuhendid
├── Äri
│   ├── Klientide hankimine 6 kanali kaudu
│   ├── Astmeline tugistruktuur
│   ├── Tegevusrutiinid (igapäevased, iganädalased, igakuised, kvartalipõhised)
│   ├── Mõõdikute jälgimine (MRR, churn, LTV, CAC)
│   └── Automatiseerimine webhookide ja e-kirjade seeriate kaudu
└── Kasv
    ├── Taristu skaleerimise plaan
    ├── Tulu optimeerimine (lisamüügid, aastaseks muutmine, uued teenused)
...
- **Võitluskunstide koolid** -- vööastmed, turniiriinfo, tundide hierarhia
- **Tantsustuudiod** -- esinemiste ajakavad, esinemisgaleriid, kostüümiinfo
...
- **Tundide broneerimise integratsioon** -- sügavam integratsioon populaarsete broneerimisplatvormidega
- **Liikmeportaal** -- sisselogitud ala jõusaali liikmetele ajakavade ja ressurssidega
...
- **Tervisepraktikud** -- kiropraktikud, füsioterapeudid, toitumisnõustajad
...
1. Kas neil on vaja veebisaite, kuid neil on raske häid saada?
2. Kas nad saavad endale lubada igakuist tellimust?
3. Kas nad suhtlevad omavahel?

Iga uus nišš vajab:

- Uusi selle valdkonna jaoks loodud malle
- Uusi paketistruktuure, mis peegeldavad selle valdkonna vajadusi
- Uusi turundus- ja hankekanaleid
- Uut kasutuselevõtu sisu ja tugiteadmisi
...
- `fitsite.com` -- fitnessistuudiod
- `menusite.com` -- restoranid
- `lawsite.com` -- advokaadibürood
...
FitSite 200 kliendiga ja $89 ARPU-ga:

- **MRR**: $17,800
- **ARR**: $213,600
- **Võimalik müügihind**: $640,000 - $1,068,000
...
1. **Vali nišš**, millel on selged vajadused, maksevõime ja kogukond
2. **Valideeri** enne ehitamist
3. **Ehita kohandatud platvorm** nišispetsiifiliste mallide, pakettide ja brändinguga
4. **Loo hõõrdumiseta kogemus** registreerumisest kuni kasutuselevõtuni
5. **Hinnasta väärtuse järgi** strateegiliste tasemete, prooviperioodide ja allahindlustega
6. **Käivita metoodiliselt** pehme käivituse ja kontrollnimekirjaga
7. **Hangi kliente** nišispetsiifiliste kanalite kaudu
8. **Juhi tegevusi** kestlike rutiinidega
9. **Skaleeri** taristut, tulu ja meeskonda kasvades
...
Parim aeg alustamiseks oli eile. Paremuselt teine aeg on praegu.
