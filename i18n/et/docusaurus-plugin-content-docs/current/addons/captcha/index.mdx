...
## Cap Captcha: isehostitud kaitse, mis lihtsalt töötab {#cap-captcha-self-hosted-protection-that-just-works}
...
## WooCommerce’i kaarditestimise kaitse {#woocommerce-card-testing-protection}
...
## Vajad ikka Google reCAPTCHA’t või hCaptcha’t? {#still-need-google-recaptcha-or-hcaptcha}
...
## Ehitatud skaleerumiseks {#built-for-scale}
...
## Kas ma pean midagi seadistama? {#do-i-need-to-configure-anything}
...
## Kas mul on Cap Captcha jaoks API-võtmeid vaja? {#do-i-need-api-keys-for-cap-captcha}

Ei. Cap Captcha on täielikult isehostitud. Pole väliseid kontosid, domeeni registreerimist ega seadistamise peavalu.

## Kas see aeglustab minu kassaprotsessi? {#will-this-slow-down-my-checkout}

Cap Captcha on vaid 20KB ja käitab proof-of-work väljakutseid taustal. Sinu kliendid ei märka mingit viivitust.

## Kas see kaitseb kaarditestimise rünnakute eest? {#does-this-protect-against-card-testing-attacks}
...
## Kas saan kasutada eri saitidel erinevaid captcha teenusepakkujaid? {#can-i-use-different-captcha-providers-on-different-sites}

Captcha teenusepakkuja määratakse kogu võrguüleselt, kuid saad raskusastme seadeid oma vajaduste põhjal kohandada.

## Kas see vastab GDPR-i nõuetele? {#is-this-gdpr-compliant}
...
## Kiiruse piiramine {#rate-limiting}
...
#### Tarpit’i ajastus {#tarpit-timing}
...
### Laiendatavus {#extensibility}

#### IP lubatud loendi filter {#ip-whitelist-filter}
...
#### Blokeerimiseelne tegevus {#pre-block-action}
...
## Kliendi IP tuvastamine ja usaldusväärsed puhverserverid {#client-ip-detection--trusted-proxies}
...
### Konfiguratsioon {#configuration-1}

#### Cloudflare päise usaldamine {#cloudflare-header-trust}
...
#### Usaldatud puhverserverite konfiguratsioon {#trusted-proxy-configuration}
...
#### Esmakordse lubamise automaattuvastus {#first-enable-auto-detection}
...
### Miks see on oluline {#why-this-matters}
